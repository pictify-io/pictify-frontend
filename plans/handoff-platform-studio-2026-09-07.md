...
User: *"Can we bring this experience in tools page itself rather than a full page. Check how canva and other tools do it."* and *"We will design tools one at a time. This is good for the OG image generator. We need list of pre-made template and input box for user to add the url and ai will create OG image."*

References: Canva's free tools and Figma's free tools keep the tool working inside the marketing page with expand as a second step; Slite's /editor page embeds a live editor demo above the fold; Pictify's own tool pages already run the tool inline. So: **no `/tools/template-editor` page.** The editor is embedded in each tool page in place of the code-pane + static-preview block, one tool at a time, starting with `/tools/og-image-generator`. TS-01 `KM0-0` is superseded (keep its chrome for the Expand overlay). TS-02's hub card and result-card "Open in editor" are dropped; the result card stays as it is on `8R0-0`.

Boards: **TS-03 `KW3-0`** (the OG tool page with the embedded editor after generation), **TS-04 `L1R-0`** (the same page in its start state: URL → AI, and the template gallery), notes **`L12-0`**.

Embed anatomy (1200 wide, inside the tool page content column, 1.5 px ink border, radius 12, 6 px offset shadow):
- Top strip 44 px: Design · Code · Preview segmented; status `DRAFT · KEPT IN THIS BROWSER` (field square) or `NOTHING DRAWN YET · PASTE A URL OR PICK A TEMPLATE` on start; right: **Templates 19**, **New from URL**, undo/redo (the SVG icons), **Expand ⤢**.
- Body 560 px. Start state (TS-04): headline "Paste a page URL. AI makes the OG image from what's on it.", one 720 px URL field + plum **Generate OG image**, line "About 20 seconds · nothing is saved until you download · No URL? Describe it instead · Paste HTML"; below on paper: `OR START FROM A TEMPLATE · 19` with filter chips ALL · BLOG · PRODUCT · DOCS · EVENT · MINIMAL, six thumbnails (name + category), "See all 19 →", and the line "Pick one and it opens in the editor with sample text. Paste a URL first and the template fills with your page instead." Editor state (TS-03): canvas (add tools, zoom, artboard at fit, selection chrome, status) + 340 px side panel with tabs **SAY IT · SELECTION · INPUTS**; Say it opens with a `FROM YOUR PAGE · <url>` receipt ("Read the title, description, logo and 3 colours. Used the Editorial dark template."), the make receipt, and the composer with the 3/day AI counter.
- Toolbar 64 px, identical to every tool: PNG · JPG · WEBP, size ⌄, `Sample: title = “…” · Edit`, quota squares `4 FREE TODAY · NO WATERMARK`, outlined **Save to Pictify**, ink **Download PNG** with the pink offset shadow. On start the download is mute and the sample chip reads "fills from your page".
...
Flow: URL → `GET /api/tools/website-info` (exists: title, description, logo/favicon, colours) → AI drafts the card from the chosen or default template with those values → the editor state; **Templates** opens the gallery as an overlay panel over the canvas (same six-up grid + filters + See all), picking one re-renders with the current page values or samples; **New from URL** returns to the URL band with the draft kept. Expand ⤢ opens the full studio (PS-01 chrome in tool context) as an overlay on the same page, `#editor` in the URL, Esc collapses with nothing lost. Download = the tool's public render with sample values substituted server-side (TS-3); guest rules, quota ladder, save-with-draft signup and server enforcement (TS-B1…B3) unchanged. The 19 existing files in `templates/og-image/` become the gallery; each gets a name, a category and a 1200×630 thumbnail rendered once at build time.

Tasks (replace TS-1/TS-5/TS-6; TS-2, TS-3, TS-4 and TS-B1…B3 stand):
- [ ] **TS-1a** `context="tool"` shell chrome for the Expand overlay (from §9 TS-1, minus the page) + an **EmbeddedStudio.svelte** (top strip · body · toolbar as above) that mounts StudioStage + one side panel; Layers/proof/USE IT live only in the expanded overlay.
- [ ] **TS-5a** OG image generator page: replace the current tool block with EmbeddedStudio in its start state; URL band wired to website-info → AI draft (the tool's existing `getWebsiteInfo` + `editTemplateBySaying`-style guest edit from TS-B2 with the page values as the brief); gallery from `GET /api/tools/templates/og-image/all` with names/categories/thumbnails; Templates overlay; New from URL; "No URL? Describe it" swaps the field for the description box; Paste HTML opens Code. Result card, analytics (`tool_editor_*` events with tool_name og_image_generator) and SEO copy unchanged except the hero facts line.
- [ ] **TS-6a** Verify logged-out in a fresh profile on /tools/og-image-generator: URL → card in ≈20 s with the page's title/logo/colours; pick a template → re-rendered; select/edit/say it; Code; Preview; Download twice; AI counter; reload restores the draft; Expand and Esc; Save → signup → template at rev 1. Then the same page logged in. Mobile at 390: canvas preview + Say it + Download.
