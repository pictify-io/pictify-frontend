...
| html-to-[format] (png · jpg · webp · pdf) | **none** (two zones: code + canvas) | paste **or upload .html** (TS-07) | **code** | from the format chip; PDF = page size | **Inputs** when a template proposal exists, else Say it | See §9c / TS-07 `LMJ-0` |
...
1. **Opens in Code**, code pane 460 px on the left, live canvas beside it, side panel right; **no left panel** (user 2026-09-09: "We don't need starter template for HTML to image"); click-to-select works on their own elements; Say it acts on the markup ("centre the card", "transparent background", "fit to A4").
2. **The page opens on the empty drop target (TS-07 B)**, not on a starter: the first thing a visitor sees is where to paste or drop their file.
...
Tasks:
- [ ] **TS-10** html-to-[format] page on EmbeddedStudio: no left panel; open-in-Code on the empty drop target; upload path (drop target, button, hero secondary, file read in browser, size/type checks, S3 report with the three finding kinds, "Upload the images" data-URL rewriter); per-format toolbar with the route setting the default and URL sync; capture selector picker (click on the canvas or type).
  - [x] Code-first embed, no left panel (28558eb).
  - [x] **Upload path, TS-07 B (2026-09-12)** — skipped by 28558eb, built here: the page opens on the empty drop target (no starter); the whole code zone takes a dropped file, and a full pane says "Drop to replace"; `Paste · Upload .html` in the pane header and an **Upload .html** secondary in the hero ("OR PASTE IT BELOW · ONE FILE UP TO 2 MB · .ZIP NOT SUPPORTED YET"); read with the File API, nothing sent until Download; one file, .html/.htm, ≤ 2 MB, refused with a one-line reason naming the file (.zip gets its own); the loaded card `INVOICE.HTML · 14 KB · Replace` / "Loaded · n lines · styles kept" / `WHAT WE CHANGED · n` / Continue · Upload the images; "Upload the images" matches picked files to relative paths by file name and rewrites `src` and CSS `url()` to data URLs (≤ 1 MB each), also offered under the paste report. The file goes through `setHtmlFromCode` like a paste, so undo, the report, the draft and the canvas treat it as typed text. Logic in `src/lib/tools/html-upload.js` (unit-tested); card in `HtmlUploadSummary.svelte`.
  - Deviations: the hero had no paste CTA to sit next to, so the secondary sits alone with the paste line as text. Report tones follow the existing S3 report (relative = alarm, script = field), not TS-07's (relative = field, script = alarm), because the studio shares it; "Google Fonts kept" is added as a `proof` line via `keptLines()`, shown only on the tools.
  - Found while building: the report said scripts are "removed on save" and nothing removed them — `/image/public` hands the html to the renderer as it comes. Code-first downloads now pass through `stripScripts()` (script blocks, `on*=` handlers, `javascript:` urls); the pane keeps the visitor's text.
  - [ ] Per-format toolbar with the route setting the default and URL sync; capture selector picker. Not started.
- [ ] **TS-11** Template proposal: `proposeVariables(html)` heuristic with unit tests (tokens, currency numbers, dates, codes, names after Bill to/Dear; cap 6), the Inputs card, accept → tokens + chips + Inputs rows, decline remembered per draft.
- [ ] **TS-12** Verify logged out on /tools/html-to-png and /tools/html-to-pdf: page opens on the drop target; paste → canvas follows, select on canvas → line highlighted; upload a 14 KB file with a script and two relative images → report shows all three, Continue, canvas renders; upload the images → they appear; PNG 2× transparent download; PDF A4 landscape 3-page download; template proposal on an invoice-like file → accept → Save → template at rev 1 with the variables.
  - Upload items, 2026-09-12, headless, logged out, fresh profile, /tools/html-to-png on :5199 + campaigns-r1 :3101: opens on the drop target ✓; 14 KB invoice (193 lines, one `<script>`, `img/logo.png` + CSS `url('stripe.png')`, a Google Fonts link) → card shows 2 relative paths · scripts removed · Google Fonts kept ✓, canvas already drawing it with the script not run ✓; Upload the images (both picked) → "Embedded 2 of 2", relative line gone, both visible on the canvas ✓; Continue → the file in the pane as `INVOICE.HTML` ✓; Download → `/image/public` 200 in 3.4 s, the PNG has the logo, the stripe, Inter, and "Northwind Ltd" rather than the script's text ✓; drops of a .zip, two files, 2 MB + 10 B and a .png each refused with their reason, a .htm drop replaces ✓; undo/redo across an upload ✓; reload restores the draft ✓; client-side nav from the OG tool to html-to-jpg opens empty, not on the OG card ✓; 390 px ✓ (CodePane's fixed 432 px clips its header's last button — pre-existing).
  - **/tools/html-to-pdf does not exist**: `formatExtensionMap` has no `pdf`, so the page redirects to /tools/html-to-jpg. The PDF items cannot run until that route does.
  - Not run: PNG 2× transparent (toolbar not built), template proposal → Save.
