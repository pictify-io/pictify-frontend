...
**P1 High:**
3. **Add credential caching** - Decrypt once, cache 5 minutes (90% reduction in crypto ops)
4. **Add DNS caching** - Cache resolutions for 5 minutes (eliminates 20-100ms latency)
5. **Make worker concurrency configurable** - Currently hardcoded at 10

**P2 Medium:**
6. **Add job priority by user tier** - Enterprise customers shouldn't wait behind free tier
7. **Add webhook deduplication** - Prevent duplicate deliveries from rapid-fire events
...
const { connectors, total } = await ConnectorConfig.listForUser(
  user._id, type, page, limit
);
```
