...
const teamSchema = new mongoose.Schema({
  ...
  featureUsage: {
    aiCopilot: { daily: Number, monthly: Number, total: Number, dailyReset: Date, monthlyReset: Date },
    backgroundRemover: { daily: Number, monthly: Number, total: Number, dailyReset: Date, monthlyReset: Date },
...
const rollbackMigration = async (userId, teamId) => {
  ...
  try {
    ...
    const models = [Template, Image, BrandAsset, ConnectorConfig, DataSource, Binding, Gif, Pdf];
    await Promise.all(models.map(M =>
      M.updateMany({ team: teamId }, { $unset: { team: '' } }, { session })
    ));
...
// Derived stores
export const isTeamOwner = derived(
  currentMembership,
  $m => $m?.role?.name === 'Owner'
);

export const isTeamAdmin = derived(
  currentMembership,
  $m => ['Owner', 'Admin'].includes($m?.role?.name)
);

export const permissions = derived(
  currentMembership,
  $m => $m?.role?.permissions || {}
);

export const canInviteMembers = derived(
  permissions,
  $p => $p['team.invite'] === true
);
...
const checkTeamQuota = async (request, reply) => {
  ...
  await TeamMember.updateOne(
    ...
    { $inc: { 'usageContribution.count': 1 }, $set: { 'usageContribution.lastActivity': new Date() } }
  );
};
```
