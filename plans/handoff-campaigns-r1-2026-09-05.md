...
- **W03** [x] BE-10 Brand kit (FE-20) — built 2026-09-07 on `feat/campaigns-r1`. `GET /brand-kit` → `{ revision, savedAt, website, locked, kit, usedBy[] }` where `usedBy` is the editions on the CURRENT brand rev with their own `designRevision` and state; `revision: 0` with a null kit is the empty state, not a 404. `POST /brand-kit/revisions` `{ kit, expectedRevision }` → rev n+1, `409` on a stale expectedRevision returning the current kit so the loser can look before choosing; approved editions pin the brand rev they were approved with and are never rewritten. `POST /brand-kit/detect` `{ website }` reads the PUBLIC homepage ONCE and RETURNS A PROPOSAL WITH NO SIDE EFFECT (the dialog promises "nothing is saved until you confirm" — that sentence is only true if this route writes nothing), responding `{ website, found: { logos, colours, fonts }, kit }`; unreachable/nothing-found is an error code, never an empty kit. `POST /brand-kit/assets` multipart for logos and fonts. **CORRECTION to this line as first written:** it said "private per team, never the public asset helper", and that is wrong. A logo must be fetchable BY THE RENDERER to be drawn onto a card, so a gateway-gated URL would mean teaching the render path to authenticate for every logo; it is also not customer data, being the team's own mark, usually read off their public homepage. Built on `uploadToS3` like the existing brand-assets route. The privacy that matters is preserved elsewhere and more strongly: `campaign-snapshot.pinAssets` copies the BYTES into the edition's own private storage at approval, so an approved edition never reads the live URL and cannot be changed by anything that happens to it later. `DELETE /brand-kit/assets/:uid` requires the D10 typed confirmation when the asset is used by an approved edition, and re-checks it server-side. SSRF guard on detect: the URL is user-supplied and fetched by us.
...
Implementation task: FE-20 (W03) — built 2026-09-07. Backend: BE-10 in the list above.

Deviations from the boards, and why (FE-20 build):

- **The contrast ratios are computed, not copied.** D11 prints 10.9 : 1, 19.3 : 1 and 2.4 : 1 for `#1B3A6B`, `#111111` and `#7FB3E6`. The true WCAG ratios are **11.3, 18.9 and 2.2** — verified against an independent implementation. The board's figures are illustrative, and the entire reason this screen shows a ratio rather than a verdict is that a ratio is checkable; shipping the mock's digits would have shipped a number that fails the check it invites. The classification is identical either way (Brand and Ink pass, Accent fails), so the design reads exactly as drawn. Fixtures are in `src/lib/campaigns/brand-kit.test.js`.
- **Label column is 130 px, not 124 px.** The handoff text says 124; `IVW-0` measures 130 with a 16 px gap, and 22 + 130 + 16 + 562 + 22 = 752, the card width on the board. Followed the board.
- **WCAG luminance is implemented locally rather than reused.** `$lib/video/inline-text.js` exports a `luminance()` that is a flat sRGB average for picking a readable backdrop by eye. It is not gamma-expanded and would print visibly wrong ratios (pure blue would read 12.1 : 1 instead of 8.6 : 1). Two functions, two jobs; a test guards the difference.
- ~~**No delete affordance.**~~ RESOLVED 2026-09-07: the boards now draw one (`JKM-0`/`JKQ-0` on D11, `JKU-0`/`JKY-0` on D11b) — a stacked action column per logo card, Replace over Remove. Built. Remove takes one of three paths, and the branch is the interesting part: nothing saved yet, or no APPROVED edition on the current brand rev, means it just leaves the draft (undoable with Discard, since nothing is written until Save); only a logo that is in the SAVED kit **and** pinned by an approved edition raises the D10 typed confirmation, which names those editions. A logo added to the draft a minute ago cannot be inside an edition approved last week, so confirming it would be ceremony implying a permanence that has not happened. The rule lives in `pinnedApprovedEditions()` with seven tests rather than inline in the component.
- **`rounded-full` for the tone chips.** Paper has a `--radius-pill` token; the Tailwind config deliberately stops at four steps (btn/tile/card/pane) and has no pill. Used `rounded-full` rather than widening the shape scale for one component.
- **Contrast strip matches `J18-0`/`J7I-0`** — single line on `#F4F6F4`, radius 6, 9/12 padding, 7 px alarm square, right-aligned "Darken to pass". Board ratios now read 11.3 / 18.9 / 2.2, so board and code agree.
- **Detection finds logos and fonts, not just a colour.** The existing A4 `detectBrand` returns ONE dominant colour from a screenshot — it cannot satisfy D11b's "2 logos · 4 colours · 1 font". BE-10 keeps that detector for the rendered colour (what a person actually sees) and adds a guarded homepage-markup read for logos, Google-font families and CSS palette (what the page declares). Partial results are a supported outcome per `JJI-0`, so a site that yields only a colour returns `found: { logos: 0, ... }` and the rows render NOT FOUND rather than inventing a logo.
- **`found` counts discoveries, not kit entries.** Ink and Wash are constants BE-10 supplies, so they are excluded from the count — the detector must not take credit for two defaults on the one number a buyer might check.
- **Not yet exercised in a browser.** `svelte-check` is clean, both halves are unit-tested and all five routes register, but the page has not been driven end to end against a running server.
