...
const bindingSchema = new mongoose.Schema({
  ...
  refreshPolicy: {
    ...
    onError: { type: String, enum: ['serve_stale', 'serve_error', 'serve_default'], default: 'serve_stale' }
  },
...
const coalescedRender = coalesce(async (bindingId) => {
...
async function renderBinding(bindingId, options = {}) {
  ...
  await Binding.updateOne({ uid: bindingId }, {
...
<div class="bindings-panel">
  ...
  {#if showForm || selectedBindingId}
    <BindingForm
      ...
      on:save={() => { showForm = false; selectedBindingId = null; }}
      on:cancel={() => { showForm = false; selectedBindingId = null; }}
    />
  {:else}
