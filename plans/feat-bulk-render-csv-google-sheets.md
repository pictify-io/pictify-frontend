...
<script>
  ...
  import { getTemplateById, getTemplateVariables, batchRenderTemplate, getBatchJobResults, cancelBatchJob } from '$api/template';
  ...
  <div class="flex gap-4 mb-8">
    {#each ['upload', 'map', 'progress', 'results'] as s, i}
      <div class="flex items-center gap-2">
        <div class="w-8 h-8 rounded-full border-2 border-gray-900 flex items-center justify-center
          {step === s ? 'bg-yellow-400' : i < ['upload', 'map', 'progress', 'results'].indexOf(step) ? 'bg-green-400' : 'bg-white'}">
          {i + 1}
        </div>
  ...
  {#if step === 'upload'}
    ...
    <div class="bg-white border-[3px] border-gray-900 rounded-xl shadow-[8px_8px_0_0_#1f2937] p-8">
      ...
      <p class="text-gray-600 mb-6">
        Upload a CSV file with columns matching your template variables.
        First row should be column headers.
      </p>

      <label class="block border-2 border-dashed border-gray-400 rounded-lg p-12 text-center cursor-pointer
    ...
    <div class="bg-white border-[3px] border-gray-900 rounded-xl shadow-[8px_8px_0_0_#1f2937] p-8">
      ...
      <div class="grid grid-cols-4 gap-4 mb-8">
        {#each results.filter(r => r.success).slice(0, 20) as result}
          <a href={result.url} target="_blank" class="border-2 border-gray-300 rounded-lg overflow-hidden hover:border-gray-900">
