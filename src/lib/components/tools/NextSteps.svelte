<script>
	...
	const truncate = (value, max = 1200) => {
		...
		return `${s.slice(0, max)}\n\n// … truncated for display (${
			s.length
		} chars total). Copy uses full snippet.`;
	};
	...
	function saveDraftAndOpenWorkspace() {
		if (!templateDraft) {
			toast.set({
				message: 'No template draft available for this tool yet.',
				type: 'warning',
				duration: 2000
			});
			return;
...
<section class="w-full max-w-5xl mx-auto mt-8">
	<div class="bg-white border-[3px] border-black shadow-[8px_8px_0_0_#000]">
		...
		<div class="p-6 md:p-8">
			<div class="mb-6">
				<h3 class="text-2xl md:text-3xl font-black text-black uppercase tracking-tight">
					{heading}
				</h3>
			...
			{#if showEmailForm}
				...
				>
					<div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
						<div class="flex-1">
							<h4
								class="text-lg font-black uppercase tracking-tight text-black flex items-center gap-2"
							>
			...
			{#if !isLoggedIn}
				<div
					class="mt-6 pt-6 border-t-[2px] border-dashed border-gray-300 flex flex-col sm:flex-row items-center justify-between gap-4"
				>
