<script>
	...
	const formats = [
		...
		{ id: 'CODE93', label: 'Code 93', desc: 'Compact alphanumeric', placeholder: 'TEST-93' },
		{
			id: 'codabar',
			label: 'Codabar',
			desc: 'Numeric, libraries/blood banks',
			placeholder: 'A12345B'
		},
		{ id: 'MSI', label: 'MSI', desc: 'Numeric, inventory', placeholder: '1234567' },
		{
			id: 'pharmacode',
			label: 'Pharmacode',
			desc: 'Pharma packaging (3-131070)',
			placeholder: '1234'
		}
	];
		...
		switch (format) {
			...
			case 'ITF14':
				if (!/^\d{14}$/.test(value)) return 'ITF-14 requires exactly 14 digits';
				{
					...
					if (parseInt(value[13]) !== (10 - (s % 10)) % 10)
						return 'Invalid check digit. Last digit should be ' + ((10 - (s % 10)) % 10);
				}
				break;
			case 'CODE39':
				if (!/^[A-Z0-9\-\.\ \$\/\+\%]+$/i.test(value))
					return 'Code 39 supports A-Z, 0-9, - . $ / + % and space';
				break;
			case 'codabar':
				if (!/^[A-Da-d][0-9\-\$\:\/\.\+]+[A-Da-d]$/.test(value))
					return 'Codabar must start and end with A-D, digits and - $ : / . + in between';
				break;
			case 'MSI':
	...
	async function generateImage() {
		...
		try {
			...
			const jsBlock = `JsBarcode("#barcode", "${barcodeValue.replace(
				/"/g,
				'\\"'
			)}", { format: "${barcodeFormat}", displayValue: ${showText}, width: ${barWidth}, height: ${barHeight}, lineColor: "${barColor}", background: "${effectiveBg}", fontSize: ${fontSize}, font: "Inter", textMargin: 8, margin: 20 });`;
			...
			const html =
				'<html><head>' +
				'<scr' +
				'ipt src="https://cdn.jsdelivr.net/npm/jsbarcode@3.11.6/dist/JsBarcode.all.min.js"></scr' +
				'ipt>' +
...
<div class="w-full max-w-6xl mx-auto">
	<!-- Editor Window -->
	<div class="bg-brand-paper border border-brand-ink">
		<!-- Window Header -->
		<div
			class="bg-brand-ink text-white px-4 py-2.5 flex justify-between items-center border-b border-brand-ink"
		>
			<div class="flex gap-2">
				<div class="w-3 h-3 rounded-full bg-brand-pink border border-white" />
				<div class="w-3 h-3 rounded-full bg-brand-field border border-white" />
				<div class="w-3 h-3 rounded-full bg-brand-proof border border-white" />
			</div>
			<div class="font-mono font-bold tracking-widest text-xs">BARCODE_GENERATOR</div>
			<div class="w-16" />
		</div>
		...
		<div class="grid grid-cols-1 lg:grid-cols-[340px,1fr]">
			<!-- Left: Controls -->
			<div
				class="p-5 border-b-[3px] lg:border-b-0 lg:border-r border-brand-rule space-y-5 bg-brand-subtle"
			>
				<!-- Data Input -->
				<div>
					<label class="block text-xs font-semibold tracking-wider mb-1.5 text-brand-slate"
						>Barcode Data</label
					>
					<input
						...
						class="w-full border border-brand-ink px-3 py-2.5 font-mono font-bold text-sm focus:outline-none focus:border-brand-danger transition-all"
					/>
					{#if barcodeError}
						<p class="text-xs font-bold text-brand-pink mt-1">{barcodeError}</p>
					{/if}
				</div>
						...
						{#each formats as fmt}
							<button
								on:click={() => {
									...
									barcodeValue = fmt.placeholder;
								}}
								class="w-full text-left px-3 py-2 border border-brand-ink text-sm font-bold transition-all {barcodeFormat ===
								fmt.id
							...
							>
								...
								<span
									class="block text-[10px] font-medium {barcodeFormat === fmt.id
										? 'text-brand-mute'
										: 'text-brand-mute'}">{fmt.desc}</span
								>
							</button>
						{/each}
					</div>
				...
				<!-- Display Options -->
				<div class="space-y-3">
					<label class="block text-xs font-semibold tracking-wider text-brand-slate">Display</label>

					<label class="flex items-center gap-2 cursor-pointer">
						<input
							type="checkbox"
							bind:checked={showText}
							class="w-4 h-4 border-2 border-black accent-black"
						/>
					...
					<div class="grid grid-cols-2 gap-3">
						<div>
							<span class="block text-[10px] font-semibold tracking-wider text-brand-mute mb-1"
								>Bar Width (1-5)</span
							>
							<input
								type="number"
								...
								max="5"
								on:change={() => {
									barWidth = Math.min(5, Math.max(1, barWidth || 1));
								}}
								class="w-full border border-brand-ink px-2 py-1.5 font-mono text-sm text-center focus:outline-none"
							/>
						</div>
						<div>
							<span class="block text-[10px] font-semibold tracking-wider text-brand-mute mb-1"
							...
							<input
								type="number"
								bind:value={barHeight}
								min="30"
								max="300"
								on:change={() => {
									barHeight = Math.min(300, Math.max(30, barHeight || 30));
								}}
								class="w-full border border-brand-ink px-2 py-1.5 font-mono text-sm text-center focus:outline-none"
							/>
						</div>
					...
					<div class="grid grid-cols-2 gap-3">
						<div>
							<span class="block text-[10px] font-semibold tracking-wider text-brand-mute mb-1"
								>Bar Color</span
							>
							<div class="flex items-center gap-2 border border-brand-ink px-2 py-1">
								<input
									type="color"
									bind:value={barColor}
									class="w-6 h-6 border-none cursor-pointer"
								/>
						...
						<div>
							...
							{#if transparentBg}
								<button
									...
									class="w-full border border-brand-ink px-2 py-1 text-xs font-bold bg-brand-subtle text-brand-mute flex items-center gap-2"
								>
									<span
										class="w-6 h-6 rounded"
										style="background: repeating-conic-gradient(#ccc 0% 25%, #fff 0% 50%) 50% / 12px 12px"
									/>
									Transparent
								</button>
							{:else}
							...
							<label class="flex items-center gap-2 cursor-pointer mt-1.5">
								<input
									...
									class="w-3.5 h-3.5 border-2 border-black accent-black"
								/>
								<span class="text-[10px] font-bold text-brand-mute">Transparent</span>
							</label>
						</div>
					</div>
				</div>
					...
					<div class="flex gap-0">
						{#each ['png', 'jpg', 'webp'] as fmt}
							<button
								...
								class="flex-1 px-3 py-2 border border-brand-ink font-semibold text-xs transition-all -ml-[2px] first:ml-0 {fileFormat ===
								fmt
			...
			>
				{#if barcodeError}
					<div class="text-center">
						...
						<p class="font-bold text-brand-slate text-sm">{barcodeError}</p>
					</div>
				{:else}
					<div bind:this={svgContainer} class="flex items-center justify-center w-full" />
				{/if}
			</div>
		</div>
		...
		<!-- Generate Bar -->
		<div
			class="border-t border-brand-ink p-4 flex flex-wrap items-center justify-center gap-3 bg-brand-subtle"
		>
			<button
				on:click={generateImage}
				disabled={!!barcodeError || isGenerating || !barcodeValue}
				class="px-8 py-3 bg-brand-proof text-brand-ink border border-brand-ink font-semibold tracking-wide transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none disabled:transform-none flex items-center gap-2"
			>
				{#if isGenerating}
			...
			<button
				...
				class="px-6 py-3 bg-brand-paper text-brand-ink border border-brand-ink font-semibold tracking-wide transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none disabled:transform-none flex items-center gap-2"
			>
	...
	{#if imageUrl}
		<div class="mt-12">
			...
			>
				...
				<div class="inline-block bg-brand-paper border border-brand-ink p-4 rotate-1 mb-8">
					<img
						loading="lazy"
						src={imageUrl}
						alt="Generated barcode"
						class="max-w-full h-auto max-h-[200px]"
					/>
				</div>
				...
				<div class="flex flex-wrap justify-center gap-4">
					<button
						on:click={() =>
							downloadFile(imageUrl, `barcode.${fileFormat}`, { tool_name: 'barcode_generator' })}
	...
	<section class="mt-20 mb-12">
		<div class="text-center mb-10">
			<h2 class="text-3xl md:text-4xl font-semibold text-brand-ink tracking-[-0.02em]">
				Barcode Types <span class="text-brand-pink">Explained</span>
			</h2>
			<p class="text-lg font-bold text-brand-slate mt-3 max-w-2xl mx-auto">
				Choose the right barcode format for your use case. Each type has different data capacity,
				character support, and industry applications.
			</p>
		</div>

		<div class="overflow-x-auto border border-brand-ink">
			<table class="w-full text-left border-collapse bg-brand-paper">
				<thead>
					<tr class="bg-brand-ink text-white">
						<th class="p-3 font-semibold text-xs tracking-wider">Format</th>
						<th class="p-3 font-semibold text-xs tracking-wider">Data Type</th>
						<th class="p-3 font-semibold text-xs tracking-wider">Length</th>
						<th class="p-3 font-semibold text-xs tracking-wider">Common Use</th>
					</tr>
				</thead>
				<tbody class="text-sm font-medium text-brand-slate">
					<tr class="border-b border-gray-200 hover:bg-brand-field/30">
						<td class="p-3 font-semibold">Code 128</td>
						<td class="p-3">Alphanumeric + symbols</td>
						<td class="p-3">Variable</td>
						<td class="p-3">Shipping labels, logistics, general purpose</td>
					</tr>
					<tr class="border-b border-gray-200 hover:bg-brand-field/30">
						<td class="p-3 font-semibold">EAN-13</td>
						<td class="p-3">Numeric only</td>
						<td class="p-3">13 digits</td>
						<td class="p-3">Retail products worldwide (international standard)</td>
					</tr>
					<tr class="border-b border-gray-200 hover:bg-brand-field/30">
						<td class="p-3 font-semibold">UPC-A</td>
						<td class="p-3">Numeric only</td>
						<td class="p-3">12 digits</td>
						<td class="p-3">Retail products in North America</td>
					</tr>
					<tr class="border-b border-gray-200 hover:bg-brand-field/30">
						<td class="p-3 font-semibold">Code 39</td>
						<td class="p-3">Alphanumeric</td>
						<td class="p-3">Variable</td>
						<td class="p-3">Manufacturing, military, healthcare IDs</td>
					</tr>
					<tr class="border-b border-gray-200 hover:bg-brand-field/30">
						<td class="p-3 font-semibold">ITF-14</td>
						<td class="p-3">Numeric only</td>
						<td class="p-3">14 digits</td>
						<td class="p-3">Shipping containers, carton-level tracking</td>
					</tr>
					<tr class="border-b border-gray-200 hover:bg-brand-field/30">
						<td class="p-3 font-semibold">Code 93</td>
						<td class="p-3">Alphanumeric</td>
						<td class="p-3">Variable</td>
						<td class="p-3">Postal services, electronic components</td>
					</tr>
					<tr class="border-b border-gray-200 hover:bg-brand-field/30">
						<td class="p-3 font-semibold">Codabar</td>
					...
					<tr class="hover:bg-brand-field/30">
						<td class="p-3 font-semibold">Pharmacode</td>
	...
	<section class="mb-12">
		<div class="border border-brand-ink bg-brand-paper p-6 md:p-10">
			<h2 class="text-2xl md:text-3xl font-semibold text-brand-ink tracking-[-0.02em] mb-6">
				How to Create a <span class="text-brand-pink">Barcode</span>
			</h2>

			<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
				<div class="border border-brand-ink p-5 bg-brand-field/30">
					<div
						class="w-10 h-10 bg-brand-ink text-white font-semibold text-lg flex items-center justify-center mb-3"
					>
						1
					</div>
					<h3 class="font-semibold text-lg mb-2">Enter Your Data</h3>
					<p class="text-sm font-medium text-brand-slate">
						Type the text or numbers you want to encode. Choose a barcode format that matches your
						use case: Code 128 for general purpose, EAN-13 for retail products, UPC-A for US retail.
					</p>
				</div>
				<div class="border border-brand-ink p-5 bg-brand-proof/10">
					<div
						class="w-10 h-10 bg-brand-ink text-white font-semibold text-lg flex items-center justify-center mb-3"
					>
						2
					</div>
					<h3 class="font-semibold text-lg mb-2">Customize Appearance</h3>
					<p class="text-sm font-medium text-brand-slate">
						Adjust bar width, height, colors, and choose whether to display the encoded text below
						the bars. Enable transparent background for overlay use.
					</p>
				</div>
				<div class="border border-brand-ink p-5 bg-brand-pink/10">
					<div
						class="w-10 h-10 bg-brand-ink text-white font-semibold text-lg flex items-center justify-center mb-3"
					>
						3
					</div>
					<h3 class="font-semibold text-lg mb-2">Download or Automate</h3>
					<p class="text-sm font-medium text-brand-slate">
						Download as PNG, JPG, WebP, or SVG. For bulk generation, use the Pictify API to create
						thousands of unique barcodes programmatically from your inventory or product data.
					</p>
				</div>
			</div>
		</div>
	...
	<section class="mb-12">
		<div class="border border-brand-ink bg-[#0b0b1f] text-white p-6 md:p-10">
			<div class="flex items-start gap-4 mb-4">
				<span
					class="px-3 py-1 bg-brand-proof text-brand-ink text-xs font-semibold tracking-wider border-[2px] border-brand-proof rounded"
					>API</span
				>
			</div>
			<h2 class="text-2xl md:text-3xl font-semibold tracking-[-0.02em] mb-3">
				Bulk Barcode Generation
			</h2>
			<p class="text-brand-rule font-medium mb-6 max-w-2xl">
				Need thousands of barcodes? Use the Pictify batch API to generate unique barcode images for
				every product, bin, or shipment in your system. Feed a CSV of SKUs and get back CDN-hosted
				barcode images in seconds.
			</p>
			<div class="flex flex-wrap gap-4">
				...
				<a
					...
					class="px-6 py-3 bg-transparent text-white font-semibold border border-brand-rule rounded-xl tracking-wide hover:bg-brand-paper/10 transition-colors"
				>
					Batch API Docs
				</a>
			</div>
