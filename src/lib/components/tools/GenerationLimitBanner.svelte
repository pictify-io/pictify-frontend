...
{#if showBanner}
  <div
    class="w-full max-w-5xl mx-auto mb-4"
  >
    <div
      class={`
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-1">
          {#each Array(GUEST_DAILY_LIMIT) as _, i}
            <div
              class={`w-3 h-3 border-[2px] border-black ${i < (GUEST_DAILY_LIMIT - remaining) ? 'bg-gray-400' : 'bg-[#4ade80]'}`}
            ></div>
          {/each}
        </div>
        ...
        <div class="text-sm font-bold">
          ...
          {:else if isLow}
            ...
            <span class="text-gray-700"> free {remaining === 1 ? 'generation' : 'generations'} left today</span>
          {:else}
            <span class="text-gray-700">{remaining} of {GUEST_DAILY_LIMIT} free generations remaining</span>
          {/if}
        </div>
      </div>
