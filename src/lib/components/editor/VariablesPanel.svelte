<script>
	...
	function saveVariable() {
		...
		if (
			variableActions.has(sanitizedName) &&
			(!editingVariable || editingVariable.name !== sanitizedName)
		) {
		...
		if (
			[
				...
				VARIABLE_TYPES.CHART,
				VARIABLE_TYPES.TABLE
			].includes(formType)
		) {
			try {
				parsedDefault = formDefaultValue
					? JSON.parse(formDefaultValue)
					: getDefaultForType(formType);
			} catch (e) {
		...
		if (editingVariable) {
			...
			toast.set({
				message: `Variable "${sanitizedName}" updated`,
				type: 'success',
				duration: 2000
			});
		} else {
			// Create new custom variable
			variableActions.createCustom(sanitizedName, formType, parsedDefault, formDescription);
			variableActions.update(sanitizedName, { required: formRequired });
			toast.set({
				message: `Variable "${sanitizedName}" created`,
				type: 'success',
				duration: 2000
			});
		}
	...
	function deleteSelectedVariables() {
		...
		toast.set({
			message: `${names.length} variable${names.length > 1 ? 's' : ''} deleted`,
			type: 'success',
			duration: 2000
		});
		selectedVariableNames = new Set();
	...
	function getDefaultForType(type) {
		switch (type) {
			...
			case VARIABLE_TYPES.CHART:
				return [{ label: 'Jan', value: 30 }];
			case VARIABLE_TYPES.TABLE:
				return { headers: ['Col 1'], rows: [['Val 1']] };
			default:
				return '';
		}
	}
	...
	function getTypeColor(type) {
		switch (type) {
			case 'text':
				return 'bg-blue-100 text-blue-700 border-[2px] border-blue-200';
			case 'image':
				return 'bg-purple-100 text-purple-700 border-[2px] border-purple-200';
			case 'color':
				return 'bg-orange-100 text-orange-700 border-[2px] border-orange-200';
			case 'chart':
				return 'bg-pink-100 text-pink-700 border-[2px] border-pink-200';
			case 'table':
				return 'bg-teal-100 text-teal-700 border-[2px] border-teal-200';
			case 'boolean':
				return 'bg-green-100 text-green-700 border-[2px] border-green-200';
			case 'number':
				return 'bg-indigo-100 text-indigo-700 border-[2px] border-indigo-200';
			case 'array':
				return 'bg-amber-100 text-amber-700 border-[2px] border-amber-200';
			case 'object':
				return 'bg-violet-100 text-violet-700 border-[2px] border-violet-200';
			default:
				return 'bg-gray-100 text-gray-700 border-[2px] border-gray-900';
		}
	}
	...
	function getSourceBadge(source) {
		switch (source) {
			case 'property':
				return {
					label: 'Element',
					color: 'bg-emerald-100 text-emerald-700 border-[2px] border-emerald-200'
				};
			case 'condition':
				return {
					label: 'Condition',
					color: 'bg-yellow-100 text-yellow-700 border-[2px] border-yellow-200'
				};
			case 'loop':
				return { label: 'Loop', color: 'bg-cyan-100 text-cyan-700 border-[2px] border-cyan-200' };
			case 'custom':
				return { label: 'Custom', color: 'bg-gray-100 text-gray-600 border-[2px] border-gray-900' };
			default:
...
<div class="w-full bg-brand-bg h-full flex flex-col z-10">
	...
	<div class="flex-shrink-0 mx-4 mt-3">
		<PanelTabs
			tabs={[
				{ id: 'list', label: `All (${$variables.length})` },
				{ id: 'preview', label: 'Preview', badge: $isPreviewActive },
				{ id: 'test', label: 'API' }
			]}
			{activeTab}
		...
		{#if activeTab === 'create'}
			<div class="space-y-4">
				<div class="flex items-center justify-between">
					...
					<button class="text-gray-400 hover:text-gray-900 transition-colors" on:click={cancelEdit}>
						<i class="fa fa-times" />
					</button>
				</div>
				...
				{#if formError}
					<div
						class="bg-red-50 border-[2px] border-red-200 rounded-lg p-3 shadow-[2px_2px_0_0_#ef4444]"
						role="alert"
					>
				...
				<div>
					...
					<select id="variable-type" class={selectClass + ' font-medium'} bind:value={formType}>
		...
		{:else if activeTab === 'list'}
			{#if $variables.length === 0}
				<div class="text-center py-8">
					...
					<h4 class="text-sm font-black text-gray-900 mb-2 uppercase tracking-wide">
						No Variables Yet
					</h4>
			...
			{:else}
				...
				<div class="flex items-center justify-between mb-3">
					<button
						class="text-[10px] font-bold uppercase tracking-wide transition-colors {isSelectMode
							? 'text-blue-600 hover:text-blue-800'
							: 'text-gray-400 hover:text-gray-600'}"
						on:click={toggleSelectMode}
						aria-label={isSelectMode ? 'Exit select mode' : 'Enter select mode'}
					>
						<i class="fa fa-{isSelectMode ? 'times' : 'check-square'} mr-1" />
					...
					{#if isSelectMode}
						<div class="flex items-center gap-2">
							<button
								...
								on:click={selectedCount === $variables.length
							...
							{#if selectedCount > 0}
								...
								>
									<i class="fa fa-trash mr-1" />Delete ({selectedCount})
								</button>
							{/if}
						</div>
					{/if}
						...
						<div class="mb-4">
							...
							<div class="space-y-3">
								{#each vars as variable}
									<div
										class="bg-white border-[2px] rounded-lg p-3 transition-all group {selectedVariableNames.has(
											variable.name
										)
										...
										aria-checked={isSelectMode
											? selectedVariableNames.has(variable.name)
											: undefined}
									>
										<div class="flex items-start justify-between mb-2">
											<div class="flex items-center gap-2 flex-wrap">
												{#if isSelectMode}
													<div
														class="w-5 h-5 rounded border-[2px] flex items-center justify-center flex-shrink-0 transition-colors cursor-pointer {selectedVariableNames.has(
															variable.name
														)
															? 'bg-blue-600 border-blue-600'
															: 'border-gray-900 hover:shadow-[1px_1px_0_0_#ffc480] bg-white'}"
														on:click|stopPropagation={() => toggleVariableSelection(variable.name)}
													>
														{#if selectedVariableNames.has(variable.name)}
															<i class="fa fa-check text-white text-[9px]" />
														{/if}
													</div>
												{/if}
												<span
													class={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${getTypeColor(
														variable.type
													)}`}
												>
													<i class={`fa ${getTypeIcon(variable.type)} mr-1`} />
													{variable.type}
												</span>
												{#if variable.source !== 'property'}
													<span
														class={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${
															getSourceBadge(variable.source).color
														}`}
													>
												...
												{#if variable.required}
													<span
														class="px-1.5 py-0.5 bg-red-100 text-red-700 border-[2px] border-red-200 text-[10px] rounded font-bold uppercase tracking-wide"
													>
														Required
													</span>
												{/if}
											...
											{#if !isSelectMode}
												<div class="flex items-center gap-1">
													{#if variable.objectId}
														...
														>
															<i class="fa fa-crosshairs text-xs" />
														</button>
													{/if}
													<button
														class="p-1.5 text-gray-400 hover:text-white hover:bg-gray-900 rounded-lg border-[2px] border-transparent hover:border-gray-900 transition-colors"
										...
										>
											...
											{#if variable.description}
												<p class="text-xs text-gray-500 line-clamp-2 font-medium">
				...
				<div class="pt-4 border-t border-gray-200">
					...
					>
						...
						<i
							class="fa fa-chevron-{showApiExample
								? 'up'
					...
					{#if showApiExample}
						<div class="space-y-3" transition:slide={{ duration: 150 }}>
							...
							>
								...
								>
									<i class="fa fa-copy text-xs" />
								</button>
								<pre class="text-xs text-green-400 overflow-x-auto font-mono"><code
										>{generateApiExample()}</code
									></pre>
							</div>
							...
							<div class="bg-blue-50 border-[2px] border-blue-200 rounded-lg p-3">
								<p class="text-xs text-blue-900 mb-2 break-all font-bold font-mono">
									<span class="bg-blue-200 px-1 rounded">POST</span> /templates/{$template?.uid ||
										'{uid}'}/render
								</p>
			...
			<div class="space-y-4" id="panel-preview" role="tabpanel" aria-labelledby="tab-preview">
						...
						<div class="flex items-center gap-2">
							{#if previewAutoUpdateFlash && $isPreviewActive}
								<span class="text-[10px] font-bold text-green-600 animate-pulse" aria-live="polite">
									<i class="fa fa-sync text-[8px] mr-0.5" />Updated
								</span>
							{/if}
							<button
								class="text-[10px] font-bold transition-colors {resetDisabled
					...
					{#if $variables.length === 0}
						...
						>
							...
							<p class="text-[10px] font-medium text-gray-500 mt-1">
								Add variables to preview your template
							</p>
						</div>
					{:else}
						{#each $variables as variable}
							<div>
								<label
									for={'preview-' + variable.name}
									class="flex items-center gap-2 text-xs font-bold text-gray-700 mb-1.5"
								>
									<span
										class={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase ${getTypeColor(
											variable.type
										)}`}
									>
										<i class={`fa ${getTypeIcon(variable.type)} mr-0.5`} />
										{variable.type}
									</span>
								...
								{:else if variable.type === 'boolean'}
									...
									>
										...
										<span class="text-xs font-bold text-gray-600 uppercase">
											{testValues[variable.name] === true || testValues[variable.name] === 'true'
												? 'true'
												: 'false'}
										</span>
									</label>
								...
								{:else}
									...
									<textarea
										id={'preview-' + variable.name}
										class={'w-full h-24 text-[10px] font-mono font-medium ' +
											inputBaseClass +
											(jsonParseErrors[variable.name]
												? ' border-red-400 focus:border-red-500 focus:shadow-[2px_2px_0_0_#ef4444]'
												: '')}
										placeholder={JSON.stringify(variable.defaultValue || {}, null, 2)}
										value={typeof testValues[variable.name] === 'string'
									...
									{#if jsonParseErrors[variable.name]}
										<p class="text-[10px] font-medium text-red-500 mt-1" role="alert">
											<i class="fa fa-exclamation-circle mr-0.5" />{jsonParseErrors[variable.name]}
										</p>
									{/if}
								{/if}
				...
				<div class="pt-3 border-t border-gray-200">
					<div class="flex items-center justify-between mb-3">
						...
						{#if $isPreviewActive}
							<span
								class="flex items-center gap-1.5 text-[10px] font-bold text-green-700 bg-green-100 px-2 py-0.5 rounded-full border-[2px] border-green-300"
								aria-live="polite"
							>
					...
					<p class="text-[10px] text-gray-600 mb-3 font-medium">
						See how your template behaves with test data. Changes auto-update while active.
					</p>
					...
					>
						{#if $isPreviewActive}
							<i class="fa fa-stop mr-1.5" />Stop Preview
						{:else}
							<i class="fa fa-play mr-1.5" />Start Preview
						{/if}
					</button>
				</div>
					...
					<div class="grid grid-cols-2 gap-2" aria-live="polite" aria-label="Preview statistics">
						...
						>
							...
							<div class="text-lg font-black text-gray-900 leading-tight">
								{previewStats.hiddenByCondition}<span
									class="text-[10px] font-medium text-gray-400 ml-1"
									>/ {previewStats.conditionsCount}</span
								>
							</div>
						...
						>
							...
							<div class="text-lg font-black text-gray-900 leading-tight">
								{previewStats.loopIterations}<span
									class="text-[10px] font-medium text-gray-400 ml-1">items</span
								>
							</div>
							<div class="text-[9px] font-medium text-gray-400">
								from {previewStats.loopsCount} loop{previewStats.loopsCount !== 1 ? 's' : ''}
							</div>
						</div>
					</div>
				{/if}
				...
				{#if previewValidation.errors.length > 0}
					...
					>
						<div class="flex items-center gap-2 mb-2">
							<i class="fa fa-exclamation-circle text-red-500" />
				...
				{#if previewValidation.warnings.length > 0}
					<div
						class="bg-yellow-50 border-[2px] border-yellow-200 rounded-lg p-3 shadow-[2px_2px_0_0_#fbbf24]"
						role="alert"
						aria-label="Validation warnings"
					>
						<div class="flex items-center gap-2 mb-2">
							<i class="fa fa-exclamation-triangle text-yellow-600" />
				...
				<div class="pt-3 border-t border-gray-200">
					...
					>
						<span class="flex items-center gap-1.5">
							<i class="fa fa-lightbulb text-amber-500 text-[10px]" />
							<span class={sectionHeaderClass}>How It Works</span>
						</span>
						<i
							class="fa fa-chevron-{showQuickTips
								? 'up'
								: 'down'} text-[10px] text-gray-300 group-hover:text-gray-500 transition-colors"
						/>
					</button>
					{#if showQuickTips}
						<div class="mt-2 space-y-1.5" transition:slide={{ duration: 150 }}>
							<p class="text-[10px] font-medium text-gray-600 flex items-start gap-1.5">
								<i class="fa fa-font text-blue-400 text-[9px] mt-0.5 flex-shrink-0" />
								Variables replace
								<code class="bg-gray-200 px-1 rounded text-gray-900 font-mono">{'{{ name }}'}</code>
								in text elements
							</p>
							<p class="text-[10px] font-medium text-gray-600 flex items-start gap-1.5">
								<i class="fa fa-eye-slash text-orange-400 text-[9px] mt-0.5 flex-shrink-0" />
								Conditions show/hide elements based on variable values
							</p>
							<p class="text-[10px] font-medium text-gray-600 flex items-start gap-1.5">
								<i class="fa fa-layer-group text-purple-400 text-[9px] mt-0.5 flex-shrink-0" />
		...
		{:else if activeTab === 'test'}
			<div class="space-y-4">
				{#if guestMode && !$user?.email}
					<div class="bg-[#fff7ed] border-[2px] border-amber-300 rounded-lg p-3">
						<div class="flex items-start gap-2">
							<i class="fa fa-lock text-amber-600 mt-0.5" />
							<div>
								<p class="text-xs font-black text-gray-900 uppercase mb-1">Guest mode</p>
								<p class="text-[10px] font-medium text-gray-700">
									You can design and preview variables here. Create a free account to save the
									template and test the Render API.
								</p>
								<button
				...
				{#if !$template?.uid}
					...
					>
						<div class="flex items-start gap-2">
							<i class="fa fa-exclamation-triangle text-yellow-700 mt-0.5" />
							<div>
						...
						>
							{#each apiTokens as token}
								<option value={token.token}
									>{token.token.slice(0, 12)}...{token.token.slice(-4)}</option
								>
							{/each}
						</select>
					{:else}
						<div
							class="bg-gray-50 border-[2px] border-gray-900 rounded-lg px-3 py-2 text-center border-dashed shadow-[2px_2px_0_0_#e5e5e5]"
						>
				...
				{#if $variables.length > 0}
					<div class="space-y-3">
						...
						{#each $variables as variable}
							<div>
								<label
									for={'test-val-' + variable.name}
									class="block text-[10px] font-bold text-gray-700 mb-1 uppercase"
								>
									{variable.name}
									{#if variable.required}
									...
									{/if}
									<span
										class={`ml-1 ${getTypeColor(variable.type)} px-1 py-0.5 rounded text-[9px]`}
									>
										{variable.type}
									</span>
								</label>
								...
								{:else if variable.type === 'number'}
									<input
										...
										value={testValues[variable.name] ?? variable.defaultValue ?? ''}
										on:input={(e) =>
											updateTestValue(variable.name, parseFloat(e.target.value) || 0)}
									/>
								{:else}
									<!-- Array, Object, Chart, Table -->
									<textarea
				...
				{#if guestMode && !$user?.email}
					<div class="bg-[#fff7ed] border-[2px] border-amber-300 rounded-lg p-3">
						<div class="flex items-center gap-2">
							<i class="fa fa-info-circle text-amber-500" />
				...
				{#if renderedImageUrl}
					<div class="space-y-3">
						...
						<div class="relative group">
							{#if !imgLoaded}
								<div
									class="w-full aspect-video rounded-lg border-[2px] border-gray-900 bg-gray-50 flex items-center justify-center shadow-[2px_2px_0_0_#e5e5e5]"
								>
									<div class="flex items-center gap-2">
										<div
							...
							{#if imgLoaded}
								...
								>
									...
									>
										<i class="fa fa-download text-gray-900" />
									</button>
								</div>
							{/if}
						</div>
				...
				{#if renderedPdfUrl}
					<div class="space-y-3">
						...
						<div class="bg-green-50 border-[2px] border-green-300 rounded-lg p-3">
							<div class="flex items-center gap-2 mb-2">
								<i class="fa fa-file-pdf text-red-500 text-lg" />
				...
				<div class="pt-3 border-t border-gray-200">
					...
					>
						...
						<i
							class="fa fa-chevron-{showRequestDetails
								? 'up'
								: 'down'} text-[10px] text-gray-300 group-hover:text-gray-500 transition-colors"
						/>
					</button>
					...
					{#if showRequestDetails}
						<div class="space-y-2" transition:slide={{ duration: 150 }}>
							...
							>
								...
								>
									<i class="fa fa-copy text-[10px]" />
								</button>
								<pre
									class="text-[10px] text-green-400 overflow-x-auto whitespace-pre-wrap font-mono"><code
	...
	{#if showDeleteConfirm}
		...
		>
			<div
				class="bg-brand-bg rounded-xl border-[3px] border-gray-900 shadow-[8px_8px_0_0_#1f2937] max-w-sm w-full relative overflow-hidden"
			>
				<!-- Red header strip -->
				<div
				...
				<div
					class="absolute inset-0 opacity-[0.03] pointer-events-none bg-[radial-gradient(#000_1px,transparent_1px)] [background-size:8px_8px]"
				/>
					...
					<div class="text-center">
						...
						{#if selectedCount > 0 && !variableToDelete}
							...
							<h4
								...
								class="text-lg font-black text-gray-900 mb-2 uppercase tracking-wider"
							>
							...
							<div class="flex flex-wrap gap-1.5 justify-center mb-5">
								{#each [...selectedVariableNames] as name}
									<span
										class="font-mono font-bold text-gray-900 bg-brand-accent px-2 py-0.5 rounded border-[2px] border-gray-900 text-xs shadow-[2px_2px_0_0_#1f2937]"
										>{name}</span
									>
								{/each}
							</div>
							<div class="flex gap-3">
								...
								<button
									class="flex-1 py-2.5 px-4 bg-brand-danger text-white border-[3px] border-gray-900 rounded-lg text-sm font-black uppercase tracking-wider shadow-[4px_4px_0_0_#1f2937] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
						...
						{:else}
							...
							<p class="text-sm font-medium text-gray-600 mb-5">
								...
								<span
									class="font-mono font-bold text-gray-900 bg-brand-accent px-1.5 py-0.5 rounded border-[2px] border-gray-900 text-xs shadow-[2px_2px_0_0_#1f2937]"
									>"{variableToDelete?.name}"</span
								>&nbsp;&nbsp;?
								{#if variableToDelete?.source === 'property'}
									<br /><span class="text-xs text-gray-500 mt-1 inline-block"
										>This will also remove the variable from the associated element.</span
									>
								{/if}
							</p>
							<div class="flex gap-3">
