<script>
	...
	const TABLE_STYLES = {
		modern: { name: 'Modern', headerBg: '#1a1a2e', headerText: '#ffffff', rowBg: '#ffffff', altRowBg: '#f8fafc', borderColor: '#e2e8f0' },
		minimal: { name: 'Minimal', headerBg: 'transparent', headerText: '#1a1a1a', rowBg: 'transparent', altRowBg: 'transparent', borderColor: '#e5e5e5' },
		colorful: { name: 'Colorful', headerBg: '#ff6b6b', headerText: '#ffffff', rowBg: '#ffffff', altRowBg: '#fff5f5', borderColor: '#ffccd5' },
		corporate: { name: 'Corporate', headerBg: '#0f172a', headerText: '#ffffff', rowBg: '#ffffff', altRowBg: '#f1f5f9', borderColor: '#cbd5e1' },
		gradient: { name: 'Gradient', headerBg: '#6366f1', headerText: '#ffffff', rowBg: '#ffffff', altRowBg: '#eef2ff', borderColor: '#c7d2fe' }
	};
	...
	$: filteredFonts = getFilteredFonts(allFonts, extendedSearchResults, selectedCategory, fontSearchQuery);
	...
	async function handleAddFont(font) {
		...
		try {
			...
			if (result.success) {
				...
				link.href = `https://fonts.googleapis.com/css2?family=${font.family.replace(/ /g, '+')}:wght@${result.font.weights.join(';')}&display=swap`;
				link.rel = 'stylesheet';
	...
	async function addCustomFont() {
		...
		try {
			...
			if (result.success) {
				...
				// Load the font
				const link = document.createElement('link');
				link.href = fontUrl || `https://fonts.googleapis.com/css2?family=${familyName.replace(/ /g, '+')}:wght@${result.font.weights.join(';')}&display=swap`;
				link.rel = 'stylesheet';
				document.head.appendChild(link);
	...
	function updateLocalState() {
		...
		if (obj.isRoundedRect) {
			...
			if (cornerRadii.tl === cornerRadii.tr && cornerRadii.tr === cornerRadii.br && cornerRadii.br === cornerRadii.bl) {
	...
	function parseCSVToChartData(csvText) {
		...
		const labelIndex = headers.findIndex(h => h.toLowerCase() === 'label' || h.toLowerCase() === 'name');
		const valueIndex = headers.findIndex(h => h.toLowerCase() === 'value' || h.toLowerCase() === 'amount');
	...
	function loadSampleChartData() {
		if (chartDataFormat === 'csv') {
			...
			chartDataInput = JSON.stringify(chartData.length > 0 ? chartData : [
				{ label: 'Jan', value: 30 },
				{ label: 'Feb', value: 45 },
				{ label: 'Mar', value: 60 },
				{ label: 'Apr', value: 35 },
	...
	async function updateProperty(prop, value) {
		...
		if (prop === 'width' || prop === 'height') {
			...
			cornerRadii = { tl: parseInt(value), tr: parseInt(value), br: parseInt(value), bl: parseInt(value) };
							...
							<div class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-[400px] flex flex-col">
								...
								<div class="p-3 border-b border-gray-100 sticky top-0 bg-white z-10">
									{#if showCustomFontInput}
										<div>
											<p class="text-xs text-gray-500 mb-2">Paste a Google Fonts URL or enter a font name:</p>
											<input
												type="text"
									...
									{:else}
										<div class="flex gap-2 mb-2">
											<div class="relative flex-1">
												...
												{#if isSearchingGoogle}
													<i class="fa fa-spinner fa-spin absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
												{/if}
											</div>
											<button
										...
										<div class="flex flex-wrap gap-1">
											<button
												class="px-2 py-0.5 text-xs rounded-full transition-colors {selectedCategory === 'all' ? 'bg-black text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}"
											...
											{#each fontCategories as category}
												<button
													class="px-2 py-0.5 text-xs rounded-full transition-colors {selectedCategory === category ? 'bg-black text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}"
								...
								<div class="overflow-y-auto flex-1">
									{#if fontsLoading}
										<div class="p-4 text-center text-gray-500 text-sm">
											<i class="fa fa-spinner fa-spin mr-2"></i> Loading fonts...
										</div>
									...
									{:else if filteredFonts.length === 0 && isSearchingGoogle}
										<div class="p-4 text-center text-gray-500 text-sm">
											<i class="fa fa-spinner fa-spin mr-2"></i> Searching Google Fonts...
										</div>
									{:else}
										...
										{#if extendedSearchResults.length > 0 && fontSearchQuery.length >= 3}
											...
											{#if localFonts.length > 0}
												<div class="px-4 py-1.5 bg-gray-50 text-[10px] font-medium text-gray-500 uppercase tracking-wider sticky top-0">
												...
												{#each localFonts as font}
													<button
														...
														on:click={() => {
															const link = document.createElement('link');
															link.href = `https://fonts.googleapis.com/css2?family=${font.family.replace(/ /g, '+')}:wght@${font.weights?.join(';') || '400;700'}&display=swap`;
															link.rel = 'stylesheet';
															document.head.appendChild(link);
													...
													>
														<div class="flex items-center gap-2 min-w-0 flex-1">
															<span style="font-family: '{font.family}', sans-serif" class="truncate">{font.family}</span>
															{#if font.popular}
																<span class="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full flex-shrink-0">Popular</span>
															{/if}
														</div>
											...
											{#if googleFonts.length > 0}
												<div class="px-4 py-1.5 bg-blue-50 text-[10px] font-medium text-blue-600 uppercase tracking-wider sticky top-0 flex items-center gap-1">
												...
												{#each googleFonts as font}
													...
													>
														<div class="flex items-center gap-2 min-w-0 flex-1">
															...
															<span class="text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded-full flex-shrink-0">
																{font.category}
															</span>
														</div>
														<span class="text-[10px] text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
								...
								<div class="px-3 py-2 border-t border-gray-100 bg-gray-50 text-[10px] text-gray-500 flex items-center justify-between">
									<span>
										...
										{#if extendedSearchResults.length > 0}
											<span class="text-blue-500">+ {extendedSearchResults.length} from Google</span>
										{/if}
									</span>
									{#if isSearchingGoogle}
					...
					<div class="relative">
						...
						>
							<span style="font-weight: {styles.fontWeight === 'normal' ? 400 : (styles.fontWeight === 'bold' ? 700 : styles.fontWeight)}">
								{fontWeights.find(w => w.value == styles.fontWeight)?.label || styles.fontWeight}
							</span>
							<i class="fa fa-chevron-down text-xs text-gray-400"></i>
						</button>
						...
						{#if isWeightDropdownOpen}
							<div class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
								{#each fontWeights as weight}
									...
									>
										<span style="font-weight: {weight.value === 'normal' ? 400 : (weight.value === 'bold' ? 700 : weight.value)}">{weight.label}</span>
						...
						<div class="mt-3 pt-3 border-t border-gray-100 space-y-3">
							...
							<div>
								<div class="flex items-center justify-between mb-1.5">
									...
									<span class="text-xs text-gray-500">{clipSettings.offsetX > 0 ? '+' : ''}{clipSettings.offsetX}%</span>
								</div>
								<input
									type="range"
									min="-50"
							...
							<div>
								<div class="flex items-center justify-between mb-1.5">
									...
									<span class="text-xs text-gray-500">{clipSettings.offsetY > 0 ? '+' : ''}{clipSettings.offsetY}%</span>
								</div>
								<input
									type="range"
									min="-50"
				...
				<div class="space-y-3 pt-4 border-t border-gray-100">
						...
						{#if showTableDataEditor}
							<div class="space-y-3">
								...
								<div class="bg-gray-50 border border-gray-200 rounded-lg p-2 text-[10px] text-gray-600">
									...
									{#if tableDataFormat === 'csv'}
										{#if tableType === 'stats'}
											<code class="block text-gray-500">Metric,Value,Change<br/>Revenue,$45k,+12%<br/>Users,2345,+8%</code>
										{:else if tableType === 'comparison'}
											<code class="block text-gray-500">Feature,Basic,Pro<br/>Price,$9,$29<br/>Storage,10GB,100GB</code>
										{:else}
											<code class="block text-gray-500">Product,Price,Stock<br/>Widget A,$29,150</code>
										{/if}
									{:else}
					...
					{:else if tableType === 'comparison' && tableData.features}
						<div class="text-xs text-gray-600 bg-gray-50 rounded-lg p-2">
							...
							<p><strong>Plans:</strong> {tableData.plans?.length || 0} ({tableData.plans?.map(p => p.name).join(', ')})</p>
						</div>
					{:else if tableData.headers}
						<div class="text-xs text-gray-600 bg-gray-50 rounded-lg p-2">
							<p><strong>Columns:</strong> {tableData.headers.length} ({tableData.headers.join(', ')})</p>
							{#if tableData.rows}
