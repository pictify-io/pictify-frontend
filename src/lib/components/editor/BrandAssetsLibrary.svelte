	...
	const ASSET_TYPES = [
		{
			key: null,
			label: 'ALL',
			icon: `<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M4 6h16M4 12h16M4 18h16"/></svg>`
		},
		{
			key: 'logo',
			label: 'LOGOS',
			icon: `<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>`
		},
		{
			key: 'color',
			label: 'COLORS',
		...
		},
		{
			key: 'font',
			label: 'TYPE',
			icon: `<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M4 6h16M4 12h16m-7 6h7"/></svg>`
		},
		{
			key: 'image',
			label: 'IMGS',
			icon: `<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>`
		},
		{
			key: 'icon',
			label: 'ICONS',
			icon: `<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M14 10l-2 1m0 0l-2-1m2 1v2.5M20 7l-2 1m2-1l-2-1m2 1v2.5M14 4l-2-1-2 1M4 7l2-1M4 7l2 1M4 7v2.5M12 21l-2-1m2 1l2-1m-2 1v-2.5M6 18l-2-1v-2.5M18 18l2-1v-2.5"/></svg>`
		}
	];
...
<div class="h-full flex flex-col bg-brand-bg">
	<!-- Search & Filters -->
	<div
		class="bg-white border-b-[3px] border-gray-900 p-4 space-y-4 shrink-0 shadow-sm relative z-20"
	>
		<!-- Search Input -->
		<div class="relative">
			...
			<input
				...
				class="w-full pl-9 pr-3 py-2.5 bg-gray-50 border-[2px] border-gray-900 text-xs font-bold uppercase tracking-wide focus:outline-none focus:bg-white focus:shadow-brutal-md transition-all placeholder-gray-400 rounded-none"
			/>
		</div>
			...
			{#each ASSET_TYPES as type}
				<button
					...
					{selectedType === type.key
						? 'bg-brand-accent text-gray-900 shadow-brutal-sm -translate-y-0.5'
	...
	<!-- Content Area -->
	<div
		class="flex-1 overflow-y-auto p-4 bg-brand-bg relative scrollbar-thin scrollbar-thumb-gray-900 scrollbar-track-transparent"
	>
		<!-- Background Grid -->
		<div
			class="absolute inset-0 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:20px_20px] opacity-50 pointer-events-none"
		/>
		...
		<div class="relative z-10">
			{#if loading}
				<div class="flex flex-col items-center justify-center py-12">
					...
					<p class="text-gray-400 text-xs font-bold uppercase tracking-widest mt-4 animate-pulse">
						Syncing Database...
					</p>
				</div>
			{:else if assets.length === 0}
				<div class="flex flex-col items-center justify-center h-full text-center py-10 px-4">
					<div
						class="w-16 h-16 bg-gray-100 border-[3px] border-gray-900 flex items-center justify-center mb-4 shadow-brutal-lg rotate-3"
					>
					...
					<p class="text-gray-900 font-black text-sm uppercase tracking-wide mb-1">
						Inventory Empty
					</p>
					<p class="text-gray-500 text-xs mb-6 max-w-[200px]">
						Upload assets to populate your design library.
					</p>
					<a
						href="/dashboard/brand-assets"
						target="_blank"
						class="inline-flex items-center gap-2 px-4 py-2 bg-gray-900 text-white border-[2px] border-gray-900 text-xs font-bold uppercase hover:bg-brand-accent hover:text-gray-900 transition-all hover:shadow-brutal-md hover:-translate-y-0.5"
					>
			...
			{:else}
				...
				{#if selectedType === null}
					...
					{#if groupedAssets.logo.length > 0}
						<div class="mb-8">
							<h3
								class="text-[10px] font-black text-gray-900 uppercase tracking-widest mb-3 flex items-center gap-2"
							>
							...
							<div class="grid grid-cols-2 gap-3">
								{#each groupedAssets.logo as asset}
									<button
										...
										class="group relative aspect-square bg-white border-[2px] border-gray-900 p-3 hover:shadow-brutal-lg hover:-translate-y-0.5 transition-all flex items-center justify-center overflow-hidden"
										title={asset.name}
									>
										<div
								...
								{#each groupedAssets.color as asset}
									<div class="group relative">
										<button
											...
											class="w-full aspect-square border-[2px] border-gray-900 shadow-brutal-sm hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
							...
							<div class="space-y-2">
								{#each groupedAssets.font as asset}
									<button
										...
										class="w-full p-3 bg-white border-[2px] border-gray-900 text-left hover:shadow-brutal-lg hover:-translate-y-0.5 transition-all group relative overflow-hidden"
									>
										<span
										...
										<span
											class="text-[9px] font-mono font-bold text-gray-400 uppercase group-hover:text-brand-danger transition-colors relative z-10"
										>
						...
						<div class="mb-8">
							...
							<div class="grid grid-cols-2 gap-3">
								{#each [...groupedAssets.image, ...groupedAssets.icon] as asset}
									<button
										on:click={() => addImageToCanvas(asset)}
										class="group bg-white border-[2px] border-gray-900 p-2 hover:shadow-brutal-lg hover:-translate-y-0.5 transition-all"
										title={asset.name}
									>
										<div
											class="aspect-video bg-gray-50 border border-gray-100 overflow-hidden relative flex items-center justify-center"
										>
											<div
										...
										<div
											class="mt-2 text-[9px] font-bold text-gray-500 truncate text-center group-hover:text-gray-900 uppercase tracking-wide"
										>
											{asset.name}
										</div>
									</button>
				...
				{:else}
					...
					<div
						class="grid {selectedType === 'color' || selectedType === 'icon'
							? 'grid-cols-3'
							: 'grid-cols-2'} gap-3"
					>
						{#each filteredAssets as asset}
							{#if selectedType === 'font'}
								<button
									on:click={() => applyFontToSelected(asset.metadata?.fontFamily || asset.name)}
									class="col-span-2 w-full p-3 bg-white border-[2px] border-gray-900 text-left hover:shadow-brutal-lg hover:-translate-y-0.5 transition-all group"
								>
									<span
									...
									<span
										class="text-[9px] font-mono text-gray-400 mt-1 block uppercase group-hover:text-brand-danger"
							...
							{:else if selectedType === 'color'}
								<button
									on:click={() => applyColorToSelected(asset.value)}
									class="w-full aspect-square border-[2px] border-gray-900 shadow-brutal-sm hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px] transition-all relative"
									style="background-color: {asset.value}"
									title="Click to apply"
								>
									<div
										class="absolute bottom-0 left-0 right-0 bg-white/90 text-[8px] font-mono font-bold py-0.5 text-center border-t border-gray-900 opacity-0 hover:opacity-100"
									>
										{asset.value}
									</div>
								</button>
							{:else}
								<button
									on:click={() => addImageToCanvas(asset)}
									class="bg-white border-[2px] border-gray-900 p-2 hover:shadow-brutal-lg hover:-translate-y-0.5 transition-all"
								>
									<div
										class="aspect-square flex items-center justify-center bg-gray-50 border border-gray-100 relative overflow-hidden"
									>
										<div
											class="absolute inset-0 opacity-10 bg-[radial-gradient(#000_1px,transparent_1px)] [background-size:4px_4px]"
										/>
