<script>
	...
	$: leftPos = (() => {
		...
		if (typeof window !== 'undefined') {
			if (targetX + widthToCheck > window.innerWidth - 20) {
				// 20px padding
...
{#if element}
	<div class="absolute z-50 pointer-events-none" {style}>
		<!-- Inner container with pointer-events-auto to capture clicks -->
		<div class="pointer-events-auto">
			{#if !isOpen}
				<button
					class="group flex items-center justify-center w-10 h-10 bg-brand-accent rounded-full shadow-brutal-lg border-[3px] border-gray-900 hover:scale-110 transition-all duration-300 hover:shadow-xl"
					on:click={toggleOpen}
				...
				>
					...
					>
						...
						/>
						<path
							d="M17 4C17.5523 4 18 3.55228 18 3C18 2.44772 17.5523 2 17 2C16.4477 2 16 2.44772 16 3C16 3.55228 16.4477 4 17 4Z"
							fill="white"
						/>
						<path
						...
						<path
							fill-rule="evenodd"
							clip-rule="evenodd"
							d="M15.5858 5.58579C16.3668 4.80474 17.6332 4.80474 18.4142 5.58579C19.1953 6.36683 19.1953 7.63316 18.4142 8.41421L8.41421 18.4142C7.63316 19.1953 6.36683 19.1953 5.58579 18.4142C4.80474 17.6332 4.80474 16.3668 5.58579 15.5858L15.5858 5.58579ZM17 7L7 17L8.41421 18.4142L18.4142 8.41421L17 7Z"
							fill="white"
						/>
					</svg>
				</button>
			{:else}
				<div
					class="w-[320px] bg-brand-bg rounded-xl shadow-brutal-2xl border-[3px] border-gray-900 overflow-hidden"
				...
				>
					...
					<div class="p-4">
						<textarea
							...
							class="w-full text-sm border-2 border-gray-900 rounded-lg p-3 focus:shadow-brutal-accent focus:border-gray-900 focus:ring-0 min-h-[100px] resize-none bg-white mb-3 placeholder-gray-500 transition-all shadow-brutal-lg font-medium"
							disabled={isLoading}
							autofocus
						/>
						...
						{#if error}
							<div
								class="flex items-center gap-2 text-xs text-red-500 mb-3 px-1 bg-red-50 p-2 rounded-lg border border-red-100"
								transition:fly={{ y: -5 }}
							>
								<svg
									width="12"
						...
						<div class="flex justify-between items-center">
							<span class="text-[11px] text-gray-400 font-medium px-1 flex items-center gap-1">
								{#if isLoading}
									...
									>
								{:else}
									<span class="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200 text-[10px]"
										>Enter</span
									> to generate
								{/if}
							</span>
