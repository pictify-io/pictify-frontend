...
<div class="w-full bg-[#FFFDF8] h-full flex flex-col transition-all duration-300 z-0 overflow-hidden">
	...
	<div class="px-4 py-4 flex-1 overflow-y-auto custom-scrollbar min-h-0 bg-[#FFFDF8]">
		...
		{:else if displayTab === 'text'}
			<!-- Mode Tabs -->
			<div class="flex gap-2 mb-4 p-1 bg-gray-900 rounded-lg border-[2px] border-gray-900 shadow-[4px_4px_0_0_#1f2937]">
				<button
					class="flex-1 px-3 py-2 text-[10px] font-black uppercase tracking-widest rounded-md transition-all {textMode === 'basic' ? 'bg-[#ffc480] text-gray-900 border-[2px] border-gray-900' : 'text-white hover:text-[#ffc480]'}"
					on:click={() => textMode = 'basic'}
				>
					Basic
				</button>
				<button
					class="flex-1 px-3 py-2 text-[10px] font-black uppercase tracking-widest rounded-md transition-all {textMode === 'presets' ? 'bg-[#ffc480] text-gray-900 border-[2px] border-gray-900' : 'text-white hover:text-[#ffc480]'}"
			...
			{#if textMode === 'basic'}
				<div class="space-y-3">
					<button class="w-full p-4 bg-white border-[2px] border-gray-900 rounded-xl text-left hover:shadow-[4px_4px_0_0_#1f2937] hover:-translate-y-1 transition-all group shadow-sm" on:click={() => addText('h1', 'Heading', '32px')}>
						<h1 class="text-2xl font-black text-gray-900 group-hover:text-black">Add a heading</h1>
					</button>
					<button class="w-full p-4 bg-white border-[2px] border-gray-900 rounded-xl text-left hover:shadow-[4px_4px_0_0_#1f2937] hover:-translate-y-1 transition-all group shadow-sm" on:click={() => addText('h2', 'Subheading', '24px')}>
						<h2 class="text-xl font-bold text-gray-800 group-hover:text-black">Add a subheading</h2>
					</button>
					<button class="w-full p-4 bg-white border-[2px] border-gray-900 rounded-xl text-left hover:shadow-[4px_4px_0_0_#1f2937] hover:-translate-y-1 transition-all group shadow-sm" on:click={() => addText('p', 'Body text', '16px')}>
						<p class="text-base font-medium text-gray-700 group-hover:text-black">Add a little bit of body text</p>
					</button>
				</div>
			{:else}
				<TextPresetLibrary />
		...
		{:else if displayTab === 'uploads'}
			<div class="space-y-4">
				<label class="block cursor-pointer">
					...
					>
						<div class="w-12 h-12 rounded-lg bg-gray-900 flex items-center justify-center group-hover:scale-110 transition-transform shadow-[4px_4px_0_0_#ffc480]">
							<i class="fa fa-cloud-upload text-xl text-white"></i>
						</div>
						<div class="text-center">
							<span class="text-xs font-black text-gray-900 uppercase tracking-widest">Upload Image</span>
							<p class="text-[10px] font-bold text-gray-500 mt-1 uppercase">PNG, JPG, SVG up to 10MB</p>
						</div>
					</div>
				</label>
		...
		{:else if displayTab === 'layers'}
			<div class="text-center text-gray-400 mt-10">
				<div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4 border-[3px] border-gray-900">
					<i class="fa fa-layer-group text-2xl text-gray-900"></i>
				</div>
				<p class="text-xs font-bold text-gray-900 uppercase tracking-widest">Layers coming soon</p>
			</div>
		{/if}
	</div>
	{/if}
