<script>
	...
	import { UNSPLASH_CONFIG, isUnsplashConfigured, getUnsplashAttributionUrl } from '../../config/unsplash.js';
	...
	const suggestions = [
		'Abstract', 'Business', 'Nature', 'Technology', 'People',
		'Office', 'Workspace', 'City', 'Food', 'Travel',
		'Fashion', 'Minimal', 'Colorful', 'Dark', 'Light'
	];
	...
	async function searchPhotos(query, pageNum = 1) {
		...
		try {
			const response = await fetch(
				`${UNSPLASH_CONFIG.API_BASE_URL}/search/photos?query=${encodeURIComponent(query)}&page=${pageNum}&per_page=${UNSPLASH_CONFIG.DEFAULT_PER_PAGE}&client_id=${UNSPLASH_CONFIG.ACCESS_KEY}`
			);
...
<div class="stock-photos-panel">
	...
	<div class="search-section">
		<div class="search-box">
			...
			/>
			{#if searchQuery}
				<button class="clear-btn" on:click={() => { searchQuery = ''; photos = featuredPhotos; }}>
	...
	{#if !isConfigured}
		<div class="info-message">
			...
			<div>
				...
				<p>Add your Unsplash API key for unlimited access. <a href="https://unsplash.com/developers" target="_blank">Get free API key →</a></p>
			</div>
		</div>
	{/if}
	...
	<div class="footer-credit">
		...
		<a href={getUnsplashAttributionUrl()} target="_blank">
			Unsplash
		</a>
	</div>
</div>
