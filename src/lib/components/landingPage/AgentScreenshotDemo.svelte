...
<section class="w-full bg-white border-t-[3px] border-b-[3px] border-gray-900 py-16 md:py-24">
	<div class="max-w-5xl mx-auto px-4">
		...
		<div class="text-center mb-12">
			<div class="inline-flex items-center gap-2 px-4 py-2 bg-[#ff6b6b] text-white rounded-full border-2 border-gray-900 text-sm font-bold mb-4">
			...
			<p class="text-lg text-gray-600 max-w-2xl mx-auto">
				Describe what you want in plain English. Our AI navigates any website and captures the perfect screenshot.
			</p>
		</div>
		...
		<div class="grid lg:grid-cols-2 gap-6 mb-12">
			<!-- Input Panel -->
			<div class="bg-[#FFFDF8] rounded-2xl border-[3px] border-gray-900 shadow-[5px_5px_0_0_#1f293780] overflow-hidden flex flex-col">
				<!-- Header -->
				...
				<div class="p-5 flex flex-col flex-1">
					...
					<div class="mb-4">
						...
						<div class="px-4 py-3 bg-white rounded-xl border-[3px] border-gray-900 min-h-[60px] flex items-center">
					...
					<div class="flex-1 flex items-center">
							{#if isProcessing}
							<div class="flex items-center gap-3 p-3 bg-[#ffc480]/30 rounded-xl border-2 border-gray-900 w-full">
									<div class="relative">
								...
								<span class="text-sm font-medium text-gray-900">{processingSteps[currentStep].text}</span>
								</div>
							{:else}
					...
					<div class="pt-4 mt-4 border-t-2 border-gray-200">
						...
						<div class="flex flex-wrap gap-2">
							{#each examples as example, index}
								<button
									class="px-3 py-1.5 text-xs font-bold rounded-lg border-2 border-gray-900 transition-all {
				...
				<div class="p-5 flex-1 flex flex-col items-center justify-center min-h-[300px]">
						...
						<div class="w-full space-y-4" in:fly={{ y: 20, duration: 400 }}>
							...
							<div class="bg-white rounded-xl border-[3px] border-gray-900 p-6 text-center">
								...
								<div class="text-sm font-bold text-gray-900 mb-2">{examples[selectedExample].description}</div>
								<div class="flex justify-center gap-2">
												{#each examples[selectedExample].tags as tag}
										<span class="px-2 py-1 bg-[#ffc480] text-gray-900 text-xs font-bold rounded border-2 border-gray-900">{tag}</span>
												{/each}
								</div>
							</div>
					...
					{:else if isProcessing}
						<div class="text-center space-y-4" in:fade={{ duration: 200 }}>
							...
							<div class="w-full max-w-[200px] bg-gray-200 rounded-full h-2">
								<div class="bg-[#ff6b6b] h-2 rounded-full transition-all duration-500" style="width: {((currentStep + 1) / processingSteps.length) * 100}%"></div>
							</div>
						</div>
					{:else}
