...
<div class="space-y-6">
		...
		{#each variables as variable (variable.name)}
			<div class="space-y-2">
				...
				{#if variable.description}
					<p class="text-[10px] font-bold text-gray-500 uppercase tracking-wide">{variable.description}</p>
				{/if}
			</div>
		{/each}
	{/if}
	...
	<div class="pt-6 border-t-[3px] border-gray-100 border-dashed">
		...
		>
			<div class="w-5 h-5 border-[2px] border-gray-400 group-hover:border-gray-900 rounded flex items-center justify-center transition-colors">
				<svg class="w-3 h-3 transition-transform {showJsonInput ? 'rotate-90' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		...
		{#if showJsonInput}
			<div class="mt-4 space-y-4 animate-fadeIn">
				...
				/>
				{#if jsonError}
					<div class="px-3 py-2 bg-red-100 border-[2px] border-red-200 rounded text-xs font-bold text-red-600 flex items-center gap-2">
