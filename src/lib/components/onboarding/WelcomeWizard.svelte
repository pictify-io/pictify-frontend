<script>
	import { savePersonalizationAction, skipPersonalizationAction } from '../../../store/onboarding.store';
	...
	const useCases = [
		{ id: 'social-media', label: 'Social Media Graphics', desc: 'Instagram posts, Twitter headers, LinkedIn banners, YouTube thumbnails', icon: 'share' },
		{ id: 'email-marketing', label: 'Email Marketing', desc: 'Email headers, newsletter visuals, campaign banners', icon: 'mail' },
		{ id: 'e-commerce', label: 'E-Commerce / Product', desc: 'Product banners, pricing cards, discount coupons', icon: 'cart' },
		{ id: 'dashboard-reporting', label: 'Dashboard & Reporting', desc: 'KPI cards, charts, leaderboards, automated data visuals', icon: 'chart' },
		{ id: 'certificates', label: 'Certificates & Awards', desc: 'Course certificates, achievement badges, diplomas', icon: 'award' },
		{ id: 'personalized-images', label: 'Personalized Images at Scale', desc: 'Dynamic images with user-specific data (names, stats)', icon: 'users' },
		{ id: 'content-marketing', label: 'Blog & Content Marketing', desc: 'Blog featured images, article thumbnails, content cards', icon: 'edit' },
	];

	const integrationModes = [
		{ id: 'editor', label: 'Visual Editor', desc: 'Design images with the drag-and-drop canvas', icon: 'layout' },
		{ id: 'api', label: 'API Integration', desc: 'Generate images programmatically via REST API', icon: 'code' },
		{ id: 'both', label: 'Both', desc: 'Design in the editor, generate at scale via API', icon: 'layers' },
	];
...
<div class="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
	<div class="w-full max-w-2xl bg-[#FFFDF8] border-[3px] border-black rounded-3xl shadow-[8px_8px_0_0_black] overflow-hidden animate-wizard-in">
		<!-- Header -->
		<div class="px-8 pt-8 pb-4">
			<div class="inline-flex items-center gap-2 px-3 py-1 bg-[#ffc480] border-[2px] border-black rounded-full shadow-[3px_3px_0_0_black] mb-4">
				<span class="text-[10px] font-black text-black uppercase tracking-widest">Step {step} of 2</span>
			</div>
					...
					{#each useCases as uc}
						...
						>
							<div class="flex items-start gap-3">
								<div class="w-8 h-8 rounded-lg border-[2px] flex items-center justify-center flex-shrink-0 mt-0.5
									...
									{:else if uc.icon === 'cart'}
										<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z"/></svg>
					...
					{#each integrationModes as mode}
						...
						>
							<div class="flex items-center gap-4">
								<div class="w-10 h-10 rounded-xl border-[2px] flex items-center justify-center flex-shrink-0
									{selectedMode === mode.id ? 'border-black bg-[#ffc480]' : 'border-gray-300 bg-gray-50'}">
									{#if mode.icon === 'layout'}
										<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6z"/></svg>
		...
		<!-- Footer -->
		<div class="px-8 py-5 border-t-[3px] border-black bg-gray-50/50 flex items-center justify-between">
			<button
				on:click={skip}
				disabled={saving}
