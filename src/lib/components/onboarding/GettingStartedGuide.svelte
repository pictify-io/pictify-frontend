<script>
	...
	$: baseSteps = (intent && GETTING_STARTED_STEPS[intent]) || DEFAULT_GETTING_STARTED_STEPS;
	$: steps = reorderForVariant(baseSteps, variant, intent);

	// value-first arm: for intents where "generate an image" is the real aha (api-integration,
	// no-code), stable-sort the first-image step to the top and renumber. Other intents unchanged.
	function reorderForVariant(list, v, intentVal) {
		if (v !== 'value-first' || !['api-integration', 'no-code'].includes(intentVal)) return list;
		const sorted = [...list].sort((a, b) => {
			const rank = (s) => (s.completedCheck === 'hasImages' ? 0 : 1);
			return rank(a) - rank(b);
		});
		return sorted.map((s, i) => ({ ...s, number: i + 1 }));
	}

	// The checklist had NO conversion signal before — this fires for control + value-first.
	function trackChecklistCta(step) {
		analytics.track('checklist_step_cta_clicked', {
			step_id: step.id || step.number,
			step_number: step.number,
			step_title: step.title,
			intent,
										...
										{#if (activeTabs[step.number] || tabs[0].id) === tab.id}
											<div class="mt-2 border-[2px] border-black rounded-lg overflow-hidden">
												{#each tab.steps as tabStep, i}
													<div
														class="{i % 2 === 0 ? 'bg-white' : 'bg-[#fafafa]'} {i <
														tab.steps.length - 1
