<script>
	...
	function getPlatformInfo(platform) {
		return (
			PLATFORMS.find((p) => p.value === platform) || PLATFORMS.find((p) => p.value === 'custom')
		);
	}
	...
	const MOCK_SUBSCRIPTIONS = [
		{
			...
			platform: 'zapier',
			event: 'render.completed',
			targetUrl: 'https://hooks.zapier.com/hooks/catch/123456/abcde',
			status: 'active',
			deliveryCount: 1243,
			failureCount: 0,
		...
		},
		{
			...
			platform: 'make',
			event: 'render.failed',
			targetUrl: 'https://hook.us1.make.com/abcdef123456',
			status: 'paused',
			deliveryCount: 89,
			failureCount: 2,
...
<div class="space-y-6">
	...
	<div class="flex items-center justify-end mb-6">
		<button
			on:click={() => {
				if (hasWebhookAccess) showCreateModal = true;
			}}
					...
					>
						<div class="flex items-start justify-between">
							<div class="flex-1">
									...
									>
										<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path
												...
												stroke-width="2"
												d={PLATFORM_ICONS[sub.platform]}
											/>
										</svg>
									</div>
									<div class="flex flex-col">
										<span class="font-black text-gray-900 uppercase tracking-tight text-sm"
											>{getEventInfo(sub.event).label}</span
										>
								...
								<div class="pl-[52px]">
									<p
										class="text-xs font-mono text-gray-600 mb-2 truncate max-w-md bg-gray-50 p-1.5 rounded border border-gray-200"
									>
										{sub.targetUrl}
									</p>
									<div
										class="flex items-center gap-4 text-[10px] font-bold text-gray-500 uppercase tracking-wide"
									>
									...
									>
										<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path
												...
												stroke-linejoin="round"
												stroke-width="2"
												d={PLATFORM_ICONS[sub.platform] || PLATFORM_ICONS.custom}
											/>
										</svg>
									</div>
									<div class="flex flex-col">
			...
			<div class="p-6 relative z-10">
				...
				{#if createdSecret}
					...
					<div class="space-y-6">
						...
						<div>
							<label class="block text-xs font-black text-gray-900 mb-2 uppercase tracking-widest"
								>Webhook Secret</label
							>
							<div class="flex items-center gap-2">
								<code
									class="flex-1 p-3 bg-white border-2 border-gray-900 rounded-lg text-sm font-mono break-all text-gray-800 shadow-inner"
								>
									{createdSecret}
								</code>
								<button
				...
				{:else}
					...
					<form on:submit|preventDefault={handleCreate} class="space-y-6">
						{#if createError}
							<div
								class="p-3 bg-red-50 border-2 border-red-200 rounded-xl text-red-600 text-xs font-bold"
							>
								{createError}
							</div>
						{/if}
