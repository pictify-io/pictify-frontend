...
<section>
			...
			>
				...
				>
					...
					<div class="flex items-center gap-2">
						...
						{:else}
							<span
								class="px-1.5 py-0.5 text-[8px] sm:text-[9px] font-bold font-mono bg-[#4facfe] text-white uppercase tracking-wider rounded border border-gray-900"
							>
								IMAGE
							</span>
						{/if}
						<span
							class="text-[9px] sm:text-[10px] font-bold font-mono text-gray-500 uppercase tracking-wider"
						>
				...
				<!-- Preview Container -->
				<div
					class="relative aspect-[4/3] bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] overflow-hidden group-hover:bg-gray-50 transition-colors"
				>
					<div
				...
				>
					<div>
						<h3
							class="font-black text-gray-900 text-base sm:text-lg leading-tight mb-2 sm:mb-3 line-clamp-1"
							title={template.name}
						>
							{template.name}
						</h3>
						...
						<div class="flex flex-wrap gap-1.5 sm:gap-2">
							...
							<!-- Date Tag -->
							<div
								class="inline-flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-gray-100 border border-gray-300 rounded text-[9px] sm:text-[10px] font-bold uppercase text-gray-600 tracking-wide"
							>
								<svg
									class="w-2.5 h-2.5 sm:w-3 sm:h-3"
									fill="none"
	...
	{#if pagination.totalPages > 1}
		<div
			class="flex flex-col sm:flex-row items-center justify-between mt-8 sm:mt-12 pt-6 sm:pt-8 border-t-[2px] sm:border-t-[3px] border-gray-900 gap-4 sm:gap-6"
		>
			<!-- Results count -->
			<div
			...
			<div class="flex items-center gap-1.5 sm:gap-2">
				...
				<!-- Page numbers -->
				<div
					class="flex items-center gap-1 sm:gap-2 mx-1 sm:mx-2 bg-gray-100 p-0.5 sm:p-1 rounded-lg border border-gray-200"
				>
					{#each getPageNumbers() as pageNum}
						{#if pageNum === '...'}
							<span
								class="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center text-gray-400 font-black text-[10px] sm:text-xs"
								>...</span
							>
						{:else}
							<button
