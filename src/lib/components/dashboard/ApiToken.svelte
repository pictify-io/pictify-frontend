	...
	<div class="max-w-6xl p-4 sm:p-5 m-auto">
		<div class="">
			...
			<div class="flex flex-col sm:flex-row justify-between sm:items-center mt-10 mb-10">
				<div class="text-sm sm:text-md text-gray-700">
					API tokens used to authenticate with Pictify API. <br />Treat them like passwords and keep
					them secret. Learn more in our
					<a href="https://docs.pictify.ai" target="_blank" class="text-blue-500">docs</a>.
				</div>
				<div>
					<button
						class="mt-10 sm:mt-0 text-sm text-gray-900 hover:text-gray-900 font-bold py-1 px-4 rounded border-2 border-black"
						on:click={() => {
							createAPITokenAction();
				...
				<div class="flex flex-col mt-5 mb-5">
					<div class="flex items-baseline">
						<div class="flex flex-col">
							<div
								class="text-md text-red-400 whitespace-nowrap overflow-hidden overflow-ellipsis max-w-xs sm:max-w-full"
							>
								{token.token}
							</div>
							...
							<div>
								<button
									class="text-xs text-gray-500 py-1 px-4 rounded border-2 border-gray-500 mt-2"
									on:click={() => {
										deleteAPITokenAction(token.uid);
									}}>Disable</button
								>
							</div>
						</div>
