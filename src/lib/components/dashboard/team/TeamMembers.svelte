...
<div class="min-h-full">
	...
	<div class="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6 mb-8 sm:mb-12">
		<div>
			<a
				href="/dashboard/team"
				class="inline-flex items-center gap-2 px-2 sm:px-3 py-1 bg-gray-900 text-white text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded mb-2 sm:mb-3 hover:bg-gray-800 transition-colors"
			>
	...
	{:else}
		...
		>
			<div class="p-8">
				...
				<div
					class="w-full h-6 bg-gray-100 rounded-full overflow-hidden border-[3px] border-gray-900"
				>
					<div
		...
		<div class="grid lg:grid-cols-3 gap-8">
			...
			<div class="lg:col-span-2 space-y-8">
										...
										<div>
											...
											<div class="flex items-center gap-2 mt-1">
												...
												{#if member.role?.name === 'Owner'}
													<span
														class="px-2 py-0.5 text-[10px] font-black uppercase tracking-wider bg-gray-900 text-white rounded border border-gray-900"
													>
														Owner
													</span>
												{:else}
													<span
														class="px-2 py-0.5 text-[10px] font-black uppercase tracking-wider bg-gray-100 text-gray-600 rounded border border-gray-300"
													>
				...
				>
					<div class="px-8 py-6 border-b-[3px] border-gray-900 bg-gray-50">
						<h2 class="text-xl font-black text-gray-900 uppercase tracking-wide">
							Pending Invites ({$teamInvitations.length})
						</h2>
					</div>
					<div class="divide-y-[3px] divide-gray-100">
						{#each $teamInvitations as invitation}
							<div class="p-6 hover:bg-[#FFFDF8] transition-colors">
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-4">
										<div
											class="w-10 h-10 rounded-xl bg-yellow-100 border-[3px] border-gray-900 flex items-center justify-center text-yellow-600"
										>
											<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
												<path
													stroke-linecap="round"
			...
			<div class="lg:col-span-1">
				{#if $isTeamOwner}
					<div
						class="bg-gray-900 text-white border-[3px] border-gray-900 rounded-2xl shadow-[8px_8px_0_0_#ff6b6b] overflow-hidden sticky top-8"
					>
						<div class="p-8">
							...
							{#if !canInvite}
								<div
									class="bg-yellow-400 text-gray-900 border-[3px] border-gray-900 rounded-xl p-4 text-center shadow-[4px_4px_0_0_rgba(0,0,0,0.2)]"
								>
									<p class="font-black text-sm uppercase tracking-wider">Seat limit reached</p>
									<a
										href="/dashboard/upgrade"
										class="inline-block mt-2 text-xs font-bold underline hover:no-underline"
										>Upgrade Plan</a
									>
								</div>
							{:else}
