	...
	<div class="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6 mb-8 sm:mb-12">
		...
		<div class="flex items-center gap-4">
			<div class="text-right">
				<div class="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-wider">Members</div>
				<div class="text-lg sm:text-xl font-black text-gray-900 uppercase">{$teamMembers ? $teamMembers.length : 0} Active</div>
			</div>
		</div>
	</div>
	...
	{:else}
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
				...
				<div class="bg-white border-[3px] border-gray-900 rounded-2xl shadow-[8px_8px_0_0_#1f2937] overflow-hidden">
					<div class="bg-gray-100 border-b-[3px] border-gray-900 p-4 flex justify-between items-center">
						<div class="flex items-center gap-3">
							<div class="w-8 h-8 rounded-lg bg-gray-900 flex items-center justify-center flex-shrink-0 shadow-[2px_2px_0_0_#1f2937]">
								<svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
								</svg>
							</div>
							<div>
								<h2 class="text-xs font-black text-gray-900 uppercase tracking-widest">Team Members</h2>
								<p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Manage access & roles</p>
							</div>
						</div>
						<div class="text-[10px] font-black uppercase tracking-wider bg-gray-200 px-2 py-1 rounded text-gray-600">
						...
						{#each $teamMembers as member}
							<div class="p-6 hover:bg-[#FFFDF8] transition-colors group">
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-4">
										<div class="w-10 h-10 rounded-lg bg-[#ffc480] border-[2px] border-gray-900 flex items-center justify-center shadow-[2px_2px_0_0_#1f2937]">
											<span class="text-sm font-black text-gray-900">
												{member.user?.email?.charAt(0)?.toUpperCase() || '?'}
											</span>
										</div>
										<div>
											<p class="font-bold text-gray-900 text-sm">{member.user?.email || 'Unknown'}</p>
											<div class="flex items-center gap-2 mt-0.5">
												<span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">
													Joined {formatDate(member.joinedAt)}
												</span>
												{#if member.role?.name === 'Owner'}
													<span class="px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider bg-gray-900 text-white rounded border border-gray-900">Owner</span>
												{:else}
													<span class="px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider bg-gray-100 text-gray-600 rounded border border-gray-300">Member</span>
												{/if}
											</div>
										</div>
									</div>
				...
				{#if $teamInvitations.length > 0}
					<div class="bg-white border-[3px] border-gray-900 rounded-2xl shadow-[8px_8px_0_0_#1f2937] overflow-hidden">
						<div class="bg-gray-100 border-b-[3px] border-gray-900 p-4 flex items-center gap-3">
							<div class="w-8 h-8 rounded-lg bg-yellow-400 border-2 border-gray-900 flex items-center justify-center flex-shrink-0 shadow-[2px_2px_0_0_#1f2937]">
								<svg class="w-4 h-4 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
								</svg>
							</div>
							<div>
								<h2 class="text-xs font-black text-gray-900 uppercase tracking-widest">Pending Invites</h2>
								<p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Awaiting acceptance</p>
							</div>
						</div>
								...
								<div class="p-6 hover:bg-[#FFFDF8] transition-colors">
									<div class="flex items-center justify-between">
										<div>
											...
											<p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">Expires {formatDate(invitation.expiresAt)}</p>
										</div>
										<div class="flex items-center gap-2">
											...
											{#if $isTeamOwner}
												<button
													...
													disabled={resendingInvitationId === invitation.uid || resendCooldowns[invitation.uid]}
												...
												>
													...
													{:else if resendCooldowns[invitation.uid]}
														Wait {Math.floor(resendCooldowns[invitation.uid] / 60)}:{(resendCooldowns[invitation.uid] % 60).toString().padStart(2, '0')}
													{:else}
														Resend
													{/if}
												</button>
			...
			<div class="lg:col-span-4 space-y-8 sticky top-8">
				...
				<div class="bg-white border-[3px] border-gray-900 rounded-2xl shadow-[8px_8px_0_0_#1f2937] overflow-hidden">
					<div class="bg-gray-100 border-b-[3px] border-gray-900 p-4 flex items-center gap-3">
						<div class="w-8 h-8 rounded-lg bg-blue-500 border-2 border-gray-900 flex items-center justify-center flex-shrink-0 shadow-[2px_2px_0_0_#1f2937]">
							<svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						...
						<div>
							<h2 class="text-xs font-black text-gray-900 uppercase tracking-widest">Team Settings</h2>
							<p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Update Workspace</p>
						</div>
					</div>
					...
					<div class="p-6">
						{#if $canManageTeam}
							<div class="mb-4">
								<label for="teamName" class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Team Name</label>
								<input
									id="teamName"
									type="text"
									bind:value={teamName}
							...
							{#if error}
								<div class="bg-red-50 border-2 border-[#ff6b6b] rounded-xl p-3 mb-4 flex items-start gap-2">
									<svg class="w-4 h-4 text-[#ff6b6b] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						...
						{:else}
							<div class="flex items-center">
								<div class="w-12 h-12 rounded-xl bg-[#ffc480] border-[2px] border-gray-900 flex items-center justify-center shadow-[2px_2px_0_0_#1f2937]">
									<span class="text-xl font-black text-gray-900">
										{$currentTeam?.name?.charAt(0)?.toUpperCase() || 'T'}
									</span>
								</div>
								<div class="ml-4">
									<p class="text-lg font-black text-gray-900 tracking-tight">{$currentTeam?.name}</p>
									<p class="text-[10px] font-mono font-bold text-gray-500 mt-0.5">ID: {$currentTeam?.uid}</p>
								</div>
							</div>
						{/if}
					</div>
				...
				<!-- Invite Form -->
				{#if $isTeamOwner}
					<div class="bg-gray-900 text-white border-[3px] border-gray-900 rounded-2xl shadow-[8px_8px_0_0_#ff6b6b] overflow-hidden">
						<div class="bg-gray-800 border-b-[3px] border-gray-900 p-4 flex items-center gap-3">
							<div class="w-8 h-8 rounded-lg bg-[#4ade80] border-2 border-gray-900 flex items-center justify-center flex-shrink-0 shadow-[2px_2px_0_0_#000]">
								<svg class="w-4 h-4 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
								</svg>
							</div>
							<div>
								<h2 class="text-xs font-black text-white uppercase tracking-widest">Invite Member</h2>
								<p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Grow your team</p>
							</div>
						</div>
						
						<div class="p-6">
							{#if !canInvite}
								<div class="bg-yellow-400 text-gray-900 border-[3px] border-gray-900 rounded-xl p-4 text-center">
									<p class="font-black text-xs uppercase tracking-wider">Seat limit reached</p>
									<a href="/dashboard/upgrade" class="inline-block mt-2 text-[10px] font-bold underline hover:no-underline">Upgrade Plan</a>
								</div>
							{:else}
								<div class="space-y-4">
									<div>
										<label for="inviteEmail" class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Email Address</label>
										<input
											id="inviteEmail"
											type="email"
