...
<div class="bg-[#ffc480] border-t-[3px] border-b-[3px] border-gray-900">
	...
	>
		<div class="space-y-1 text-gray-900">
			<div class="flex items-center gap-2 mb-1">
				<span class="w-2 h-2 rounded-full bg-red-500 animate-pulse border border-gray-900"></span>
				<p class="text-xs uppercase tracking-[0.2em] font-black text-gray-900">
					Action required
				</p>
			</div>
			...
			<p class="text-sm font-medium text-gray-800 leading-snug max-w-xl">
				{#if email}
					We sent a link to <span class="font-bold underline decoration-2">{email}</span>. Click anywhere to resend.
				{:else}
					Confirm your email address so we know it's really you. Click anywhere to resend.
				{/if}
			</p>
		</div>
		<div class="flex items-center gap-2 text-sm font-black text-gray-900 bg-white px-5 py-2.5 rounded-lg border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f2937] group-hover:shadow-[2px_2px_0_0_#1f2937] group-hover:translate-x-[2px] group-hover:translate-y-[2px] transition-all uppercase tracking-wide">
			{#if isLoading()}
				<span class="inline-flex items-center gap-2">
					<span class="w-4 h-4 border-[3px] border-gray-900 border-t-transparent rounded-full animate-spin" />
					Sending…
				</span>
	...
	{#if message}
		<div class="px-5 pb-4">
			<p
				class="text-xs font-bold uppercase tracking-wide px-3 py-1.5 rounded border-[2px] border-gray-900 inline-block {status === 'error' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-800'}"
			>
				{message}
			</p>
		</div>
