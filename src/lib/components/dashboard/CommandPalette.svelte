<script>
	...
	const allItems = [
		// Navigation
		{
			...
			path: '/dashboard',
			keywords: 'home dashboard overview stats analytics'
		},
		{
			type: 'nav',
			label: 'Templates',
		...
		},
		{
			...
			keywords: 'brand assets logos fonts colors'
		},
		{
			type: 'nav',
			label: 'Experiments',
		...
		},
		{
			...
			keywords: 'images media generated renders'
		},
		{
			type: 'nav',
			label: 'GIFs',
			description: 'Generated GIFs library',
			path: '/dashboard/media/gifs',
			keywords: 'gifs animated media generated'
		},
		{
			type: 'nav',
			label: 'PDFs',
			description: 'Generated PDFs library',
			path: '/dashboard/media/pdfs',
			keywords: 'pdfs documents media generated'
		},
		{
			type: 'nav',
			label: 'Activity Logs',
		...
		},
		{
			...
			keywords: 'api playground test endpoints curl'
		},
		{
			type: 'nav',
			label: 'API Keys',
			description: 'Manage API tokens',
			path: '/dashboard/api-token',
			keywords: 'api keys tokens authentication bearer'
		},
		{
			type: 'nav',
			label: 'Integrations',
			description: 'Webhooks, storage connectors, Zapier',
			path: '/dashboard/integrations',
			keywords: 'integrations webhooks storage zapier make n8n pipedream'
		},
		{
			type: 'nav',
			label: 'Webhooks',
		...
		},
		{
			...
			description: 'S3, Google Cloud, Cloudinary, ImageKit',
			path: '/dashboard/integrations/storage',
			keywords: 'storage s3 gcs cloudinary imagekit connectors upload'
		},
		{
			type: 'nav',
			label: 'Team Settings',
			...
			path: '/dashboard/team',
			keywords: 'team settings members invite roles'
		},
		{
			type: 'nav',
			label: 'Billing',
			...
			keywords: 'billing subscription plan invoices usage upgrade pricing'
		},
		...
		},
		{
			...
			path: '/dashboard/billing',
			keywords: 'upgrade plan pricing pro business enterprise'
		}
	];
	...
	function searchItems(q) {
		...
		if (!q.trim()) {
			...
			return [...allItems].sort((a, b) => {
				const aBoost =
					(mode === 'api' && API_PATHS.has(a.path)) ||
					(mode === 'editor' && EDITOR_PATHS.has(a.path))
						? 1
						: 0;
				const bBoost =
					(mode === 'api' && API_PATHS.has(b.path)) ||
					(mode === 'editor' && EDITOR_PATHS.has(b.path))
						? 1
						: 0;
	...
	>
		...
		>
			...
			<div class="flex items-center gap-3 px-5 py-4 border-b-[3px] border-gray-900">
				...
				/>
				<kbd
					class="hidden sm:inline-block px-1.5 py-0.5 bg-gray-100 border border-gray-300 rounded text-[10px] font-mono text-gray-500"
					>ESC</kbd
				>
			</div>
				...
				{:else}
					{#each filteredItems as item, i}
						...
						>
							...
							{:else}
								<div
									class="w-8 h-8 rounded-lg bg-gray-100 border-2 border-gray-200 flex items-center justify-center flex-shrink-0"
								>
									<svg
