...
<div class="bg-white border-[3px] border-gray-900 rounded-xl p-6 space-y-5 shadow-[6px_6px_0_0_#1f2937]">
	<div class="flex items-start gap-4">
		<div class="flex-shrink-0 w-12 h-12 bg-[#ffc480] rounded-xl flex items-center justify-center border-[3px] border-gray-900 shadow-[3px_3px_0_0_#1f2937]">
		...
		<div class="flex-1">
			...
			<p class="text-sm font-medium text-gray-600 leading-relaxed">
				...
				{#if email}
					We sent a link to <span class="font-bold text-gray-900 bg-yellow-100 px-1 rounded">{email}</span>.
				{/if}
			</p>
		</div>
	</div>
	...
	{#if message}
		<p class="text-xs font-bold text-center uppercase tracking-wide {status === 'error' ? 'text-red-600' : 'text-green-600'}">
			{message}
		</p>
	{/if}
</div>
