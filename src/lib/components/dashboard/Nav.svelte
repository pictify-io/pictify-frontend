...
<header
	class="w-full h-16 sm:h-20 z-30 border-b-[3px] border-gray-900 bg-[#FFFDF8] flex justify-between items-stretch sticky top-0"
>
	<!-- Left Section: Sidebar Toggle & Brand -->
	<div class="flex items-stretch">
		<!-- Mobile Sidebar Toggle -->
		<button
			class="md:hidden w-16 sm:w-20 h-full flex items-center justify-center border-r-[3px] border-gray-900 hover:bg-gray-100 transition-colors"
			on:click={toggleSidebar}
			aria-label="Toggle sidebar"
		>
			<svg
				class="w-6 h-6 sm:w-8 sm:h-8 text-gray-900"
				fill="none"
				stroke="currentColor"
				viewBox="0 0 24 24"
		...
		<a
			href="/"
			class="w-48 sm:w-56 md:w-64 flex flex-shrink-0 items-center justify-center px-4 sm:px-6 md:px-8 no-underline cursor-pointer border-r-[3px] border-gray-900 hover:bg-[#ffc480] transition-all group relative overflow-hidden"
		>
	...
	<div class="flex items-stretch border-l-[3px] border-gray-900">
		
		<!-- Usage Widget Container -->
		<div class="flex items-center px-3 sm:px-4 md:px-6 h-full hover:bg-gray-50 transition-colors relative group">
