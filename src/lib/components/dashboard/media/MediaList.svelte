<script>
	...
	import {
		...
		formatRelativeDate,
		getPageNumbers as sharedGetPageNumbers
			...
			>
				{#each mediaList as media, index (media.id || index)}
					...
					>
						<!-- Preview Container -->
						<div
							class="relative bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] border-b-[3px] border-gray-900 overflow-hidden h-[220px] group-hover:bg-gray-50 transition-colors"
						>
							{#if mediaType === 'pdfs'}
									...
									<div class="p-8">
										...
										>
											...
											<path
												...
												stroke-linejoin="round"
												stroke-width="1.5"
												d="M9 17h6M9 13h6M13 3v5a1 1 0 001 1h5"
											/>
										</svg>
									</div>
									<!-- PDF Badge -->
							...
							{#if mediaType === 'pdfs' && media.pageCount}
								<div class="absolute bottom-3 left-3 z-20">
									<span
										class="text-[10px] font-black text-gray-900 bg-white px-2 py-1 rounded border-[2px] border-gray-900 uppercase tracking-wide shadow-sm"
									>
										{media.pageCount}
										{media.pageCount === 1 ? 'PAGE' : 'PAGES'}
									</span>
								</div>
							...
							<!-- Hover overlay -->
							<div
								class="absolute inset-0 bg-gray-900/5 group-hover:bg-gray-900/10 transition-colors duration-300 z-10 pointer-events-none"
							/>

							<!-- Action buttons on hover -->
							<div
								class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-2 group-hover:translate-y-0 z-30 flex gap-2"
							>
								<button
							...
							<div class="flex items-center justify-between">
								<div class="flex items-center gap-2">
									...
									<span class="text-xs text-gray-500 font-bold uppercase tracking-wide"
										>{formatDate(media.createdAt)}</span
									>
								</div>
								<span
			...
			{#if totalPages > 1}
				<div class="flex items-center justify-between mt-12 pt-8 border-t-[3px] border-gray-900">
					...
					<div class="bg-white border-[2px] border-gray-900 px-4 py-2 rounded-lg shadow-sm">
						...
						<span class="text-sm font-black text-gray-900 ml-1"
	...
	>
		...
		<div class="max-w-6xl w-full max-h-[90vh] flex flex-col relative" on:click|stopPropagation>
			...
			>
				...
				<div
					class="relative z-10 bg-white/50 rounded-xl overflow-hidden flex items-center justify-center min-h-[400px]"
				>
					{#if mediaType === 'pdfs'}
						<!-- PDF Viewer styled like an image viewer -->
						<div
							class="w-full max-w-6xl h-[calc(85vh-100px)] bg-white rounded-lg shadow-2xl overflow-hidden border-[3px] border-gray-900"
						>
							<iframe
				...
				<div class="flex items-center gap-3">
					{#if mediaType === 'pdfs' && selectedMedia.pageCount}
						...
						>
							...
							<span class="text-xs font-bold text-white uppercase"
								>{selectedMedia.pageCount} {selectedMedia.pageCount === 1 ? 'Page' : 'Pages'}</span
							>
						</div>
					{:else if selectedMedia.width && selectedMedia.height}
						...
						>
							...
							<span class="text-xs font-bold text-white font-mono"
								>{selectedMedia.width} × {selectedMedia.height}</span
							>
						</div>
					{/if}
					{#if selectedMedia.createdAt}
						...
						>
							...
							>
							<span class="text-xs font-bold text-white uppercase"
								>{formatDate(selectedMedia.createdAt)}</span
							>
						</div>
					{/if}
				</div>
