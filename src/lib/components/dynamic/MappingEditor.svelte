	...
	<div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
								...
								<div class="group">
									<div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-2">
										...
										<label class="w-48 font-bold flex items-center gap-2 text-gray-900 shrink-0">
											<div
												class="w-6 h-6 rounded bg-gray-100 border-2 border-gray-900 flex items-center justify-center text-xs font-black shrink-0 shadow-[2px_2px_0_0_#ccc]"
											>
												{variable.name.charAt(0).toUpperCase()}
											</div>
											<span class="truncate">{variable.name}</span>
											{#if variable.required}<span class="text-red-500 text-lg leading-none">*</span
												>{/if}
										</label>
										...
										<div class="flex-1 flex items-center gap-3 min-w-0">
											<div class="relative flex-1">
												<input
													...
													placeholder={suggestedPaths.length > 0
														? 'Select or type path...'
														: '$.data.field'}
												...
												{#if suggestedPaths.length > 0}
													...
													>
														<button
															...
															on:click={() =>
																(openDropdown =
																	openDropdown === variable.name ? null : variable.name)}
														>
															<svg
																class="w-4 h-4 transition-transform {openDropdown === variable.name
																	? 'rotate-180'
																	: ''}"
																fill="none"
																stroke="currentColor"
												...
												{#if openDropdown === variable.name && suggestedPaths.length > 0}
													...
													>
														...
														>
															<p
																class="text-[10px] font-black text-gray-500 uppercase tracking-wide"
															>
																Available Paths
															</p>
															<span
																class="bg-gray-200 text-gray-600 px-1.5 py-0.5 rounded text-[10px] font-bold"
																>{getFilteredPaths(variable.name).length}</span
															>
														</div>
														{#each getFilteredPaths(variable.name) as { path, value, type, isLeaf }}
															<button
																...
																on:mousedown|preventDefault={() =>
																	handleSelectPath(variable.name, path)}
															>
																<div class="flex-1 min-w-0">
																	...
																	{#if isLeaf}
																		<span
																			class="text-[10px] font-mono {getTypeColor(
																				type
																			)} truncate block mt-0.5 opacity-75"
																		>
														...
														{#if getFilteredPaths(variable.name).length === 0}
															<div
																class="px-3 py-4 text-center text-gray-500 text-xs font-bold uppercase tracking-wide"
															>
																No matching paths
															</div>
														{/if}
									...
									<div class="sm:pl-[210px] flex flex-col sm:flex-row gap-4 mb-2">
										...
										<div class="flex-1 min-w-0 h-8 flex items-center">
											{#if mapping[variable.name] && sampleData}
												...
												{:else}
													<div
														class="inline-block px-3 py-1.5 bg-orange-50 border border-orange-200 rounded text-xs font-bold text-orange-800"
													>
														Path not found
													</div>
												{/if}
		...
		<div class="lg:col-span-4 space-y-6">
			...
			>
				...
				>
					<h3
						class="text-xs font-black text-gray-900 uppercase tracking-widest flex items-center gap-2"
					>
