<script>
	...
	const PROPERTIES = [
		...
		},
		{
			...
			category: 'Device',
			valueType: 'select',
			options: ['iOS', 'Android', 'Windows', 'macOS', 'Linux', 'ChromeOS']
		},
		...
		{ value: 'time.dayOfWeek', label: 'Day of Week', category: 'Time', valueType: 'dayOfWeek' },
		{
			...
			category: 'Browser',
			valueType: 'select',
			options: ['en', 'es', 'fr', 'de', 'pt', 'it', 'nl', 'ja', 'ko', 'zh', 'ar', 'hi', 'ru']
		},
		{
			value: 'url.param',
			label: 'URL Parameter',
	...
	const OPERATORS_BY_TYPE = {
		...
		hour: [{ value: 'in', label: 'includes' }],
		dayOfWeek: [{ value: 'in', label: 'is on' }],
		text: [
			{ value: 'eq', label: 'equals' },
...
>
	...
	{#if rule.property}
		...
		<div
			class="flex items-center gap-2 px-3 py-2 bg-brand-bg border-[3px] border-black shadow-[2px_2px_0_0_black] rounded-xl shrink-0"
		>
			<span
			...
			/>
			<span class="text-xs font-black text-black whitespace-nowrap"
				>{def?.label || rule.property}</span
			>
		</div>
	{:else}
		<select
	...
	{#if rule.property}
		<select
			...
			{disabled}
			class="px-3 py-2 border-[3px] border-black shadow-[2px_2px_0_0_black] rounded-xl text-xs font-bold bg-brand-accent focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 hover:-translate-y-[1px] hover:shadow-[4px_4px_0_0_black] transition-all shrink-0 cursor-pointer text-black"
		>
	...
	{#if rule.property}
		{#if propDef?.valueType === 'select'}
			{#if rule.operator === 'in' || rule.operator === 'not_in'}
				<div
					class="flex-1 min-w-0 flex flex-wrap gap-2 p-3 border-[3px] border-black shadow-[inset_2px_2px_4px_rgba(0,0,0,0.1)] rounded-xl bg-gray-100 min-h-[40px]"
				>
					{#each propDef.options as opt}
						...
						<button
							...
							class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border-[3px] border-black transition-all cursor-pointer
								...
								: 'bg-white text-black shadow-[2px_2px_0_0_black] hover:bg-brand-accent hover:-translate-y-[1px] hover:shadow-[4px_4px_0_0_black]'}"
						>
							{propDef.optionLabels?.[opt] ||
								(propDef.value === 'geo.continent' ? CONTINENT_LABELS[opt] : null) ||
								(propDef.value === 'browser.language' ? LANGUAGE_LABELS[opt] : null) ||
								opt}
						</button>
					{/each}
				</div>
		...
		{:else if propDef?.valueType === 'country'}
			<div
				class="flex-1 min-w-0 flex flex-col gap-3 p-3 border-[3px] border-black shadow-[inset_2px_2px_4px_rgba(0,0,0,0.1)] rounded-xl bg-gray-100"
			>
						...
						<span class="truncate pr-2">
							...
							{:else}
								<span class="text-gray-500"
									>Select countr{rule.operator === 'eq' ? 'y' : 'ies'}...</span
								>
							{/if}
						</span>
				...
				{:else}
					...
					<div class="relative w-full flex gap-2">
						...
						<button
							...
							class="px-3 py-2 bg-brand-accent border-[3px] border-black rounded-xl text-xs font-black shadow-[2px_2px_0_0_black] hover:-translate-y-[1px] hover:shadow-[4px_4px_0_0_black] transition-all cursor-pointer"
						>
							Done
						</button>
					</div>
					...
					<!-- Countries List -->
					<div
						class="flex flex-col gap-1 max-h-[200px] overflow-y-auto pr-2 rounded-xl border-[3px] border-black bg-white p-1 mt-1"
					>
						{#each COUNTRIES.filter((c) => c.name
									.toLowerCase()
									.includes((countrySearchQuery || '').toLowerCase()) || c.code
									.toLowerCase()
									.includes((countrySearchQuery || '').toLowerCase())) as country}
							{@const isMulti = rule.operator === 'in' || rule.operator === 'not_in'}
							{@const selected = isMulti
								? Array.isArray(rule.value) && rule.value.includes(country.code)
							...
							>
								<div class="flex items-center gap-2">
									...
									<span
										>{country.name}
										<span class="text-gray-400 font-mono text-[10px] ml-1">({country.code})</span
										></span
									>
								</div>
								{#if selected}
		...
		{:else if propDef?.valueType === 'hour'}
			<div
				class="flex-1 min-w-0 flex flex-col gap-3 p-3 border-[3px] border-black shadow-[inset_2px_2px_4px_rgba(0,0,0,0.1)] rounded-xl bg-gray-100 min-h-[40px]"
			>
				<!-- Quick ranges -->
				<div class="flex flex-wrap gap-1.5">
					{#each HOUR_QUICK_RANGES as range}
						{@const allSelected =
							Array.isArray(rule.value) && range.hours.every((h) => rule.value.includes(h))}
						<button
							type="button"
							{disabled}
							on:click={() => {
							...
							class="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border-[2px] border-black transition-all cursor-pointer
								...
								: 'bg-white text-gray-500 shadow-[1px_1px_0_0_black] hover:bg-brand-accent hover:text-black'}"
						>
							{range.label}
						</button>
					{/each}
							...
							{#each Array.from({ length: 12 }, (_, i) => i) as hour}
								...
								<button
									...
									class="w-9 h-8 rounded-md text-[11px] font-bold border-[2px] border-black transition-all flex items-center justify-center cursor-pointer
										...
										: 'bg-white text-gray-500 hover:bg-brand-accent hover:text-black hover:-translate-y-[1px] hover:shadow-[2px_2px_0_0_black]'}"
								>
									{displayHour}
								</button>
							{/each}
		...
		{:else if propDef?.valueType === 'dayOfWeek'}
			<div
				class="flex-1 min-w-0 flex flex-col gap-2 p-2 border-[3px] border-black shadow-[inset_2px_2px_4px_rgba(0,0,0,0.1)] rounded-xl bg-gray-100 min-h-[40px]"
			>
				<!-- Quick selects -->
	...
	<button
		...
		on:click={emitRemove}
		{disabled}
		class="p-2 border-[3px] border-transparent rounded-xl text-gray-400 hover:text-black hover:bg-brand-danger hover:border-black hover:shadow-[2px_2px_0_0_black] transition-all shrink-0 self-center"
		title="Remove rule"
	>
