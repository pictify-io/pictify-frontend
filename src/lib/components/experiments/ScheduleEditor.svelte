...
<div class="space-y-6">
			...
			<div class="relative">
				...
				{#if nowPos >= 0 && nowPos <= 100}
					<div class="absolute top-0 bottom-0 z-10" style="left: {nowPos}%;">
						...
						<span class="absolute -top-5 -translate-x-1/2 text-[8px] font-black uppercase tracking-widest text-red-500 bg-red-50 px-1.5 py-0.5 rounded border border-red-200">Now</span>
					</div>
				{/if}
				{/if}
				...
				<div class="space-y-2.5 pt-2">
					{#each variants as variant, i}
						...
						<div class="relative h-9 bg-gray-100 rounded-lg border-[2px] border-gray-200 overflow-hidden">
							{#if style}
								<div
							...
							{:else}
								<div class="flex items-center justify-center h-full">
									<span class="text-[10px] font-bold text-gray-300 italic">No schedule — {variant.name}</span>
								</div>
							{/if}
						</div>
					{/each}
	...
	{#if overlaps.length > 0}
		<div class="bg-[#ffc480]/20 border-[3px] border-[#ffc480] rounded-xl p-4 flex items-start gap-3">
			<svg class="w-5 h-5 text-[#e6a050] shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	{#each variants as variant, i}
		...
		<div class="bg-white border-[3px] border-black rounded-xl shadow-[4px_4px_0_0_black] overflow-hidden">
			...
			<div class="flex items-center gap-3 px-5 py-3 border-b-[3px] border-gray-200 bg-gray-50">
				<div class="w-4 h-4 rounded-full border-[2px] border-black {color.bg}"></div>
			...
			<div class="p-5 space-y-4">
				...
				{#if variant.schedule?.startAt}
					<div class="bg-gray-50 border-[2px] border-gray-200 rounded-lg px-4 py-2.5 flex items-center gap-2">
				...
				{:else}
					<div class="bg-gray-50 border-[2px] border-dashed border-gray-300 rounded-lg px-4 py-2.5 text-center">
						<span class="text-[10px] font-bold text-gray-400">
							{variant.isDefault ? 'Default variant — shown when no other variant is active' : 'Set a start date to schedule this variant'}
						</span>
					</div>
				{/if}
			</div>
	...
	<div class="bg-white border-[3px] border-black rounded-xl shadow-[4px_4px_0_0_black] p-5 space-y-4">
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<div>
				...
				<input
					...
					class="w-full px-3 py-2.5 border-[3px] border-black rounded-xl text-sm font-bold bg-white shadow-[2px_2px_0_0_black] focus:outline-none focus:shadow-[4px_4px_0_0_black] focus:-translate-y-[1px] transition-all"
				/>
				<p class="text-[9px] text-gray-400 mt-1">After this date, the image link returns 410 Gone or shows fallback.</p>
			</div>
			<div>
				<label class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1.5">
					Fallback Image URL
