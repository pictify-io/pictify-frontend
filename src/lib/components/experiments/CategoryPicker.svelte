<script>
	...
	const CATEGORIES = [
		...
		},
		{
			...
			properties: [{ value: 'browser.language', label: 'Browser Language' }]
		},
		{
			id: 'URL',
			label: 'URL',
			icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1',
			properties: [{ value: 'url.param', label: 'URL Parameter' }]
		},
		{
			id: 'Referrer',
			label: 'Referrer',
			icon: 'M10 19l-7-7m0 0l7-7m-7 7h18',
			properties: [{ value: 'referrer.domain', label: 'Referrer Domain' }]
		}
	];
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/20">
	...
	>
		{#if step === 'category'}
			...
			<div
				class="px-5 py-4 bg-[#FFDE82] border-b-[3px] border-black flex items-center justify-between"
			>
				<span class="text-xs font-black uppercase tracking-widest text-black/80"
			...
			<div class="p-5 grid grid-cols-3 gap-4">
				{#each CATEGORIES as cat}
					...
					>
						...
						<span
							class="text-[10px] font-black uppercase tracking-widest text-gray-700 group-hover:text-black"
							>{cat.label}</span
						>
					</button>
				{/each}
		...
		{:else if step === 'property'}
			...
			<div
				class="px-5 py-4 bg-[#c4f0ff] border-b-[3px] border-black flex items-center justify-between"
			>
				<div class="flex items-center gap-2">
					...
					<span class="text-xs font-black uppercase tracking-widest text-black/80"
						>{selectedCategory?.label} properties</span
					>
				</div>
				<button
					type="button"
