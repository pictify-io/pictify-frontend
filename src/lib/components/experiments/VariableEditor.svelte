...
<div>
		...
		<div class="space-y-3 bg-gray-50 p-4 rounded-xl border-[3px] border-gray-200">
			{#each rows as row, rowIndex}
				<div class="flex flex-col sm:flex-row sm:items-center gap-2">
					...
					/>
					{#if !disabled}
						<button type="button" on:click={() => removeRow(rowIndex)}
	...
	{#if availableVars.length > 0 && !disabled}
		<div class="mt-3 flex flex-wrap gap-2">
			<span class="text-[10px] font-black uppercase tracking-widest text-gray-500 self-center">Quick add:</span>
			{#each availableVars as v}
				<button type="button" on:click={() => selectVariable(v.name, v.defaultValue)}
