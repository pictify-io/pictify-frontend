...
{#if open}
	...
	>
		<!-- Modal -->
		<div
			class="w-full max-w-md bg-white rounded-xl border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f293780] overflow-hidden"
		>
			<div class="p-6">
				<!-- Icon -->
				<div
					class="w-12 h-12 {variantStyles.iconBg} rounded-full flex items-center justify-center mx-auto mb-4"
				>
					{#if variant === 'danger'}
						<svg
							class="w-6 h-6 {variantStyles.icon}"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
				...
				{#if showResumeDate}
					<div class="mb-6">
						<label class="block text-sm font-bold text-gray-700 mb-2"> Resume on (optional) </label>
						<input
							type="date"
