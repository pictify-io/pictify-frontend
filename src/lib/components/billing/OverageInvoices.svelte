...
{#if loading}
	<div class="bg-white rounded-2xl border-[3px] border-gray-900 shadow-[8px_8px_0_0_#1f2937] p-8 text-center">
	...
	<div class="bg-white rounded-2xl border-[3px] border-gray-900 shadow-[8px_8px_0_0_#1f2937] overflow-hidden">
		...
		<div class="px-5 py-4 border-b-2 border-gray-200 flex items-center justify-between">
			<div>
				...
				<p class="text-sm text-gray-600">Pay-as-you-go charges for renders beyond your plan limit</p>
			</div>
		...
		{#if hasOverdue}
			<div class="px-5 py-3 bg-red-50 border-b-2 border-red-200 flex items-center gap-3">
				<div class="w-8 h-8 bg-red-100 rounded-lg border-2 border-red-300 flex items-center justify-center flex-shrink-0">
			...
			<table class="w-full">
				<thead>
					<tr class="bg-gray-50 border-b border-gray-200">
						<th class="px-5 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Period</th>
						<th class="px-5 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Renders</th>
						<th class="px-5 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Amount</th>
				...
				<tbody class="divide-y divide-gray-100">
					{#each invoices as invoice (invoice.month + invoice.createdAt)}
						<tr class="hover:bg-gray-50 transition-colors {isOverdue(invoice) ? 'bg-red-50' : ''}">
							...
							<td class="px-5 py-4 whitespace-nowrap">
								<span class="text-sm text-gray-900">{invoice.overageCount} render{invoice.overageCount !== 1 ? 's' : ''}</span>
							</td>
							<td class="px-5 py-4 whitespace-nowrap">
