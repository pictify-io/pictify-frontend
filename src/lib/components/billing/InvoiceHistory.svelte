			...
			<table class="w-full">
				<thead>
					<tr class="bg-gray-50 border-b border-gray-200">
						...
						<th class="px-5 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Description</th>
						...
						<th class="px-5 py-3 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Invoice</th>
					</tr>
				</thead>
					...
					{#each invoices as invoice (invoice.id)}
						<tr class="hover:bg-gray-50 transition-colors">
							<td class="px-5 py-4 whitespace-nowrap">
								<span class="text-sm font-medium text-gray-900">{formatDate(invoice.date || invoice.createdAt)}</span>
							</td>
							<td class="px-5 py-4">
								<span class="text-sm text-gray-900">{getBillingReasonLabel(invoice.billingReason)}</span>
							</td>
							<td class="px-5 py-4 whitespace-nowrap">
								<span class="text-sm font-bold text-gray-900">{invoice.totalFormatted || invoice.total}</span>
							</td>
							<td class="px-5 py-4 whitespace-nowrap">
								<span class="px-2 py-1 text-xs font-bold rounded-full {getStatusColor(invoice.status)}">
