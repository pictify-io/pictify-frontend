...
{:else if !hasAccess && showLockOverlay}
  ...
  <div class="relative">
    <!-- Lock overlay -->
    <div class="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f2937] p-6 max-w-sm w-full text-center relative overflow-hidden">
        <!-- Decorative bg pattern -->
        <div class="absolute inset-0 opacity-5 pointer-events-none bg-[radial-gradient(#000_1px,transparent_1px)] [background-size:8px_8px]"></div>
        
        <div class="relative z-10">
          <div class="w-16 h-16 mx-auto mb-4 bg-[#ffc480] rounded-xl border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f2937] flex items-center justify-center transform hover:scale-105 transition-transform duration-200">
             <svg class="w-8 h-8 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
               <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={featureIconPath} />
             </svg>
          </div>
          ...
          <p class="text-sm font-medium text-gray-600 mb-6 leading-relaxed">
            {upgradePrompt?.message || `Upgrade to ${upgradePrompt?.targetPlanName || 'a higher plan'} to unlock ${featureName.toLowerCase()}.`}
          </p>
          
          {#if upgradePrompt?.benefit}
             <div class="inline-flex items-center gap-1.5 px-3 py-1 bg-[#10b981]/10 text-gray-900 border border-[#10b981]/30 rounded-md text-xs font-black mb-5 transform -rotate-1">
...
{:else if hasAccess && isAtLimit}
  ...
  <div class="relative">
    ...
    <div class="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f2937] p-6 max-w-sm w-full text-center relative overflow-hidden">
        <div class="absolute top-0 left-0 w-full h-1.5 bg-[#ff6b6b] border-b border-gray-900"></div>
        
        <div class="relative z-10 mt-2">
           <div class="w-16 h-16 mx-auto mb-4 bg-white rounded-xl border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f2937] flex items-center justify-center">
             <svg class="w-8 h-8 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
               <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={featureIconPath} />
             </svg>
           </div>
          ...
          <p class="text-sm font-medium text-gray-600 mb-6">
            {limitMessage || `You've used all ${formatLimit(featureAccess.limit)} ${featureName.toLowerCase()} for this month.`}
          </p>
          ...
          {#if upgradePrompt}
            <button
              ...
              on:click={handleUpgrade}
            >
              Upgrade for {upgradePrompt.targetLimit === null ? 'Unlimited' : `${formatLimit(upgradePrompt.targetLimit)}/mo`}
            </button>
          {/if}
        </div>
      </div>
      ...
      >
        ...
        <div class="flex-1">
          <p class="text-sm text-amber-900 font-bold">
            Only {usageStatus.remaining} {featureName} use{usageStatus.remaining !== 1 ? 's' : ''} remaining
          </p>
