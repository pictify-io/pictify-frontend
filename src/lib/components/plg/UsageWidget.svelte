		...
		{#if isHovered}
			<div class="absolute top-full right-0 pt-2 sm:pt-4 w-72 sm:w-80 z-50 animate-fade-in">
				...
				>
					<!-- Header -->
					<div
						class="bg-gray-100 border-b-[3px] border-gray-900 p-3 sm:p-4 flex items-center justify-between"
					>
						<span class="text-xs sm:text-sm font-black text-gray-900 uppercase tracking-widest"
							>Monthly Usage</span
						>
						<span
							...
							<div class="bg-[#ffc480]/20 border-2 border-[#ffc480] rounded-lg p-3">
								...
								<div class="flex justify-between text-xs">
									...
									<span class="font-bold text-gray-900"
										>${(currentOverageCost / 100).toFixed(2)}</span
									>
								</div>
							</div>
						{/if}
...
{:else}
	...
	>
		...
		<div class="bg-gray-100 border-b-[3px] border-gray-900 p-4 flex justify-between items-center">
			...
			{#if showPlan}
				<span
					class="text-[10px] font-black px-2 py-0.5 rounded-md bg-white border-2 border-gray-200 text-gray-900 uppercase tracking-wider"
				>
		...
		<div class="p-6">
			...
			<div class="space-y-2 mb-8">
              ...
              {i < Math.floor($usageWidget.percentage / 10)
							...
							style="background-color: {i < Math.floor($usageWidget.percentage / 10)
								? progressColor
								: ''}"
						/>
					{/each}
