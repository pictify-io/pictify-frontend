...
{#if prompt}
	...
	>
		<!-- Modal -->
		<div
			class="relative bg-brand-bg rounded-xl border-[3px] border-gray-900 shadow-brutal-3xl max-w-lg w-full overflow-hidden"
		...
		>
			...
			<div class="{styles.headerBg} p-6 relative overflow-hidden border-b-[3px] border-gray-900">
				...
				<div class="relative z-10 flex gap-5 items-start">
					<div
						class="w-16 h-16 bg-white rounded-xl border-[3px] border-gray-900 shadow-brutal-lg flex items-center justify-center flex-shrink-0 text-gray-900 transform -rotate-2"
					>
						<svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path
								...
								stroke-width="2"
								d={iconPaths[styles.icon]}
							/>
						</svg>
					</div>
					<div>
						<div
							class="inline-block px-2 py-0.5 rounded bg-black/10 border border-black/10 text-[10px] font-black uppercase tracking-widest text-gray-900 mb-2"
						>
							{styles.label || 'Upgrade'}
						</div>
						<h2
							class="text-2xl font-black text-gray-900 uppercase tracking-tight leading-none mb-2"
						>
							{prompt.title}
						</h2>
						<p class="text-gray-900 text-sm font-bold leading-relaxed opacity-90">
							{prompt.message}
						</p>
					</div>
				</div>
			...
			<div class="p-6">
					...
					<div class="grid grid-cols-2 gap-4 mb-6">
						...
						<div class="p-4 bg-white rounded-lg border-2 border-gray-200 text-center opacity-70">
							<p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">
								Current: {PLAN_DISPLAY_NAMES[plan]}
							</p>
						...
						<!-- Next plan -->
						<div
							class="p-4 bg-brand-accent/10 rounded-lg border-2 border-gray-900 shadow-brutal-md relative text-center relative overflow-hidden"
						>
							<!-- Highlight bg -->
							...
							<p class="text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2">
								To: {PLAN_DISPLAY_NAMES[nextPlan]}
							</p>
									...
									{@const isUpgrade =
										...
										(nextVal === null ||
											...
											(typeof nextVal === 'number' &&
												(currentVal === false || nextVal > currentVal)))}
									<div class="flex justify-between items-center text-xs">
										<span class="font-bold text-gray-600">{label}</span>
										<span class="font-black {isUpgrade ? 'text-[#059669]' : 'text-gray-900'}"
											>{formatFeatureValue(nextVal, unit)}</span
										>
									</div>
								{/each}
							</div>
				...
				{#if timeSaved && timeSaved.value > 0}
					<div
						class="flex items-center gap-3 p-3 bg-brand-success rounded-lg mb-6 border-2 border-gray-900 shadow-brutal-lg"
					>
						...
						<div>
							...
							<p class="text-[10px] uppercase font-bold text-gray-800/80 tracking-wide">
								With more renders, imagine what you could do!
							</p>
						</div>
					</div>
				{/if}
				...
				<div class="space-y-3">
					<button
						class="w-full py-3 px-6 bg-brand-accent text-gray-900 font-black uppercase tracking-widest text-sm rounded-lg border-2 border-gray-900 shadow-brutal-lg hover:shadow-brutal-sm hover:translate-x-[2px] hover:translate-y-[2px] transition-all flex items-center justify-center gap-2 group"
				...
				<!-- Trust signals -->
				<div
					class="mt-4 pt-4 border-t-2 border-gray-100 flex items-center justify-center gap-6 text-[10px] text-gray-400 font-bold uppercase tracking-wide"
				>
