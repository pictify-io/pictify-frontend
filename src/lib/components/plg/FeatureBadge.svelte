...
{#if !hasAccess}
  ...
  <span class="inline-flex items-center gap-1.5 rounded-md bg-gray-100 text-gray-900 font-bold border-2 border-gray-200 uppercase tracking-tight {sizeClasses}">
...
{:else if showLimit && typeof limit === 'number'}
  ...
  {#if remaining !== null}
    <span
      class="inline-flex items-center gap-1.5 rounded-md font-black border-2 {sizeClasses} {remaining <= 2 ? 'bg-[#ff6b6b] text-white border-gray-900' : 'bg-[#10b981] text-gray-900 border-gray-900'}"
    >
      {remaining}/{formatLimit(limit)}
    </span>
  {:else}
    <span class="inline-flex items-center gap-1.5 rounded-md bg-white text-gray-900 font-black border-2 border-gray-900 {sizeClasses}">
      {formatLimit(limit)}/mo
    </span>
  {/if}
{:else if limit === null && hasAccess}
  <!-- Unlimited access -->
  <span class="inline-flex items-center gap-1.5 rounded-md bg-[#10b981] text-gray-900 font-black border-2 border-gray-900 {sizeClasses}">
    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
