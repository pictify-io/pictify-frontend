...
const validators = {
	...
	array: (itemValidator) => (value) => Array.isArray(value) && value.every(itemValidator),
	object: (shape) => (value) => {
...
export const CANVAS_TOOLS = [
	{
		name: 'modify_element',
		description:
			'Change properties of an existing element on the canvas. Use this for color changes, resizing, repositioning, or updating text.',
		input_schema: {
			type: 'object',
			properties: {
				element_id: {
	...
	},
	{
		...
		description:
			'Remove elements from the canvas. Requires user confirmation for destructive operations.',
		input_schema: {
			type: 'object',
			properties: {
				element_ids: {
...
export const PARITY_TOOLS = [
	{
		name: 'undo',
		description:
			'Undo the last canvas change. Use this to revert mistakes or try a different approach.',
		input_schema: {
			type: 'object',
			properties: {}
		}
	...
	},
	{
		name: 'label_element',
		description:
			'Assign a semantic label to an element for easier reference in future conversations.',
		input_schema: {
			type: 'object',
			properties: {
				element_id: {
...
export const ANALYSIS_TOOLS = [
	{
		name: 'analyze_design',
		description:
			'Analyze the current canvas for design quality issues including hierarchy, accessibility, color harmony, and spacing.',
		input_schema: {
			type: 'object',
			properties: {
				checks: {
	...
	},
	{
		...
		description:
			'Check WCAG 2.1 AA compliance for the canvas, focusing on color contrast and text readability.',
		input_schema: {
			type: 'object',
			properties: {
				element_ids: {
 ...
 */
export const COPILOT_TOOLS = [...CANVAS_TOOLS, ...PARITY_TOOLS, ...ANALYSIS_TOOLS];
