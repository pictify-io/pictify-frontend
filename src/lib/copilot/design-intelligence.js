...
export function checkAccessibility(canvasState) {
  ...
  for (const obj of canvasState.objects || []) {
    ...
    if (obj.type === 'textbox' || obj.type === 'i-text' || obj.type === 'text') {
      ...
      const requiredContrast = fontSize >= 18 || (fontSize >= 14 && obj.fontWeight === 'bold') ? 3 : 4.5;
      ...
      if (contrast < requiredContrast) {
        issues.push({
          ...
          suggestion: `Increase contrast to at least ${requiredContrast}:1. Current: ${contrast.toFixed(2)}:1`
        });
      }
    }
...
function detectHierarchyIssues(elements) {
  ...
  const textElements = elements.filter(el =>
    el.type === 'textbox' || el.type === 'i-text' || el.type === 'text'
  );
