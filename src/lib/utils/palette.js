...
export function extractCanvasColors(fabricCanvas) {
	...
	fabricCanvas.getObjects().forEach(obj => {
		...
		// Stroke colors
		if (obj.stroke && typeof obj.stroke === 'string' && obj.stroke.toLowerCase() !== 'transparent') {
...
export function findDominantColor(colors) {
	...
	console.log('Color scores:', colorData.map(c => ({ hex: c.hex, score: c.score.toFixed(3), s: c.hsl.s, l: c.hsl.l })));
