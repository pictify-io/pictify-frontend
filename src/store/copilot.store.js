 ...
 */
export const hasToolsPending = derived(
  copilotTools,
  ($tools) => $tools.pendingTools.length > 0
);
 ...
 */
export const highPrioritySuggestions = derived(
  copilotSuggestions,
  ($suggestions) => $suggestions.pending.filter(s => s.priority === 'high')
);
