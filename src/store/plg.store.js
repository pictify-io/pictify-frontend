...
const _doInitPLG = async () => {
	...
	try {
		...
		const timeSaved =
			minutesSaved >= 60
				? {
						value: Math.round(minutesSaved / 60),
						unit: 'hours',
						display: `${Math.round(minutesSaved / 60)} hour${
							Math.round(minutesSaved / 60) !== 1 ? 's' : ''
						}`
				  }
...
export const getFeatureUpgradePrompt = (feature) => {
	...
	return {
		...
		message:
			upgradeMessages.message ||
			`Unlock ${metadata.name || feature} with ${PLAN_DISPLAY_NAMES[targetPlan]}`,
...
export const checkRenderMilestone = async (newCount) => {
	...
	const percentage = status.usage.limit > 0 ? Math.round((newCount / status.usage.limit) * 100) : 0;
...
const getThresholdConfig = (percentage) => {
	const thresholds = [
		{ percentage: 30, type: 'info', title: 'Great Progress! 🎉', showUpgrade: false },
		{
			...
			title: 'Halfway There!',
			showUpgrade: true,
			cta: { text: 'View plans', action: 'pricing' }
		},
		{
			percentage: 65,
			type: 'soft_warning',
		...
		},
		{
			...
			showUpgrade: true,
			nudgeType: 'proactive_modal',
			cta: { text: 'Upgrade - 5% off', action: 'upgrade', discount: 5 }
		},
		{
			percentage: 85,
			type: 'urgent',
			...
			showUpgrade: true,
			nudgeType: 'toast',
			cta: { text: 'Upgrade - 10% off', action: 'upgrade', discount: 10 }
		},
		{
			percentage: 95,
			type: 'critical',
			title: 'Almost Out!',
			showUpgrade: true,
			cta: { text: 'Upgrade - 15% off', action: 'upgrade', discount: 15 }
		},
		{
			percentage: 100,
			type: 'limit',
			title: 'Limit Reached',
			showUpgrade: true,
			cta: { text: 'Upgrade - 20% off', action: 'upgrade', discount: 20 }
		}
	];
	...
	renders: [
		{
			count: 1,
			type: 'celebration',
			title: '🎉 First Image Created!',
			message: 'You just created your first image with Pictify!'
		},
		{
			count: 5,
			type: 'celebration',
			title: "🚀 You're Getting Started!",
			message: "You've created 5 images. You're on your way!"
		},
		{
			count: 10,
			type: 'soft_upsell',
			title: "🔥 You're on Fire!",
			message: "You've saved ~2 hours of design time!",
			timeSaved: '2 hours'
		},
		{
			count: 25,
			...
			message: "You're in the top 20% of Pictify users!"
		},
		{
			count: 40,
			type: 'urgent_upsell',
			title: '⚠️ Almost at Your Limit',
			message: "You've used 80% of your free tier."
		},
		{
			count: 50,
			type: 'limit_reached',
			title: '🎯 Impressive Work!',
			message: "You've maxed out your free tier this month."
		}
	]
};
...
export const isOverageEnabled = derived(
	overageState,
	($overage) => $overage.allowOverages && $overage.eligible
);
