...
<svelte:head>
	...
	<meta name="keywords" content="{comparison?.title?.toLowerCase()}, comparison, alternative, {comparison?.competitor?.toLowerCase()} alternative" />
	...
	<main class="w-full max-w-5xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12 pb-16 md:pt-20 md:pb-32 relative z-10">
		{#if validComparison}
			...
			<div class="relative flex flex-col items-center justify-center text-center mb-16 pt-4 sm:pt-8">
				...
				<div class="flex items-center justify-center gap-4 sm:gap-8 mb-10">
					...
					<!-- VS Badge -->
					<div class="w-12 h-12 sm:w-16 sm:h-16 bg-[#ff6b6b] border-[3px] border-gray-900 rounded-full flex items-center justify-center font-black text-white text-lg sm:text-2xl shadow-[4px_4px_0_0_#1f2937] z-10">
						VS
					</div>

					<!-- Competitor Icon -->
					<div class="w-20 h-20 sm:w-28 sm:h-28 bg-white border-[3px] border-gray-900 rounded-2xl flex items-center justify-center shadow-[6px_6px_0_0_#1f2937] transform rotate-3 hover:rotate-0 transition-transform duration-300" style="color: {icon.color || '#1f2937'}">
					{#if icon.type === 'url'}
						<img src={icon.url} alt={comparison.competitor} class="w-10 h-10 sm:w-14 sm:h-14" />
					{:else if icon.type === 'text'}
						<span class="text-2xl sm:text-4xl font-black" style="color: {icon.color}">{icon.text}</span>
					{:else if icon.type === 'svg'}
						<svg class="w-10 h-10 sm:w-14 sm:h-14" fill="currentColor" viewBox={icon.viewBox}>
							<path d={icon.path} />
						</svg>
					{:else if icon.type === 'fa'}
						<i class="{icon.class} text-4xl sm:text-6xl"></i>
					{:else}
						<span class="text-xs sm:text-sm font-black text-gray-900 px-2">{comparison.competitor}</span>
					{/if}
					</div>
				</div>
			...
			{#if comparison.tldr}
				<section class="mb-12">
					<div class="bg-white border-[3px] border-gray-900 rounded-2xl p-6 shadow-[4px_4px_0_0_#ffc480]">
						<p class="text-gray-700 font-bold leading-relaxed">
							<span class="text-[#ff6b6b] font-black uppercase tracking-wide">TL;DR:</span> {comparison.tldr}
						</p>
					</div>
				</section>
			{/if}
			...
			<section class="grid md:grid-cols-2 gap-6 mb-16">
				...
				<div class="bg-gray-50 border-[3px] border-dashed border-gray-400 rounded-2xl p-8">
					<div class="flex items-center gap-4 mb-6">
						<div class="w-14 h-14 bg-white border-[3px] border-gray-900 rounded-xl flex items-center justify-center font-black text-gray-600 text-sm shadow-[3px_3px_0_0_#9ca3af]" style="color: {icon.color || '#1f2937'}">
						{#if icon.type === 'url'}
						...
						{#each comparison.competitorAdvantages as advantage}
							<li class="flex items-start gap-3">
								<div class="w-6 h-6 rounded-full bg-gray-200 border-[2px] border-gray-400 flex items-center justify-center flex-shrink-0 mt-0.5">
									<svg class="w-3 h-3 text-gray-500 font-bold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M5 13l4 4L19 7" />
									</svg>
								</div>
			...
			<!-- Best For Section -->
			<section class="mb-16">
				<h2 class="text-2xl md:text-3xl font-black text-gray-900 mb-8 text-center uppercase tracking-tight">
				...
				<div class="grid md:grid-cols-2 gap-6">
					<div class="bg-white border-[3px] border-gray-900 rounded-2xl p-8 shadow-[6px_6px_0_0_#4ade80]">
						<h3 class="text-xl font-black text-gray-900 mb-4 flex items-center gap-3">
							<span class="w-8 h-8 bg-[#4ade80] border-[2px] border-gray-900 rounded-lg flex items-center justify-center text-gray-900 font-black text-sm shadow-[2px_2px_0_0_#1f2937]">1</span>
							Choose Pictify if...
						</h3>
					...
					<div class="bg-white border-[3px] border-gray-900 rounded-2xl p-8 shadow-[6px_6px_0_0_#e5e7eb]">
						<h3 class="text-xl font-black text-gray-700 mb-4 flex items-center gap-3">
							<span class="w-8 h-8 bg-gray-100 border-[2px] border-gray-900 rounded-lg flex items-center justify-center text-gray-500 font-black text-sm shadow-[2px_2px_0_0_#1f2937]">2</span>
					...
					<div class="bg-white border-[3px] border-gray-900 rounded-2xl overflow-hidden shadow-[6px_6px_0_0_#1f2937]">
						<table class="w-full">
							<thead class="bg-gray-900 text-white">
								<tr>
									<th class="px-6 py-4 text-left font-black uppercase tracking-wide text-sm">Feature</th>
									<th class="px-6 py-4 text-center font-black uppercase tracking-wide text-sm border-l-[3px] border-gray-800">Pictify</th>
									<th class="px-6 py-4 text-center font-black uppercase tracking-wide text-sm border-l-[3px] border-gray-800">{comparison.competitor}</th>
								</tr>
							</thead>
							<tbody class="divide-y-[3px] divide-gray-100">
								{#each Object.entries(comparison.features) as [feature, scores], i}
									<tr class="{i % 2 === 1 ? 'bg-gray-50' : 'bg-white'} hover:bg-gray-50">
										<td class="px-6 py-4 font-bold text-gray-900">{featureLabels[feature] || feature}</td>
										<td class="px-6 py-4 text-center border-l-[3px] border-gray-100">
											<div class="flex justify-center gap-1.5">
												{#each Array(5) as _, star}
													<div class="w-3 h-3 sm:w-4 sm:h-4 rounded-full border-[2px] border-gray-900 {star < scores.pictify ? 'bg-[#4ade80]' : 'bg-transparent opacity-20'}"></div>
												{/each}
											</div>
										</td>
										<td class="px-6 py-4 text-center border-l-[3px] border-gray-100">
											<div class="flex justify-center gap-1.5">
												{#each Array(5) as _, star}
													<div class="w-3 h-3 sm:w-4 sm:h-4 rounded-full border-[2px] border-gray-900 {star < scores.competitor ? 'bg-gray-400' : 'bg-transparent opacity-20'}"></div>
												{/each}
											</div>
										</td>
									</tr>
			...
			{#if comparison.pricing}
				<section class="mb-16">
					...
					<div class="bg-white border-[3px] border-gray-900 rounded-2xl overflow-hidden shadow-[6px_6px_0_0_#4ade80]">
						<table class="w-full">
							<thead class="bg-gray-900 text-white">
								<tr>
									<th class="px-6 py-4 text-left font-black uppercase tracking-wide text-sm">Plan</th>
									<th class="px-6 py-4 text-left font-black uppercase tracking-wide text-sm border-l-[3px] border-gray-800">Pictify</th>
									<th class="px-6 py-4 text-left font-black uppercase tracking-wide text-sm border-l-[3px] border-gray-800">{comparison.competitor}</th>
								</tr>
							</thead>
							<tbody class="divide-y-[3px] divide-gray-100">
			...
			{#if comparison.migration && comparison.migration.difficulty !== 'N/A'}
				<section class="mb-16">
					...
					<div class="bg-white border-[3px] border-gray-900 rounded-2xl p-8 shadow-[4px_4px_0_0_#4ade80]">
						<div class="flex flex-wrap gap-8 mb-8 border-b-[3px] border-gray-100 pb-8">
							<div>
								<span class="text-sm font-bold text-gray-500 uppercase tracking-wide block mb-1">Difficulty</span>
								<div class="inline-flex px-3 py-1 bg-gray-900 text-white rounded-lg font-black text-sm uppercase tracking-wide">
									{comparison.migration.difficulty}
								</div>
							</div>
							<div>
								<span class="text-sm font-bold text-gray-500 uppercase tracking-wide block mb-1">Time Estimate</span>
								<p class="text-2xl font-black text-gray-900">{comparison.migration.timeEstimate}</p>
							</div>
						</div>
						<div>
							<span class="text-sm font-bold text-gray-500 uppercase tracking-wide mb-6 block">Migration Steps</span>
							<ol class="space-y-4">
								{#each comparison.migration.steps as step, i}
									<li class="flex items-center gap-4">
										<span class="w-10 h-10 bg-[#4ade80] border-[3px] border-gray-900 rounded-xl flex items-center justify-center text-gray-900 text-lg font-black shadow-[2px_2px_0_0_#1f2937] flex-shrink-0">{i + 1}</span>
				...
				<section class="mb-16">
					...
					<div class="space-y-4">
						{#each comparison.faqs as faq}
							<div class="bg-white border-[3px] border-gray-900 rounded-xl p-6 shadow-[4px_4px_0_0_#e5e7eb] hover:shadow-[2px_2px_0_0_#e5e7eb] hover:translate-x-[2px] hover:translate-y-[2px] transition-all">
								<h3 class="text-lg font-black text-gray-900 mb-3 flex items-start gap-3">
									<span class="text-[#ff6b6b]">Q.</span> {faq.q}
								</h3>
								<p class="text-gray-600 font-medium leading-relaxed pl-7 border-l-[3px] border-gray-100 ml-1.5">
									{faq.a}
								</p>
							</div>
						{/each}
			...
			<!-- CTA Section -->
			<section class="mb-16 bg-gray-900 border-[4px] border-gray-900 rounded-3xl p-8 md:p-12 text-center shadow-[8px_8px_0_0_#ff6b6b]">
				<h2 class="text-3xl md:text-4xl font-black text-white mb-4">
					Try Pictify Free
				</h2>
				<p class="text-gray-400 font-bold mb-8 max-w-xl mx-auto">
					See why teams choose Pictify for HTML to image generation. Start in minutes, no credit card required.
				</p>
					...
					{#each otherComparisons as comp}
						...
						>
							<div class="flex items-center gap-4">
								<div class="w-12 h-12 bg-white border-[2px] border-gray-900 rounded-xl flex items-center justify-center font-black text-gray-900 text-lg shadow-[2px_2px_0_0_rgba(0,0,0,0.1)]" style="color: {compIcon.color || '#1f2937'}">
								{#if compIcon.type === 'url'}
									<img src={compIcon.url} alt={comp.competitor} class="w-6 h-6" />
								{:else if compIcon.type === 'text'}
									<span class="text-sm font-black" style="color: {compIcon.color}">{compIcon.text}</span>
								...
								{:else if compIcon.type === 'fa'}
									<i class="{compIcon.class} text-xl"></i>
								{:else}
								...
								<div class="flex-1">
									<div class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-0.5">VS</div>
									<h3 class="font-black text-gray-900 text-lg leading-none group-hover:text-gray-900">{comp.competitor}</h3>
								</div>
		...
		{:else}
			...
			<div class="min-h-[50vh] flex flex-col items-center justify-center text-center space-y-8 px-4">
				...
				<h1 class="text-4xl md:text-6xl font-black uppercase tracking-tighter text-gray-900">Comparison not found</h1>
				<a
					href="/compare"
