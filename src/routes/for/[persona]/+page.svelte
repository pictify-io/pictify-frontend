...
$: canonical = validPersona
	? `https://pictify.io/for/${personaId}`
	: 'https://pictify.io';
...
<svelte:head>
	...
	<meta name="keywords" content="pictify for {personaId?.replace('-', ' ')}, image generation {personaId?.replace('-', ' ')}, {personaId?.replace('-', ' ')} tools" />
	...
	<main class="w-full max-w-6xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12 pb-16 md:pt-20 md:pb-32 relative z-10">
		{#if validPersona}
			...
			<div class="relative flex flex-col items-center justify-center text-center mb-16 pt-4 sm:pt-8">
				<!-- Icon -->
				<div class="w-20 h-20 bg-[#ff6b6b] border-[4px] border-gray-900 rounded-2xl flex items-center justify-center shadow-[6px_6px_0_0_#1f2937] mb-8">
			...
			<!-- Benefits Section -->
			<section class="mb-20">
				<h2 class="text-2xl md:text-3xl font-black uppercase tracking-tight text-gray-900 mb-8 text-center">
					Why {personaId.replace('-', ' ')} love Pictify
				</h2>
				<div class="grid sm:grid-cols-2 gap-6">
					{#each persona.benefits as benefit, i}
						<div class="bg-white border-[3px] border-gray-900 p-6 rounded-2xl shadow-[6px_6px_0_0_#1f2937] hover:-translate-y-1 transition-all">
							<div class="flex items-start gap-4">
								<div class="w-10 h-10 bg-[#4ade80] border-[3px] border-gray-900 rounded-xl flex items-center justify-center font-black text-gray-900 shadow-[3px_3px_0_0_#1f2937] flex-shrink-0">
									{i + 1}
								</div>
				...
				<div class="bg-[#ffc480]/20 border-[3px] border-[#ffc480] rounded-2xl p-8">
					<div class="grid md:grid-cols-2 gap-6">
						{#each persona.useCases as useCase}
							<div class="flex items-start gap-4">
								<div class="w-6 h-6 bg-[#ffc480] border-[2px] border-gray-900 rounded-lg flex items-center justify-center flex-shrink-0 mt-1">
			...
			{#if persona.testimonial}
				<section class="mb-20">
					<div class="bg-gray-900 border-[4px] border-gray-900 rounded-3xl p-8 md:p-12 shadow-[8px_8px_0_0_#ff6b6b] relative overflow-hidden">
			...
			<!-- CTA Section -->
			<section class="mb-20 bg-white border-[4px] border-gray-900 rounded-3xl p-8 md:p-12 text-center shadow-[8px_8px_0_0_#4ade80]">
					...
					{#each otherPersonas as otherPersona}
						...
						>
							<div class="flex items-center gap-3">
								<div class="w-10 h-10 bg-gray-100 group-hover:bg-white border-[2px] border-gray-900 rounded-lg flex items-center justify-center">
									<svg class="w-5 h-5 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										{@html personaIcons[otherPersona.slug] || personaIcons.developers}
									</svg>
								</div>
								<span class="font-black text-gray-900 text-sm capitalize">{otherPersona.slug.replace('-', ' ')}</span>
							</div>
						</a>
					{/each}
				</div>
		...
		{:else}
			...
			<div class="min-h-[50vh] flex flex-col items-center justify-center text-center space-y-8 px-4">
				...
				<h1 class="text-4xl md:text-6xl font-black uppercase tracking-tighter text-gray-900">Page not found</h1>
				<a
					href="/"
