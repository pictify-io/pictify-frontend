...
<svelte:head>
  {#if template}
    ...
    <meta
      name="description"
      content={template.publicDescription || `Remix the ${template.name} template and generate unlimited variants via API.`}
    >
    ...
    <meta property="og:description" content={template.publicDescription || `Remix this template and automate variants via API.`}>
  ...
  <main class="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 py-8 md:py-12">
    ...
    {#if loading}
      ...
      <div class="grid lg:grid-cols-2 gap-8">
        <div class="bg-white border-[3px] border-gray-900 shadow-[6px_6px_0_0_#1f2937] aspect-video animate-pulse">
    ...
    {:else if template}
        ...
        <div>
          <div class="bg-white border-[3px] border-gray-900 shadow-[8px_8px_0_0_#1f2937] overflow-hidden">
            {#if template.thumbnail}
              ...
              />
            {:else}
              <div class="w-full aspect-video flex items-center justify-center bg-gradient-to-br from-[#ffc480] to-[#ff6b6b]">
                <span class="text-white font-black text-6xl opacity-50">{template.name.charAt(0)}</span>
              </div>
            {/if}
          </div>
        ...
        <div>
          ...
          <div class="mb-6">
            <div class="flex items-center gap-3 mb-3">
              <span class="px-3 py-1 bg-[#ffc480] border-2 border-gray-900 text-xs font-black uppercase">
                {formatLabel(template.category)}
              </span>
              {#if template.isFeatured}
                <span class="px-3 py-1 bg-[#4ade80] border-2 border-gray-900 text-xs font-black uppercase">
                  Featured
                </span>
              {/if}
            </div>
                ...
                {#each template.variableDefinitions as variable}
                  <div class="p-4">
                    <div class="flex items-center justify-between mb-1">
                      ...
                      <span class="text-xs font-bold px-2 py-0.5 bg-gray-100 border border-gray-300 rounded">
                        {variable.type}
                      </span>
                    </div>
                    ...
                    {#if variable.defaultValue}
                      <p class="text-xs text-gray-500 mt-1">
                        Default: <code class="bg-gray-100 px-1 rounded">{variable.defaultValue}</code>
                      </p>
                    {/if}
                  </div>
                {/each}
      ...
      <section class="mt-12 bg-white border-[3px] border-gray-900 shadow-[6px_6px_0_0_#1f2937]">
        ...
        <pre class="p-6 overflow-x-auto text-sm font-mono text-gray-800 bg-gray-50"><code>{getApiExample()}</code></pre>
      </section>
