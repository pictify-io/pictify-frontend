	...
	async function callEndpoint(action) {
		...
		try {
			...
			if (action === 'unsubscribe') {
				status = 'unsubscribed';
				message = "You've been unsubscribed from marketing emails. You'll still receive important account notifications.";
			} else {
				status = 'resubscribed';
...
<section class="flex justify-center items-center w-screen h-screen px-4 bg-[#FFFDF8]">
	<div class="w-full max-w-lg bg-white border-[3px] border-gray-900 rounded-xl p-8 sm:p-10 shadow-[8px_8px_0_0_#1f2937] relative overflow-hidden">
		<!-- Decorative blobs -->
		<div class="absolute -top-10 -right-10 w-40 h-40 bg-[#ffc480] rounded-full blur-3xl opacity-20 pointer-events-none"></div>
		<div class="absolute -bottom-10 -left-10 w-40 h-40 bg-[#4ade80] rounded-full blur-3xl opacity-20 pointer-events-none"></div>

		<div class="relative z-10 flex flex-col items-center justify-center text-center">
			<div class="mb-8 relative">
				<div class="flex items-center gap-4 justify-center relative z-10">
					<div class="w-12 h-12 bg-gray-900 rounded-lg flex items-center justify-center shadow-[4px_4px_0_0_#ffc480] border-[3px] border-transparent">
						<svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M13 10V3L4 14h7v7l9-11h-7z" />
						</svg>
					</div>
					<div class="text-5xl font-black tracking-tighter text-gray-900 leading-none">
						PICTIFY
					</div>
				</div>
				<div class="absolute inset-0 bg-yellow-300 blur-xl opacity-30 -z-10 transform scale-150"></div>
			</div>
			...
			{#if isProcessing}
				<div class="flex flex-col items-center justify-center py-8 space-y-4">
					<div class="relative w-16 h-16">
						<div class="absolute inset-0 border-[4px] border-gray-200 rounded-full"></div>
						<div class="absolute inset-0 border-[4px] border-gray-900 border-t-transparent rounded-full animate-spin"></div>
					</div>
					<p class="font-bold text-gray-900 animate-pulse">{message}</p>
				</div>
			{:else}
				{#if status === 'unsubscribed'}
					<div class="border-[3px] border-gray-900 bg-[#4ade80] text-gray-900 rounded-xl p-5 shadow-[4px_4px_0_0_#1f2937] flex items-start gap-4 transform rotate-1">
				...
				{:else if status === 'error'}
					<div class="border-[3px] border-gray-900 bg-[#ff6b6b] text-white rounded-xl p-5 shadow-[4px_4px_0_0_#1f2937] flex items-start gap-4 transform -rotate-1">
						<div class="bg-white text-gray-900 rounded-full p-1 border-[2px] border-gray-900 flex-shrink-0">
							<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				{:else}
					<div class="border-[3px] border-gray-900 bg-gray-50 text-gray-600 rounded-xl p-5 text-sm font-bold text-center border-dashed">
						{message}
					</div>
				{/if}
