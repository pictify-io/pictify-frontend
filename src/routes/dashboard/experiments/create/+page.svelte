<script>
	...
	async function loadExperimentForEdit(uid) {
		try {
			...
			for (let i = 0; i < form.variants.length; i++) {
				const tplUid =
					form.testMode === 'different_templates' ? form.variants[i].templateUid : form.templateUid;
				if (!tplUid) continue;
	...
	function addVarRow(variantIndex) {
		variantVarEditors[variantIndex] = [
			...(variantVarEditors[variantIndex] || []),
			{ key: '', value: '' }
		];
		variantVarEditors = variantVarEditors;
	...
	function removeVarRow(variantIndex, rowIndex) {
		variantVarEditors[variantIndex] = variantVarEditors[variantIndex].filter(
			(_, i) => i !== rowIndex
		);
		variantVarEditors = variantVarEditors;
		syncVarsFromEditor(variantIndex);
	...
	function selectVariableFromDropdown(variantIndex, varName, defaultValue) {
		...
		if (!existingKeys.includes(varName)) {
			variantVarEditors[variantIndex] = [
				...editors,
				{ key: varName, value: defaultValue != null ? String(defaultValue) : '' }
			];
			variantVarEditors = variantVarEditors;
			syncVarsFromEditor(variantIndex);
		}
	...
	async function renderVariantPreview(variantIndex) {
		...
		if (!tplData?.fabricJSData) {
			variantPreviews[variantIndex] = {
				url: null,
				loading: false,
				error: 'No canvas data available for this template'
			};
	...
	<div class="mx-auto max-w-5xl transition-all duration-300">
		...
		<div class="mb-6">
			...
			<p class="mt-2 text-sm font-bold text-gray-500">
				{isEditMode
					? 'Update your A/B test configuration.'
					: 'Split traffic randomly between variants to find the best performer.'}
			</p>
		</div>
		...
		{#if currentStep === 'setup'}
			...
			>
				<div
					class="bg-gray-50 border-b-[3px] border-gray-900 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
				>
					<div>
				...
				<div class="p-6 space-y-6 bg-brand-bg">
					...
					<div class="pt-4 border-t-[2px] border-dashed border-gray-200">
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							>
								...
								{:else}
									<div
										class="absolute top-4 right-4 w-6 h-6 bg-gray-100 border-[3px] border-gray-300 rounded-full flex items-center justify-center border-dashed"
									/>
								{/if}
								<div class="flex flex-col items-start gap-4 pr-8">
									...
									>
										<svg
											class="w-5 h-5 {form.testMode === 'same_template'
												? 'text-gray-900'
												: 'text-gray-400'}"
											fill="none"
											stroke="currentColor"
									...
									<div>
										<div class="text-sm font-black text-gray-900 uppercase tracking-wide">
											Same Template
										</div>
										<div class="text-xs text-gray-600 mt-1 font-bold leading-relaxed">
											One template, different data. Test headlines, colors, CTAs by changing
											variables.
										</div>
									</div>
								</div>
							...
							>
								...
								<div class="flex flex-col items-start gap-4 pr-8">
									...
									>
										<svg
											class="w-5 h-5 {form.testMode === 'different_templates'
												? 'text-gray-900'
												: 'text-gray-400'}"
											fill="none"
											stroke="currentColor"
					...
					<div class="pt-4 border-t-[2px] border-dashed border-gray-200">
						...
						>
							...
							<span
								class="text-[10px] font-black uppercase tracking-widest text-gray-500 bg-gray-100 border-[2px] border-gray-300 px-2 py-1 rounded shadow-[1px_1px_0_0_#d1d5db]"
								>Optional</span
							>
						</label>
						<textarea
							...
							rows="3"
							class="w-full px-4 py-3 border-[3px] border-gray-300 rounded-xl text-sm font-bold text-gray-900 bg-white focus:outline-none focus:border-brand-accent focus:shadow-brutal-accent resize-none placeholder-gray-400 transition-all shadow-[inset_2px_2px_0_0_rgba(0,0,0,0.05)]"
						/>
					</div>
				</div>
				...
				>
					<a
						href={isEditMode
							? `/dashboard/experiments/${editExperimentUid}`
							: '/dashboard/experiments'}
						class="px-4 py-2.5 text-xs font-black text-gray-500 hover:text-gray-900 uppercase tracking-wide transition-colors flex items-center gap-2"
					>
						Cancel
					</a>
					<button
			...
			>
				...
				>
					...
					<div class="flex flex-col sm:items-end gap-1.5">
						<span
							class="text-[10px] font-black uppercase tracking-widest {weightIsValid
								? 'text-green-600'
								: 'text-red-500'}"
						>
						...
						<div
							class="w-full sm:w-48 h-3 rounded-full overflow-hidden flex shadow-[inset_1px_1px_0_0_rgba(0,0,0,0.1)] border-[2px] {weightIsValid
							...
							{#each form.variants as variant, idx}
								{#if variant.weight > 0}
									<div
										...
										style="width: {variant.weight / 100}%; background-color: {idx === 0
											? '#4ade80'
											: idx === 1
											? '#ffc480'
											: idx === 2
				...
				<div class="p-6 space-y-6 bg-brand-bg">
					...
					{#if form.testMode === 'same_template'}
						<div
							class="bg-white border-[3px] border-gray-900 rounded-xl p-6 shadow-brutal-lg"
						>
							<label
								for="template-select"
								class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-3"
							...
							{#if form.templateUid}
								...
								{#if sel}
									...
									>
										...
										{:else}
											<div
												class="w-24 h-24 bg-white rounded-lg border-[2px] border-gray-300 flex items-center justify-center shadow-sm"
											>
												<svg
										...
										<div>
											...
											<div class="text-sm font-bold text-gray-500 mt-1">
												{sel.width || '?'} &times; {sel.height || '?'} px
											</div>
											{#if getTemplateVars(form.templateUid).length > 0}
												<div
													class="inline-flex mt-2 items-center px-2 py-1 rounded bg-gray-200 text-[10px] font-black uppercase tracking-widest text-gray-700"
												>
													{getTemplateVars(form.templateUid).length} editable variable{getTemplateVars(
														form.templateUid
													).length !== 1
														? 's'
														: ''}
						...
						<div class="flex flex-wrap gap-3">
							{#each quickSplits as qs}
								<button
									type="button"
									on:click={qs.fn}
									class="px-4 py-2 border-[2px] border-gray-900 rounded-lg text-xs font-black text-gray-900 uppercase tracking-wide bg-white shadow-brutal-sm hover:bg-brand-accent hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-brutal-md transition-all"
								>
									{qs.label}
								</button>
							{/each}
					...
					{#if !weightIsValid}
						<div
							class="flex items-center gap-3 px-5 py-4 bg-red-50 border-[3px] border-red-500 rounded-xl shadow-[4px_4px_0_0_#ef4444]"
						>
							<svg
						...
						{#each form.variants as variant, index (variant.id)}
							<div
								...
								{variant.isDefault
									...
									: 'border-gray-900 shadow-brutal-2xl'}"
							>
								...
								>
									<div class="flex items-center gap-4">
										...
										{#if variant.isDefault}
											<span
												class="px-3 py-1 bg-brand-accent border-[2px] border-gray-900 rounded-lg text-[10px] font-black uppercase tracking-widest text-gray-900 shadow-brutal-sm"
											>
												Default Variant
											</span>
										{/if}
									...
									<div class="flex items-center gap-4">
										<div
											class="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border-[3px] border-gray-900 shadow-brutal-sm"
										>
										...
										<span
											class="text-sm font-black {weightIsValid
												? 'text-green-600'
												: 'text-gray-500'} bg-white border-[2px] border-gray-200 px-3 py-2.5 rounded-xl"
										>
										...
										<div class="flex items-center gap-2 ml-2">
											{#if !variant.isDefault}
												<button
													type="button"
													on:click={() => setDefault(index)}
													class="p-2.5 bg-white border-[2px] border-gray-300 rounded-xl text-gray-500 hover:text-gray-900 hover:border-gray-900 hover:shadow-brutal-sm transition-all"
											...
											{#if form.variants.length > 2}
												<button
													...
													class="p-2.5 bg-white border-[2px] border-gray-300 rounded-xl text-gray-500 hover:text-red-600 hover:bg-red-50 hover:border-red-600 hover:shadow-[2px_2px_0_0_#ef4444] transition-all"
								...
								>
									...
									<div class="space-y-6">
										...
										<div>
											<div
												class="flex items-center justify-between mb-3 border-b-[3px] border-gray-900 pb-2"
											>
											...
											{#if (variantVarEditors[index] || []).length > 0}
												<div
													class="space-y-3 bg-gray-50 p-4 rounded-xl border-[3px] border-gray-200"
												>
											...
											{:else}
												<div
													class="bg-gray-50 border-[3px] border-dashed border-gray-300 rounded-xl p-6 text-center"
												>
												...
												<div class="relative z-10 w-full h-full flex items-center justify-center">
													...
													{:else if variantPreviews[index]?.error}
														...
														>
															...
															<p class="text-xs font-black uppercase text-red-900">
					...
					<button
						type="button"
						on:click={addVariant}
						class="w-full py-4 mt-3 border-[3px] border-dashed border-gray-300 rounded-xl bg-gray-50 text-gray-500 hover:border-gray-900 hover:text-gray-900 hover:bg-brand-accent/20 hover:shadow-brutal-md hover:-translate-y-1 transition-all flex items-center justify-center gap-2 group"
					>
						<div
				...
				>
					...
					<button
						...
						class="px-5 py-2.5 rounded-lg text-xs font-black uppercase tracking-wide transition-all border-[2px] flex items-center gap-2
							{canGoStep3
							? 'bg-gray-900 text-white border-gray-900 shadow-brutal-accent hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-brutal-accent-sm'
							: 'bg-gray-200 text-gray-400 border-gray-300 cursor-not-allowed hidden'}"
					>
						Next Step
						<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<div class="space-y-6">
				...
				>
					...
					<div class="p-6 space-y-6 bg-brand-bg">
						...
						>
							<button
								...
								class="text-left p-4 border-[2px] rounded-lg transition-all relative
									...
									? 'border-gray-900 bg-white shadow-brutal-lg z-10 scale-[1.02]'
							...
							>
								...
								<div class="flex flex-col gap-3">
									...
									<div>
										...
										<div
											class="text-sm font-bold mt-1 {form.goalConfig.type === 'impressions_only'
												? 'text-gray-600'
												: 'text-gray-400'}"
										>
											Track how many times each variant is viewed by users.
										</div>
									</div>
								</div>
							</button>
							...
							>
								...
								<div class="flex flex-col gap-3">
									...
									<div>
										...
										<div
											class="text-sm font-bold mt-1 {form.goalConfig.type === 'click_through'
												? 'text-gray-600'
												: 'text-gray-400'}"
										>
						...
						{#if form.goalConfig.type === 'click_through'}
							<div
								class="bg-gray-50 border-[3px] border-gray-900 p-6 rounded-xl shadow-brutal-lg"
							>
								<label
									for="destination-url"
								...
								>
									...
									<span
										...
										title="Where users should end up after clicking your ad.">?</span
									>
								</label>
								<div class="relative">
									...
									<input
										...
										class="w-full pl-10 pr-4 py-3 border-[3px] border-gray-300 rounded-xl text-sm font-bold text-gray-900 bg-white focus:outline-none focus:border-brand-accent focus:shadow-brutal-accent transition-all shadow-sm placeholder:text-gray-400"
									/>
								</div>
								<p
							...
							>
								...
								<div class="relative">
									...
									<input
										...
										placeholder="signup"
										class="w-full pl-10 pr-4 py-3 border-[3px] rounded-xl text-sm font-bold text-gray-900 bg-white focus:outline-none focus:border-brand-accent focus:shadow-brutal-accent transition-all shadow-sm placeholder:text-gray-400
					...
					<div class="px-6 py-5 flex items-start gap-4 bg-brand-bg">
						...
						<div class="flex-1">
							...
							<p class="text-xs font-bold text-gray-500 mt-1">
								Let Pictify find the winner for you. Traffic starts split evenly, then automatically
								shifts toward the best-performing variant over time.
							</p>
						</div>
					</div>
				</div>
				...
				>
					<button
						...
						class="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors bg-white border-b-[3px] {showAdvanced
							? 'border-gray-900'
							: 'border-transparent'}"
					>
						<span
							class="text-xs font-black uppercase tracking-wide text-gray-900 flex items-center gap-2"
						>
							<svg
						...
						<div
							class="w-8 h-8 rounded-full border-[2px] border-gray-900 flex items-center justify-center transition-transform {showAdvanced
						...
						<div class="p-6 space-y-6 bg-brand-bg">
							...
							<div>
								...
								<div class="relative">
									...
									{#if !isEditMode}
										<div
											class="absolute right-4 top-1/2 -translate-y-1/2 bg-white rounded-full p-1"
										>
								...
								{:else}
									<p class="mt-2 text-sm font-bold text-gray-500">
										The slug cannot be changed for an existing experiment.
									</p>
								{/if}
								<div
									class="mt-4 flex items-center gap-3 px-4 py-3 bg-gray-100 border-[3px] border-gray-200 rounded-xl overflow-hidden shadow-[inset_1px_1px_0_0_rgba(0,0,0,0.05)]"
								>
									...
									<code class="text-sm font-bold text-gray-700 truncate font-mono"
										>{previewUrl}</code
									>
								</div>
							</div>
							...
							<!-- Format & Quality -->
							<div
								class="grid grid-cols-1 md:grid-cols-2 gap-8 border-t-[3px] border-gray-200 pt-8"
							>
								<div>
									<label
										class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-3"
									...
									<div class="mt-4 px-2">
										<input
											...
											style="background: linear-gradient(to right, #1f2937 0%, #1f2937 {form
												.outputConfig.quality}%, #e5e7eb {form.outputConfig
												.quality}%, #e5e7eb 100%);"
										/>
										<div
											class="flex justify-between text-[10px] font-black uppercase tracking-widest text-gray-400 mt-2"
										>
											<span>Lightest</span>
											<span>Balanced</span>
											<span>Highest</span>
				...
				<div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
					...
					<div class="lg:col-span-2 space-y-4">
						<h3 class="text-sm font-black uppercase tracking-widest text-gray-500 ml-2">
							Verification Receipt
						</h3>
						<div
							class="bg-white border-[3px] border-gray-900 overflow-hidden shadow-brutal-xl pb-6 pt-4 px-1 relative
						...
						>
							<div
								class="text-center pb-6 border-b-[2px] border-dashed border-gray-300 mx-6 bg-[linear-gradient(to_bottom,transparent_0%,rgba(0,0,0,0.02)_100%)]"
							>
								<h2 class="text-lg font-black uppercase tracking-tighter text-gray-900">
									Pictify.io
								</h2>
							...
							<div class="px-6 py-4">
								<div class="space-y-4 font-mono text-sm leading-relaxed">
									<div class="flex justify-between items-start gap-4">
										...
										<span class="font-black text-gray-900 text-right break-words"
											>{form.name || 'UNNAMED'}</span
										>
									</div>
									...
									<div class="flex justify-between items-start gap-4">
										...
										<span class="font-black text-gray-900 text-right"
											>{form.testMode === 'same_template' ? 'SHARED_TPL' : 'MULTI_TPL'}</span
										>
									</div>
									...
									<div class="flex justify-between items-start gap-4">
										...
										<div class="text-right">
											<span class="font-black text-gray-900 block mb-1"
												>{form.variants.length} CONFIGURED</span
											>
									...
									<div class="flex justify-between items-start gap-4">
										...
										<div class="text-right">
											...
											{#if form.goalConfig.type === 'click_through' && form.goalConfig.destinationUrl}
												<div
													class="text-[10px] text-blue-600 truncate max-w-[150px] mt-1"
									...
									{#if form.hypothesis}
										<div class="pt-4 border-t-[2px] border-dashed border-gray-300 mt-4">
											...
											<p
												class="text-xs text-gray-700 italic border-l-2 border-gray-300 pl-3 py-1 font-sans font-bold"
											>
												"{form.hypothesis}"
											</p>
										</div>
					...
					<div class="lg:col-span-3 space-y-4 flex flex-col">
						...
						<div
							class="flex-1 bg-gray-900 rounded-xl border-[3px] border-gray-900 overflow-hidden shadow-brutal-xl flex flex-col"
						>
							<!-- Terminal Header -->
							<div
								class="h-10 border-b-[3px] border-gray-700 flex items-center px-4 bg-gray-800 shrink-0"
							>
								...
								<div
									class="flex-1 text-center font-mono text-[10px] text-gray-400 font-bold uppercase tracking-widest"
								>
									bash — deploy
								</div>
							</div>
								...
								<div class="space-y-6">
									...
									{#if form.goalConfig.type === 'click_through'}
										...
										<div class="space-y-2">
											...
											<div
												class="bg-black/50 p-4 rounded-lg border-[2px] border-gray-800 overflow-x-auto text-xs leading-loose"
											>
												<span class="text-gray-500">&lt;!-- A/B Tested Dynamic Image --&gt;</span>
												<span class="text-blue-400">&lt;img</span>
												<span class="text-green-400">src=</span><span class="text-amber-300"
													>"{previewUrl}"</span
												> <span class="text-blue-400">/&gt;</span>

												<span class="text-gray-500">&lt;!-- Call to Action Button --&gt;</span>
												<span class="text-blue-400">&lt;a</span>
												<span class="text-green-400">href=</span><span class="text-amber-300"
													>"{clickTrackUrl}"</span
												><span class="text-blue-400">&gt;</span>
												Continue
												<span class="text-blue-400">&lt;/a&gt;</span>
...
{'}'})</pre>
									...
									{:else}
										...
										<div class="space-y-2">
											...
											>
												...
												<span class="text-green-400">src=</span><span class="text-amber-300"
													>"{previewUrl}"</span
												> <span class="text-green-400">alt=</span><span class="text-amber-300"
													>"Dynamic presentation"</span
												> <span class="text-blue-400">/&gt;</span>
											</div>
										</div>
									{/if}
				...
				<div
					class="mt-8 bg-gray-50 border-[3px] border-gray-900 rounded-xl p-5 shadow-brutal-xl flex flex-col md:flex-row items-center justify-between gap-4"
				>
					<button
						type="button"
						on:click={prevStep}
