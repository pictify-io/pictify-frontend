<script>
	...
	let form = {
		...
		variants: [
			{ id: 'default', name: 'Fallback', weight: 10000, isDefault: true, variables: {}, templateUid: '', conditions: { type: 'group', operator: 'AND', children: [] }, priority: 0 },
			{ id: 'variant-1', name: 'Rule Variant 1', weight: 0, isDefault: false, variables: {}, templateUid: '', conditions: { type: 'group', operator: 'AND', children: [] }, priority: 1 },
		],
	};
	...
	$: ruleVariantIndices = form.variants.map((v, i) => ({ variant: v, index: i })).filter(x => !x.variant.isDefault);
	...
	async function renderVariantPreview(variantIndex) {
		...
		if (!tplData?.fabricJSData) {
			variantPreviews[variantIndex] = { url: null, loading: false, error: 'No canvas data available' };
	...
	function buildRuleSummary(conditions) {
		...
		if (conditions.type === 'group') {
			...
			const parts = conditions.children
				.map(c => buildRuleSummary(c))
				.filter(Boolean);
...
<div class="min-h-full px-4 py-8 sm:px-6 lg:px-8">
	<div class="mx-auto transition-all duration-300 {currentStep === 'rules' ? 'max-w-full' : 'max-w-5xl'}">
		<!-- Back Button -->
		<a
		...
		<div class="mb-6">
			<div class="flex items-center gap-3">
				...
				<span class="hidden sm:inline-flex px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest border-[2px] border-gray-900 shadow-[2px_2px_0_0_#1f2937] bg-[#3b82f6]/20">
					Smart Link
				</span>
			</div>
			<p class="mt-2 text-sm font-bold text-gray-500">
				{isEditMode ? 'Update your smart link routing rules.' : 'Serve different images based on viewer context — device, location, time, and more.'}
			</p>
		</div>
		...
		{#if currentStep === 'basics'}
			<div class="bg-white border-[3px] border-gray-900 rounded-xl overflow-hidden shadow-[6px_6px_0_0_#1f2937]">
				<div class="bg-gray-50 border-b-[3px] border-gray-900 px-6 py-4">
					<h2 class="text-sm font-black uppercase tracking-widest text-gray-900">Configure your smart link</h2>
					<p class="text-[10px] font-bold uppercase tracking-wider text-gray-500 mt-0.5">Rule-Based Routing</p>
				</div>
				<div class="p-6 space-y-6 bg-[#FFFDF8]">
					...
					<div>
						...
						{#if !isEditMode}
							...
							{/if}
						{/if}
						<div class="mt-3 flex items-center gap-3 px-4 py-3 bg-gray-100 border-[3px] border-gray-200 rounded-xl overflow-hidden">
				...
				<div class="px-6 py-4 bg-gray-50 border-t-[3px] border-gray-900 flex justify-between items-center">
					<a href="/dashboard/experiments" class="px-4 py-2.5 text-xs font-black text-gray-500 hover:text-gray-900 uppercase tracking-wide transition-colors">
						Cancel
					</a>
					<button
						type="button"
		...
		{#if currentStep === 'rules'}
			<div class="space-y-6">
				<!-- Explanation banner -->
				<div class="flex items-start gap-3 px-5 py-4 bg-blue-50 border-[2px] border-blue-200 rounded-xl">
					...
					<div>
						...
						<p class="text-xs font-bold text-blue-700 mt-1">
							Rules are evaluated top-to-bottom. The first variant whose conditions match the viewer is shown.
							The fallback variant is served when no rules match.
						</p>
					</div>
				</div>
				...
				{#if fallbackVariant}
					<div class="border-[3px] border-[#4ade80] rounded-xl overflow-hidden shadow-[6px_6px_0_0_#4ade80] transition-all relative">
						...
						<!-- Header -->
						<div class="px-6 py-4 border-b-[3px] border-[#4ade80] bg-[#4ade80]/20 flex flex-col md:flex-row md:items-center justify-between gap-3 relative z-10">
							<div class="flex items-center gap-4">
								<div class="w-9 h-9 bg-[#4ade80] border-[2px] border-gray-900 text-gray-900 rounded-lg flex items-center justify-center shrink-0 shadow-[2px_2px_0_0_#1f2937]">
									<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								...
								<input
									...
									on:input={() => { form.variants = form.variants; }}
								...
								/>
								<span class="px-3 py-1 bg-[#4ade80] border-[2px] border-gray-900 rounded-lg text-[10px] font-black uppercase tracking-widest text-gray-900 shadow-[2px_2px_0_0_#1f2937]">
									Fallback
								</span>
							</div>
							<span class="text-[10px] font-black uppercase tracking-widest text-green-800 bg-[#4ade80]/30 px-3 py-1.5 rounded-lg border-[2px] border-green-300">
						...
						<div class="relative z-10 bg-white/80 backdrop-blur-sm">
							...
							<div class="grid grid-cols-1 lg:grid-cols-[1fr_260px]">
								<!-- Variables -->
								<div class="p-6 space-y-4 border-b-[2px] lg:border-b-0 lg:border-r-[2px] border-gray-200">
									<div class="flex items-center justify-between">
										<label class="text-xs font-black uppercase tracking-widest text-gray-900">Variables</label>
									...
									{:else}
										<div class="bg-gray-50 border-[2px] border-dashed border-gray-300 rounded-xl p-4 text-center">
											<p class="text-xs font-bold text-gray-400">
												{getResolvedTemplateUid(fallbackIndex) ? 'No variables defined.' : 'Select template to view variables.'}
											</p>
										</div>
									{/if}
								</div>
								...
								<div class="p-5 bg-gray-50/50">
									<div class="flex items-center justify-between mb-2">
										<label class="text-[10px] font-black uppercase tracking-widest text-gray-500 flex items-center gap-1.5">
											<svg class="w-3.5 h-3.5 text-[#ffc480]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>
											Preview
										</label>
										{#if variantPreviews[fallbackIndex]?.loading}
											<svg class="w-3 h-3 animate-spin text-gray-400" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
										{/if}
									</div>
									<div class="rounded-lg border-[2px] border-gray-200 bg-gray-100 overflow-hidden flex items-center justify-center p-3 min-h-[180px]">
										{#if variantPreviews[fallbackIndex]?.url}
											<img src={variantPreviews[fallbackIndex].url} alt="Preview {fallbackVariant.name}" class="max-w-full max-h-[200px] object-contain pointer-events-none" />
										{:else if variantPreviews[fallbackIndex]?.error}
											<p class="text-[10px] font-black uppercase text-red-600 text-center">{variantPreviews[fallbackIndex].error}</p>
										{:else}
											<p class="text-[10px] font-bold text-gray-400 text-center">Awaiting template...</p>
										{/if}
									</div>
								</div>
							</div>
				...
				{#each ruleVariantIndices as { variant, index }, ruleNum (variant.id)}
					<div class="border-[3px] border-gray-900 rounded-xl overflow-hidden shadow-[6px_6px_0_0_#1f2937] transition-all relative">
						<div class="absolute inset-0 z-0 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] opacity-30 pointer-events-none"></div>

						<!-- Header -->
						<div class="px-6 py-4 border-b-[3px] border-gray-900 bg-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-3 relative z-10">
							<div class="flex items-center gap-4">
								<div class="w-9 h-9 bg-[#3b82f6] border-[2px] border-gray-900 text-white rounded-lg flex items-center justify-center text-sm font-black shrink-0 shadow-[2px_2px_0_0_#1f2937]">
									#{ruleNum + 1}
								</div>
								<input
									type="text"
							...
							<div class="flex items-center gap-2">
								<!-- Move Up -->
								<button type="button" on:click={() => moveVariant(index, -1)}
									disabled={index <= 1}
						...
						<div class="relative z-10 bg-white/80 backdrop-blur-sm">
							...
							<div class="p-6">
								<RuleBuilder
									...
									isDefault={false}
									on:change={() => { form.variants = form.variants; }}
								/>
							</div>
							...
							{@const customizedCount = varEditors.filter(r => r.value && r.value.trim() !== '').length}
							{@const previewValues = varEditors.filter(r => r.value && r.value.trim() !== '').slice(0, 2)}

							<!-- Peek strip -->
							<div class="flex items-center gap-3 px-4 py-2.5 bg-gray-50 border-t-[2px] border-gray-200">
								<!-- Thumbnail -->
								<div class="w-14 h-10 rounded border-[1.5px] border-gray-200 bg-gray-100 overflow-hidden shrink-0 flex items-center justify-center">
									{#if variantPreviews[index]?.url}
										<img src={variantPreviews[index].url} alt="Thumb" class="w-full h-full object-cover" />
									{:else}
								...
								<div class="flex-1 min-w-0">
									...
									{#if previewValues.length > 0}
										<p class="text-[10px] text-gray-400 truncate mt-0.5">
											{previewValues.map(r => `${humanizeVarName(r.key)}: ${r.value}`).join(' · ')}
										</p>
									{/if}
								</div>
								...
								<button
									type="button"
									on:click={() => { expandedVariants[variant.id] = expandedVariants[variant.id] === false ? true : false; expandedVariants = expandedVariants; }}
								...
								>
									...
									<svg class="w-3.5 h-3.5 transition-transform {expandedVariants[variant.id] !== false ? 'rotate-180' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							{#if expandedVariants[variant.id] !== false}
								<div transition:slide={{ duration: 200 }} class="border-t-[2px] border-gray-200">
									<div class="grid grid-cols-1 lg:grid-cols-[1fr_260px]">
										<!-- Variables -->
										<div class="p-5 space-y-3 border-b-[2px] lg:border-b-0 lg:border-r-[2px] border-gray-200">
											<div class="flex items-center justify-between">
												<label class="text-xs font-black uppercase tracking-widest text-gray-900">Variables</label>
											...
											{:else}
												<p class="text-xs font-bold text-gray-400 text-center py-4">
													{getResolvedTemplateUid(index) ? 'No variables defined.' : 'Select template first.'}
												</p>
											{/if}
										</div>
										...
										<div class="p-5 bg-gray-50/50">
											...
											<div class="rounded-lg border-[2px] border-gray-200 bg-gray-100 overflow-hidden flex items-center justify-center p-3 min-h-[140px]">
												{#if variantPreviews[index]?.url}
													<img src={variantPreviews[index].url} alt="Preview {variant.name}" class="max-w-full max-h-[200px] object-contain pointer-events-none" />
												{:else if variantPreviews[index]?.error}
													<p class="text-[10px] font-black uppercase text-red-600 text-center">{variantPreviews[index].error}</p>
												{:else}
													<p class="text-[10px] font-bold text-gray-400 text-center">Awaiting template...</p>
												{/if}
											</div>
				...
				>
					<div class="w-8 h-8 rounded-full bg-gray-200 group-hover:bg-[#3b82f6] group-hover:text-white flex items-center justify-center transition-colors">
						<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4" />
						</svg>
					</div>
					<span class="text-xs font-black uppercase tracking-wide text-inherit">Add Rule Variant</span>
				</button>

				<!-- Integration Preview -->
				<div>
					<h3 class="text-sm font-black uppercase tracking-widest text-gray-500 ml-2 mb-4">Integration</h3>
					<IntegrationPreview
						slug={form.slug}
				...
				<!-- Action Bar -->
				<div class="bg-gray-50 border-[3px] border-gray-900 rounded-xl p-5 shadow-[6px_6px_0_0_#1f2937] flex flex-col md:flex-row items-center justify-between gap-4">
