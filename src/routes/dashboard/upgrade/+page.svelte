...
<section class="min-h-full max-w-5xl mx-auto flex flex-col pt-4">
	<!-- Discount Banner -->
	{#if discountCode}
		<div class="mb-8 p-4 bg-[#10b981]/10 border-[3px] border-[#10b981] rounded-2xl flex items-center gap-4 shadow-[4px_4px_0_0_#10b981]">
	...
	<div class="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6 mb-8 sm:mb-12">
		...
		<div class="flex flex-col items-start md:items-end gap-2 shrink-0">
			<div class="flex items-center p-1 bg-gray-100 rounded-xl border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f2937]">
				<button
					class="px-4 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all {showAnnual ? 'text-gray-500 hover:text-gray-700' : 'bg-white text-gray-900 shadow-[2px_2px_0_0_#1f2937] border-2 border-gray-900'}"
				...
				<div class="relative">
					<button
						class="px-4 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all {showAnnual ? 'bg-[#ffc480] text-gray-900 shadow-[2px_2px_0_0_#1f2937] border-2 border-gray-900' : 'text-gray-500 hover:text-gray-700'}"
						on:click={() => showAnnual = true}
					>
						Annual
					</button>
					{#if !showAnnual}
						<span class="absolute -top-3 -right-3 px-1.5 py-0.5 bg-[#10b981] text-white text-[10px] font-black uppercase tracking-widest rounded border-2 border-gray-900 shadow-[2px_2px_0_0_#1f2937] rotate-12">
							-20%
						</span>
					{/if}
				</div>
			...
			{#if showAnnual}
				<p class="text-[10px] font-black text-[#10b981] uppercase tracking-widest bg-[#10b981]/10 px-2 py-0.5 rounded border-2 border-[#10b981]">Save 20% with annual billing</p>
			{/if}
		</div>
	</div>
	...
	<div class="flex-1 flex flex-col">
		{#if isLoading}
			<div class="flex flex-col items-center justify-center py-16 flex-1 bg-white rounded-2xl border-[3px] border-gray-900 shadow-[8px_8px_0_0_#1f2937]">
				<Loader size="10" show={true} />
				<p class="text-gray-900 font-bold mt-4 text-sm uppercase tracking-widest">Loading plans...</p>
			</div>
		{:else if error}
			<div class="flex flex-col items-center justify-center py-12 flex-1 bg-white rounded-2xl border-[3px] border-gray-900 shadow-[8px_8px_0_0_#1f2937]">
				...
				<p class="text-gray-900 font-black text-lg uppercase tracking-tight mb-2">Something went wrong</p>
		...
		{:else}
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
				{#each featuredPlans as plan (plan.name + '-' + showAnnual)}
					...
					<div class="flex flex-col relative bg-white rounded-2xl border-[3px] {isPopular ? 'border-[#ff6b6b] shadow-[8px_8px_0_0_#ff6b6b]' : 'border-gray-900 shadow-[8px_8px_0_0_#1f2937]'} {isCurrent ? 'opacity-70' : ''}">

						{#if isPopular}
							<div class="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-[#ff6b6b] text-white text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-xl border-[3px] border-gray-900 shadow-[4px_4px_0_0_#1f2937]">
								Most Popular
							</div>
						{/if}
						...
						<div class="p-6 sm:p-8 flex-1 flex flex-col">
							<div class="mb-6">
								<h3 class="text-3xl font-black text-gray-900 uppercase tracking-tight mb-2">{plan.name === 'Standard' ? 'Pro' : plan.name}</h3>
								<div class="flex items-baseline gap-2 mb-1">
									<span class="text-5xl font-black text-gray-900 tracking-tighter">${displayPrice}</span>
									<span class="text-sm font-bold text-gray-500 uppercase">/mo</span>
									{#if showAnnual && monthlySavings > 0}
										<span class="text-lg text-gray-400 font-bold line-through ml-2 decoration-2">${plan.price}</span>
									{/if}
								</div>
								{#if showAnnual && monthlySavings > 0}
									<p class="text-sm text-[#10b981] font-black uppercase tracking-widest">Save ${monthlySavings * 12}/year</p>
								{:else}
									<p class="text-sm text-gray-500 font-bold uppercase tracking-widest">{formatRequests(plan.request_per_month)} renders/mo</p>
								{/if}
							</div>

							{#if isCurrent}
								<button disabled class="w-full py-4 text-xs font-black uppercase tracking-widest rounded-xl border-[3px] border-gray-300 bg-gray-100 text-gray-400 cursor-not-allowed mb-8">
									Current Plan
								</button>
							{:else}
								<button
							...
							<div class="space-y-4 text-sm font-bold flex-1">
								<div class="flex items-center justify-between pb-3 border-b-2 border-dashed border-gray-200">
								...
								<div class="flex items-center justify-between pb-3 border-b-2 border-dashed border-gray-200">
									...
									<span class="text-gray-900">{formatFeatureValue(quotas.aiCopilot)}{quotas.aiCopilot !== false && quotas.aiCopilot !== null ? '/mo' : ''}</span>
								</div>
