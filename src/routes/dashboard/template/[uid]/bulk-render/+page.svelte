		...
		Papa.parse(file, {
			...
			complete: (parseResults) => {
				...
				if (parseResults.errors.length > 0) {
					toast.set({
						message: `CSV parse error: ${parseResults.errors[0].message}`,
						type: 'error',
						duration: 3000
					});
					return;
	...
	async function startBatch() {
		...
		if (!canStartBatch) {
			if (!selectedApiKey) {
				...
				toast.set({
					message: `Map required variables: ${unmappedRequired.join(', ')}`,
					type: 'error',
					duration: 3000
				});
			}
	...
	async function startPolling(id) {
		...
		while (!pollAbortController.signal.aborted) {
			...
			try {
				...
				if (consecutiveErrors >= maxConsecutiveErrors) {
					toast.set({
						message: 'Failed to get batch status. Please refresh the page.',
						type: 'error',
						duration: 5000
					});
					progress.status = 'failed';
				...
				>
					...
					<div class="p-8 text-center">
						<p class="text-gray-600 mb-6">
							Batch rendering lets you generate hundreds of images from a CSV file in minutes. This
							feature is available on <strong>{PLAN_DISPLAY_NAMES[targetPlan]}</strong> and higher plans.
						</p>
						<div class="bg-gray-50 rounded-xl p-4 mb-6 text-left">
							<h4 class="font-bold text-gray-900 mb-3 text-sm uppercase tracking-wide">
								What you get:
							</h4>
					...
					<div class="lg:col-span-7">
						...
						>
							...
							>
								...
								<div
									class="px-2 py-0.5 bg-white/50 border-2 border-gray-900 rounded text-[10px] font-bold"
								>
									.CSV ONLY
								</div>
							</div>
							<div class="p-8">
								<p class="text-gray-600 font-bold mb-6 text-sm">
									Upload a CSV file where columns match your template variables. The first row must
									be headers.
								</p>
								...
								>
									...
									<div class="absolute inset-0 bg-stripes-gray opacity-5 pointer-events-none" />
									...
									<div class="relative z-10">
										...
										<div class="text-sm font-bold text-gray-500 mt-2">
											or drag & drop your generic CSV file here
										</div>
									</div>
								</label>
								...
								{#if csvFile}
									<div
										class="mt-6 p-4 bg-[#dcfce7] border-[3px] border-gray-900 rounded-xl shadow-[4px_4px_0_0_#166534] flex items-center justify-between animate-bounce-in"
									>
										<div class="flex items-center gap-4">
											<div
												class="w-10 h-10 bg-white border-[2px] border-gray-900 rounded-lg flex items-center justify-center text-lg"
											>
												<svg
					...
					<div class="lg:col-span-5">
						...
						>
							...
							<div class="p-6">
										...
										{#each variables as variable}
											...
											>
												<span class="font-bold text-gray-900 flex items-center gap-2">
													<div
														class="w-1.5 h-1.5 rounded-full {variable.required
															? 'bg-red-500'
															: 'bg-gray-300'}"
													/>
													{variable.name}
												...
												<div class="flex gap-2">
													{#if variable.required}
														<span
															class="text-[10px] font-black uppercase px-1.5 py-0.5 bg-red-100 text-red-600 border border-red-200 rounded"
															>Req</span
														>
													{:else}
														<span
															class="text-[10px] font-black uppercase px-1.5 py-0.5 bg-gray-100 text-gray-500 border border-gray-200 rounded"
															>Opt</span
														>
													{/if}
													<span
														class="text-[10px] font-black uppercase px-1.5 py-0.5 bg-gray-100 text-gray-600 border border-gray-200 rounded"
														>{variable.type}</span
													>
												</div>
											</div>
								...
								{:else}
									<p
										class="text-gray-500 text-sm italic border-2 border-dashed border-gray-200 rounded-lg p-4 text-center"
									>
					...
					<div class="lg:col-span-8">
						...
						>
							...
							<div class="p-6 sm:p-8">
										...
										<div class="group">
											<div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-2">
												<label class="w-48 font-bold flex items-center gap-2 text-gray-900">
													<div
														class="w-6 h-6 rounded bg-gray-100 border-2 border-gray-900 flex items-center justify-center text-xs font-black"
													>
														{variable.name.charAt(0).toUpperCase()}
													</div>
													{variable.name}
													{#if variable.required}<span class="text-red-500 text-lg leading-none"
															>*</span
														>{/if}
												</label>
											...
											<div class="sm:pl-[210px]">
												{#if mappings[variable.name] && previewRows[0]}
													<div
														class="inline-block px-3 py-1.5 bg-[#eff6ff] border border-blue-200 rounded text-xs font-mono text-blue-900 max-w-full truncate"
													>
								...
								{#if batchError}
									<div
										class="mb-8 p-4 bg-red-50 border-[3px] border-red-500 rounded-xl shadow-[4px_4px_0_0_#ef4444]"
									>
										<div class="flex items-start gap-3">
											<svg
												class="w-8 h-8 text-red-500 mt-1 flex-shrink-0"
												fill="none"
												stroke="currentColor"
												viewBox="0 0 24 24"
												><path
											...
											>
											<div>
												<p class="font-black text-red-900 mb-1 uppercase tracking-wide">
													Batch Start Failed
												</p>
									...
									<div class="mb-8 p-4 bg-amber-50 border-[3px] border-amber-400 rounded-xl">
										<div class="flex gap-2">
											...
											<p class="text-sm font-bold text-amber-800">
												You must map these required fields: <span class="font-black underline"
													>{unmappedRequired.join(', ')}</span
												>
											</p>
										</div>
									</div>
								...
								<div class="bg-gray-50 rounded-xl border-[3px] border-gray-200 border-dashed p-6">
									<div class="flex items-center justify-between mb-4">
										...
										>
											<span
												class="w-6 h-6 rounded bg-gray-900 text-white flex items-center justify-center"
											>
												<svg
													class="w-3.5 h-3.5"
													fill="none"
						...
						>
							...
							<ul class="space-y-3">
								<li class="flex gap-2 text-sm text-blue-900 font-medium">
									...
									>
									<span
										>Images are generated in parallel. Large batches may take a few minutes.</span
									>
								</li>
								<li class="flex gap-2 text-sm text-blue-900 font-medium">
									<svg
			...
			{:else if step === 'progress'}
				<div class="max-w-3xl mx-auto">
					...
					>
						...
						<div class="p-8 sm:p-12">
							<div class="mb-8 text-center">
								<svg
									class="w-16 h-16 mx-auto mb-4 text-gray-900 animate-spin-slow"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
									><path
							...
							<!-- Chunky Progress Bar -->
							<div
								class="h-8 w-full bg-gray-100 border-[3px] border-gray-900 rounded-lg overflow-hidden relative mb-8"
							>
							...
							{#if progress.status === 'failed'}
								<div class="text-center p-4 bg-red-50 border-2 border-red-100 rounded-xl mb-6">
									...
									<button
										class="text-sm font-black underline hover:text-red-800"
										on:click={() => (step = 'map')}>Try Again</button
									>
								</div>
							{:else}
					...
					<div class="mt-8 text-center">
						<div
							class="inline-block px-4 py-2 bg-[#eff6ff] text-blue-700 border-2 border-blue-200 rounded-full text-xs font-bold animate-pulse"
						>
				...
				<div class="flex flex-col gap-8">
					...
					{#if errors.length > 0}
						<div class="bg-red-50 border-[3px] border-red-500 rounded-xl p-6">
							<h3 class="font-black text-red-900 uppercase tracking-widest text-sm mb-4">
								Failures ({errors.length})
							</h3>
							<div class="max-h-60 overflow-y-auto space-y-2 pr-2">
								{#each errors as error}
									<div
										class="flex items-start gap-2 bg-white p-3 rounded border border-red-200 text-xs text-red-700"
									>
